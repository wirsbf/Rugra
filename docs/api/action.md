...
- `build_simplify_pool()` 注册 **111 个简化 Rule**（2026-06-29 实测：`grep -cE 'pool\.add_rule'` = 105）。**镜像 Ghidra `oppool1` 精确顺序**（coreaction.cc:5511-5649）：每行标注 Ghidra 源码行号，未移植的 Rule 以 `skip` 注释标注。**2026-06-29 新增**：RuleSubCommute（5577）、RuleFloatSign（5619）、RuleSLess2Zero（5558）。
...
- 接入 `set_default_actions`：`ActionStart` → `ActionHeritage` → **`ActionSpacebase`**（2026-06-29 新增，标记 RSP 为 SPACEBASE）→ `ActionStackPtrFlow` → `ActionSimplify` → `build_simplify_pool()` → `build_cleanup_pool()`。

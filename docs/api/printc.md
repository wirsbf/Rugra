...
- **M5**：legacy `push_partial_symbol` 补 allowCast/finalcast 臂（printc.cc:2018-2029：outtype=消费 vn 的 HighVariable 类型 + `cast_strategy.is_subpiece_cast_endian`，命中则 `(outtype)` 前缀于整条 entry 链前，2044-2047），签名增 `outtype/out_space_bigend/allow_cast` 三参；`op_subpiece` 调用点传 `allow_cast=true`（printc.cc:859）。陈旧注释（"Rugra has no isSubpieceCastEndian"/"SUBPIECE-style cast is a TODO hook"）随重写消除。**M4**（cast.cc:27 promoteSize 路由）：`CastStrategyC::promote_size` 私有无访问器且 cast.rs 在本任务 write-set 外——保留字面量 4 并登记残余（所有锁定语料 x86/x64 int==4，行为等价）。

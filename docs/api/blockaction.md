...
**状态**: 已核对（当前有效，2026-07-16 5-step collapseAll 路径已加，默认 7-phase，flag RUGRA_5STEP=1 切 5-step）
**源代码路径**: `src/blockaction.rs`
**2026-07-16 修复（collapseAll 5-step 路径）**: 新增 `collapse_internal(target_idx: Option<i32>)`（对齐 cc:1768-1851，fixpoint + 第二趟，返回 isolated_count）+ `collapse_all_5step`（字面 5 步：orderLoopBodies → collapseConditions → collapseInternal(NULL) → selectGoto 循环 → collapseInternal(targetbl)，cc:1877-1893）。`collapse_all` 加 `RUGRA_5STEP=1` feature flag 路由到 5-step；默认仍 7-phase（铁律 5：不回退已验证工作）。验证：7-phase 默认 953/953 测试 curl 24/24 defects=0（不变）；5-step（flag on）curl 19/24 干净收敛，5 个函数 selectGoto 耗尽后 clipExtraRoots 优雅回退（7-phase 有 Rugra-specific guards 如 cascade-member 检测、switch-case 提取保护，字面 5-step 尚未纳入，故 5-step 需进一步硬化才能成为默认）。**硬化（续）**：select_goto 耗尽时回退 run_goto_cascade（batch cascade）+ 末尾调 finalize_structure（DEAD-sweep + reindex，emit 前提）。仍 19/24——剩余 gap 是 collapse_internal 的 apply_rules_to_block 仅顶层遍历，不复制 7-phase 的 List/Switch 子块递归。

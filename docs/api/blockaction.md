...
**2026-07-16 修复（collapseAll 5-step 路径）**: 新增 `collapse_internal(target_idx: Option<i32>)`（对齐 cc:1768-1851，fixpoint + 第二趟，返回 isolated_count）+ `collapse_all_5step`（字面 5 步：orderLoopBodies → collapseConditions → collapseInternal(NULL) → selectGoto 循环 → collapseInternal(targetbl)，cc:1877-1893）。`collapse_all` 加 `RUGRA_5STEP=1` feature flag 路由到 5-step；默认仍 7-phase（铁律 5：不回退已验证工作）。验证：7-phase 默认 953/953 测试 curl 24/24 defects=0（不变）；5-step（flag on）curl 20/24 干净收敛，4 个函数（main/file2string/getparameter/glob_set）TIMEOUT。**硬化（续）**：collapse_internal deadline 1s；selectGoto 循环 bounded by max_goto_rounds=8 + goto_deadline=3s + isolated-progress guard（3 轮无进展回退 run_goto_cascade）；per-block 仅 Basic/Copy。剩余 gap：new_block_inf_loop + identify_internal 在自循环 body 上 fixpoint 不收敛（疑似锁获取循环），需 identify_internal 死锁调试。

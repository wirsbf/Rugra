...
E2E（curl 124 fn，fast-release）：exit 0 / 0 panic，defects=0 / numbering=0，skeleton 3085→3051（−34），`selectGoto exhausted` 8→9 hits——8 个残差函数（main/getparameter/glob_set/glob_range/glob_word/match_url/next_url/parseconfig 族，jumptable 邻域不可约环）未解锁（预期内，TRI2-STRUCT-IRREDUCIBLE-TRACE-0001 继续跟踪），next_url 1→2 hits 为同一不可约邻域的两轮 collapse（两轮均 finalize 到 blocks=11，收敛不破坏）。

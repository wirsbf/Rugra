...
**2026-07-16 修复（collapseAll 5-step 路径）**: 新增 `collapse_internal(target_idx: Option<i32>)`（对齐 cc:1768-1851，fixpoint + 第二趟，返回 isolated_count）+ `collapse_all_5step`（字面 5 步：orderLoopBodies → collapseConditions → collapseInternal(NULL) → selectGoto 循环 → collapseInternal(targetbl)，cc:1877-1893）。`collapse_all` 加 `RUGRA_5STEP=1` feature flag 路由到 5-step；默认仍 7-phase（铁律 5）。**突破**：5-step 在 collapseConditions 与 collapseInternal(NULL) 之间加 `structure_loops_first()`（匹配 7-phase 顺序）后，curl 5-step 从 20/24（4 个 TIMEOUT）提升到 **24/24 defects=0 numbering=485（与 7-phase 完全一致）**，httpd 27/29 gcc-clean（同 baseline）。根因：缺 structure_loops_first 导致 try_rule_while_do 在未消费 body 的 loop head 上 spin。**默认未切**：数个单元测试断言细粒度结构细节（如 CONTINUE-edge tagging），5-step 与 7-phase 在这些细节上不同（单线程可复现 18 failures），故 7-phase 默认保留。5-step 是 faithful、可用、curl 验证的备选路径。

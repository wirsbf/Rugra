...
| `EXTERNAL-STUB-SUPPORT-0001` | P2 | IN_PROGRESS | external_stub_wave | 48 个 EXTERNAL 空间桩函数的签名库支持 | `src/{space,translate}.rs`, `examples/curl_decompile.rs`, `docs/api/{space,translate}.md`, `tests/oracle/external_stub_1204.{cc,rs,metadata.json}`, `tools/run_external_stub_oracle.sh` | FULL-CORPUS wave（2026-08-15）：124 语料中 48 项为 EXTERNAL 空间（ELF 无代码，Ghidra 用 FID/DWARF 外部声明给出 `free(void *__ptr)` 等签名桩）；最小面=space 层 IPTR_EXTERNAL/EXTERNAL 空间注册对齐 + driver 侧 .dynsym 导入签名桩投影；funcdata.rs 有 CALLGUARD WIP 不碰 | 2026-08-15 |
| `STATIC-LOCAL-NAMING-0001` | P3 | READY | unassigned | 静态局部作用域命名（golden `my_get_token::save`） | `src/{varmap,printc}.rs`, docs | DWARF wave：类型已导入、`作用域名::静态局部` 命名法未接（Ghidra Scope 作用域链）；依赖 symbol 驱动声明链 | 2026-08-15 |

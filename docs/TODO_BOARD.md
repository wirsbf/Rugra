...
| `CSPEC-TEXT-INGEST-0001` | P0 | BLOCKED | unassigned | production `.cspec` bytes → DOM/TreeDecoder → `Architecture::parseCompilerConfig` | `src/marshal.rs`, `docs/api/marshal.md`, `src/arch.rs`, `docs/api/arch.md`, `tests/oracle/cspec_text_ingest_1204.{cc,rs,metadata.json}`, `tools/run_cspec_text_ingest_oracle.sh`, `tests/oracle/fixture_registry.json`, `docs/TODO_BOARD.md` | 依赖`CSPEC-PARAMMODEL-0001`提交并释放`arch.rs`。必须从锁定真实cspec bytes按文档序解析/dispatch stackpointer、default/prototype/eval并在全部成功后才发布Architecture；worker携带immutable bytes+SHA与language/compiler-spec identity，禁止按cwd临时重读或硬编码ABI。错误矩阵覆盖malformed XML、unknown register、duplicate/multiple/no default、invalid pentry及未发布回滚；`<modelrule>`未实现须保持MISMATCH | 2026-08-14 |
| `FUNCPROTO-MODEL-BIND-0001` | P0 | BLOCKED | unassigned | `Funcdata`/DWARF prototype绑定Architecture resolved default model | `src/{funcdata,debugproto}.rs`, `docs/api/{funcdata,debugproto}.md`, `examples/curl_decompile.rs`, `tests/oracle/funcproto_model_bind_1204.{cc,rs,metadata.json}`, `tools/run_funcproto_model_bind_oracle.sh`, `tests/oracle/fixture_registry.json`, `docs/TODO_BOARD.md` | 只读审计首差异：locked named Funcdata ctor经Scope立即绑定`Architecture::defaultfp`，Rust ctor却生成`arch=None/model=None`，DWARF overlay随后锁成非法`model_locked && !has_model`。依赖`CSPEC-TEXT-INGEST-0001`；infer/decompile两个worker均须先验证cspec指纹并完成Architecture，再原子绑定同一default model Arc，DWARF clone必须保留map/default/ctor/overlay四方identity，失败前后fd完整状态相同。完整Scope/ProtoStore、modelrule storage与CallSpecs/Action eval-model调用闭包仍另属MISMATCH | 2026-08-14 |

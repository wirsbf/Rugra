...
| `MERGE-PERSISTENT-STATE-0001` | P1 | MATCH | merge_persistent_wave | 独立 merge Action 的 cover/testCache 前提 | `src/merge.rs`, `src/funcdata.rs`（仅持久 Merge 挂载点 4 hunks）, `docs/api/{merge,funcdata}.md`, `tests/oracle/merge_persistent_1204.{cc,rs,metadata.json}`, `tools/run_merge_persistent_oracle.sh`, docs/TODO_BOARD.md 该行 | evidence=`pending root commit`；REWORK 完成（复核 M1/M2/N3 全闭环）：**M1**=attach_depth 计数替换 attached 布尔（仅 0→1 take、仅 1→0 回写，嵌套入口纯 no-op，镜像单一持久对象）；**M2**=merge_adjacent 补 mergeTestAdjacent（merge.cc:175-218 已有 port 接入）+ outputTypeLocal==inputTypeLocal 门（:1001，比较族 BOOL-out/INT-in 元类型规则，typeop.cc:925-1068 ctor 表）+ 吸收 `MERGE-HIGHCOVER-UNION-0001`（merge_force 三分支吸收实例后置 HighVariable COVERDIRTY=variable.cc:660-663 mergeInternal 副作用，updateCover 重建并集）+ compute_varnode_covers def-no-read 约定修正（零读者=点、更晚块读者=[def,MAX]，对齐 oracle rebuild 编码）；**N3**→`MERGE-INFLATE-TEST-STATE-0001` 登记。fixture 三 case（ladder 4 行/disjoint 4 行/pipeline_tail 5 行含 merge_all 生产入口，消嵌套盲区）：权威 runner（pinned base=e87ebfc54444+merge.rs overlay+funcdata 四 hunk 确定性重建）exit0 **13/13 行逐字节一致 overall=MATCH**（双侧 stdout sha=a02246eb…）；merge:: 4/4、全 lib 5 失败=pristine 基线集（comment/dynamic/funcdata×2/ruleaction）；E2E 124/124 全绿 0 timeout/panic，12.0.4 差分 defects=0、skeleton 4388→4198、numbering 7→24（见 ## Differential 说明）；测试前提修正 2 处（single_entry 测试补 LOAD spacebase flag+entry 挂接先建 high 置 SYMBOLDIRTY=生产管线序）。残差：marker/trim 路径、testCache/copyTrims 计数（Ghidra 默认 private）、Funcdata::clear 生命周期 oracle 级 UNTESTED；typeop 元类型表完整化归 TYPEOP | 2026-08-16 |

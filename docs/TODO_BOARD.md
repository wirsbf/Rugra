...
| `MERGE-PERSISTENT-STATE-0001` | P1 | REWORK | merge_persistent_wave | 独立 merge Action 的 cover/testCache 前提 | `src/merge.rs`, `src/funcdata.rs`（仅持久 Merge 挂载点 4 hunks）, `docs/api/{merge,funcdata}.md`, `tests/oracle/merge_persistent_1204.{cc,rs,metadata.json}`, `tools/run_merge_persistent_oracle.sh`, docs/TODO_BOARD.md 该行 | **复核 REJECT（2026-08-16，机制 C）**：M1=detach 无嵌套深度守卫——merge_all 内层 merge_marker 的 detach 中途回收通道，compute_varnode_covers 读空 live_set（探针实证 len=0，E2E 1043 行漂移；fixture 分步驱动为盲区）；修正=attach_depth 计数（仅 0→1 take/1→0 回写）+补 merge_all 驱动 case。M2=copy_shadow 修复后 mergeadjacent 仍 5/5/5 vs 4/4/4——merge_adjacent 缺 mergeTestAdjacent（merge.cc:175-218）与 outputTypeLocal==inputTypeLocal 门（:996-1004），须一并移植（并吸收 `MERGE-HIGHCOVER-UNION-0001`）。N3=inflate_test 无状态 vs getMerge().inflateTest 消费持久 testCache（merge.cc:1620），登记覆盖面缺口。复核确认通过项（merge_speculative 新序/calcCover 前提/clear 相邻序/13 入口覆盖/runner 6/8/merge:: 4/4/E2E 124/124）保持原样，重做后需新一轮复核 | 2026-08-16 |

...
| `FUNC-GLOBRANGE-HANG-0001` | P0 | READY | unassigned | `glob_range`(0x4d60) 挂起（最后 trace：erase_descend 循环） | `src/varnode.rs`, `src/funcdata.rs`, `docs/api/{varnode,funcdata}.md`, timeout isolation 日志 | E2E（2026-08-15）：worker >10s 超时、直跑 120s exit=124，最后 trace 为 erase_descend 循环；08-14 基线工件中成功。VARNODE-BANK wave 根因线索（2026-08-15）：`erase_descend`(varnode.rs:1761) 本身有界；唯一无界循环是 `funcdata.rs:1203 total_replace` 的 `loop`；**关键死锁点**=descend 中 `Weak::upgrade()` 失败的死条目使 `position()` 永不匹配 → 依赖收缩的按身份清理循环永不终止；死 Weak 意味着 op Arc 被释放却未对所有 reader 走 opUnsetInput。须超时点 dump descend 死/活 Weak 计数并审计 op_destroy/mark_dead 路径，对照 Ghidra `Varnode::eraseDescend`/descend list 语义修复 | 2026-08-15 |

...
| `RUN-NONDETERM-0001` | P1 | IN_PROGRESS | run_nondeterm_diag | glob_word 等函数 run-to-run 非确定（vn_ 编号 65c↔65e） | `src/`, 诊断（只读） | DWARF wave 实测（与 PIPE-SNAPSHOT-2026-08-13 记录一致）：同输入两次运行 vn_ 编号漂移——违反差分可重复前提；须定位非确定源（HashMap 迭代序/时间戳/地址布局），golden 级对齐前必须清零。只读诊断中：复现漂移率→定位首漂移点→/tmp 补丁因果验证→产出修复 TODO 拆分 | 2026-08-15 |

...
| `COVER-REBUILD-COVERAGE-0002` | P2 | READY | unassigned | cover_rebuild_1204 补 "MULTIEQUAL 读 implied 中间变量" 用例 | `tests/oracle/cover_rebuild_1204.{cc,rs,metadata.json}`, `tools/run_cover_rebuild_oracle.sh` | 独立 cross-review 指出 root-vs-current 恒等修复（addRefPoint 恒以 root 匹配 MULTIEQUAL 槽）的判别场景未被 fixture 观察（implied_chain 的 reader 是 INT_OR）；补一个 implied 中间变量被 MULTIEQUAL 读取的 case，把 metadata coverage 的 `multiequal_reader_of_implied_intermediate` 从 UNTESTED 升 MATCH；重 pin hash 并重跑 runner | 2026-08-15 |
...
| `MERGE-PERSISTENT-STATE-0001` | P1 | MATCH | merge_persistent_wave | 独立 merge Action 的 cover/testCache 前提 | `src/merge.rs`, `src/funcdata.rs`（仅持久 Merge 挂载点 4 hunks）, `docs/api/{merge,funcdata}.md`, …（三轮：M1/M3 实证通过——负例 canary 真炸、groups 反例证方向判别力、13 入口深度对称、runner 13/13、E2E 124/124、defects=0；仅剩 R2-M2 两表条目：FLOAT_TRUNC 应 (Int,Float) 误为 (Int,Int)（merge.rs:2824 vs typeop.cc:1913）；BaseNoChar 等值语义反了（type.cc:3619-3626 仅 s==1&&Int 时异于 Base），修后补 shift/float-trunc fixture case 重 pin）| 2026-08-16 || `MERGE-PREEXISTING-GATES-0001` | P2 | READY | unassigned | 复核确认的 merge 家族预存在门/方向缺口（不在 MERGE-PERSISTENT diff 内） | `src/merge.rs`, `docs/api/merge.md` | 第三轮复核预存在观察（2026-08-16 登记）：①mergeIndirect 缺 isAddrForce 门/输入方存活语义（merge.cc:846-887）+ allocateCopyTrim 兜底；②buildDominantCopy 用带 cache 的 merge 而 oracle :1236 传 null cache；③merge_multi_entry 缺 :937 mergeTestRequired 门（setMergeProblems/setUnmerged 计数路径）；④compare_just_loc 缺 space 维（variable.cc:439-443 为 Addr 全序）；⑤merge_highs (Some,Some)→false vs oracle throw（Ghidra 侧不可达，保守降级）| 2026-08-16 |
...
| `EXTERNAL-STUB-SUPPORT-0001` | P2 | READY | unassigned | 48 个 EXTERNAL 空间桩函数的签名库支持 | `src/{space,translate}.rs`, fspec, docs | FULL-CORPUS wave（2026-08-15）：124 语料中 48 项为 EXTERNAL 空间（ELF 无代码，Ghidra 用 FID/DWARF 外部声明给出 `free(void *__ptr)` 等签名桩）；需 EXTERNAL 空间语义+导入签名库；等 ADDRESS/SPACE 租约释放 | 2026-08-15 |
...
| `RUN-NONDETERM-0001` | P1 | READY | unassigned | glob_word 等函数 run-to-run 非确定（vn_ 编号 65c↔65e） | `src/`, 诊断 | DWARF wave 实测（与 PIPE-SNAPSHOT-2026-08-13 记录一致）：同输入两次运行 vn_ 编号漂移——违反差分可重复前提；须定位非确定源（HashMap 迭代序/时间戳/地址布局），golden 级对齐前必须清零 | 2026-08-15 |

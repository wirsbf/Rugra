...
| `PARSECONFIG-RULE-PAIRS-0001` | P0 | AUDITING | unassigned | parseconfig越过DeadCode后`ruleaction.rs`空pair panic | 只读诊断先锁定Rule/输入状态与Ghidra调用闭包；实现write-set待根因确认 | `DEADCODE-SELFLOOP-0001`候选使parseconfig不再自锁，随后在`src/ruleaction.rs:5085`出现`res==1`但`result.pairs`为空；必须用固定prestate确认具体Rule与上游状态，禁止在unwrap处加默认值。诊断需记录locked Ghidra函数、四类语义、最小fixture与依赖DAG后再认领源码 | 2026-08-14 |

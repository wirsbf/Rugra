...
| `ACTION-OPCODE-REDISPATCH-0001` | P0 | IN_PROGRESS | next_url_panic | `ActionPool::processOp` 每条 Rule 后 opcode/dead 检查、新 opcode重派发与 Architecture诊断闭包 | `src/{action,arch}.rs`, `docs/api/{action,arch}.md`, `tests/oracle/action_opcode_redispatch_1204.{cc,rs,metadata.json}`, `tools/run_action_opcode_redispatch_oracle.sh`, `docs/TODO_BOARD.md` | locked `action.cc:822-875` + `sleigh_arch.hh:138`；当前 Rust 在整份旧 opcode rule list 完成后才检测opcode，令`next_url`把已改成INT_ZEXT/仅1输入的op误派给旧SUBPIECE规则；返回0改opcode还须原文诊断，Rust Architecture现自创`[ARCH]`前缀。须逐rule立即检查并从新opcode rule index0重派发，禁止给Rule加guard/cap；fixture覆盖正变化和返回0变化，完整比较调用序、return/count、op状态及原始stderr；修后重跑`next_url`与完整curl | 2026-08-14 |
| `DEADCODE-SELFLOOP-0001` | P0 | IN_PROGRESS | remaining_timeout_small | `ActionDeadCode::{apply,pushConsumed,propagateConsumed}` + Varnode `VAC_CONSUME/LIS_CONSUME` 状态机 | `src/{coreaction,varnode}.rs`, `docs/api/{coreaction,varnode}.md`, `tests/oracle/action_deadcode_selfloop_1204.{cc,rs,metadata.json}`, `tools/run_action_deadcode_selfloop_oracle.sh`, `docs/TODO_BOARD.md` | locked `coreaction.cc:3556-3650,3925-4010` + `varnode.hh` consume accessors；当前 Rust 持 output读锁后写同一self-loop VN而死锁，且错误地以“output有descendant”seed全部assignment inputs、缺VAC/LIS首次零传播与去重。须恢复call/nonassignment/autolive分支、LIFO worklist与精确flags顺序，不能跳过self-loop；fixture完整对拍普通assignment、自环phi、auto-live的flags/consume/return/count、op/VN/def-use与遍历顺序，修后4个超时函数及完整curl复跑 | 2026-08-14 |

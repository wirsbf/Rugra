...
| `HERITAGE-ADT-RENAME-0001` | P0 | IN_PROGRESS | heritage_adt_rename_wave | `buildADT/placeMultiequals/rename` → 23 `MULTIEQUAL` | `src/{heritage,block,varnode,cover}.rs`, `docs/api/{heritage,block,varnode,cover}.md`, `tests/oracle/heritage_adt_rename_1204.{cc,rs,metadata.json}`, `tools/run_heritage_adt_rename_oracle.sh`, TODO 行 | 依赖 `HERITAGE-OWNERSHIP-0001`(DONE 0618b1c)+`HERITAGE-CALLGUARD-0001`(126b56f guardCalls 已接线)；canonical place_multiequals 消费 disjoint tasklist + collect NEW/OLD 分类 + refinement + 块首 phi 插入 + renameRecurse phi 环旧标记跳过；direct 生产路径不切（DRIVER-SWITCH 另任务）；逐phi验证block开头创建序、storage、输入identity与reverse predecessor slot | 2026-08-15 |
...
| `NONDETERM-DOMFRONT-0001` | P0 | IN_PROGRESS | heritage_adt_rename_wave（并入） | heritage df 迭代确定性 | `src/heritage.rs`, `src/block.rs`, `docs/api/heritage.md` | RUN-NONDETERM 诊断（2026-08-16，/tmp/nondiag 全套证据）：根因=place_multiequals_direct 的 df HashSet 随机迭代（SipHash 种子）→MULTIEQUAL 创建序随机→uniqid 错位→全语料 100% 漂移+main 偶发 isolated-changed 失败。最小修=sort 后迭代（20/20 字节一致已验证）；第二层=calcMultiequals pq/augment/merge 有序向量忠实移植（并入 HERITAGE-ADT-RENAME-0001，注意 oracle 序非块索引序）。验收=20 连跑 All sha 一致+20 compare main 全 MATCH+白名单复核 | 2026-08-16 |

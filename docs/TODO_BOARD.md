...
| `PARAM-RECOVERY-0001` | P0 | BLOCKED | unassigned | `ActionInputPrototype` / `ActionUnjustifiedParams` / own-function `FuncProto` recovery | `src/{coreaction,action,fspec,funcdata}.rs`, `src/type_system/protomodel.rs`, paired API, locked oracle fixture, curl differential | 依赖 `SPACE-0001` + `ADDRESS-0001` + `ARCH-0001` + `FSPEC-0001/0002`；当前 invented `ActionInferParams` 先 seed，随后 `ActionUnjustifiedParams` 把所有 used live-in 追加为参数（curl `myprogress` 5→13、`getparameter` 5→35）；必须按 `possibleInputParam→ParamActive→resolve/derive→updateInputTypes` 恢复并对拍完整 prototype/IR mutation，完成后删除硬编码 count/type/ABI 表；诊断见 `FUNCTION_SIGNATURE_PARAMETERS_2026-08-12.md`，B2=`NO_ORACLE` | 2026-08-12 |
| `DWARF-PROTO-0001` | P0 | BLOCKED | unassigned | ELF DWARF subprogram/formal-parameter/varargs → locked `FuncProto` | `examples/curl_decompile.rs`, DWARF/type/database importer, paired docs/oracle fixture, `docs/TODO_BOARD.md` | 依赖 `TYPE-0001` + `DATABASE-0001` + `PARAM-RECOVERY-0001`；`examples/curl` 已含 `myprogress/GetStr/helpf/getparameter` 完整 DIE，但当前 driver 只手写少量 struct/global 类型；逐项保留 name/type/storage/location-list/varargs/optimized origin，并验证 debug 与 stripped 两路径 | 2026-08-12 |
| `PRINT-SIGNATURE-0001` | P0 | BLOCKED | unassigned | `PrintC::docFunction` → canonical `emitFunctionDeclaration` | `src/printc.rs`, `docs/api/printc.md`, locked prototype/text fixture, curl differential, `docs/TODO_BOARD.md` | 依赖 `PARAM-RECOVERY-0001`；删除 production `main` 特判、任意 RAX-write return heuristic 和 empty-proto 六寄存器 fallback；同一 finalized `FuncProto` 对拍 return/convention/name/fixed+varargs/type/order/markup，11.3.2 golden 仅回归信号；模块保持 L2 | 2026-08-12 |

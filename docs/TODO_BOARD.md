...
| `MARSHAL-ID-0001` | P0 | IN_PROGRESS | marshal_id_wave | 12.0.4 `AttributeId`/`ElementId` 固定表、reverse lookup 与结束哨兵 | `src/marshal.rs`, `docs/api/marshal.md`, `tests/oracle/marshal_id_1204.{cc,rs,metadata.json}`, `tools/run_marshal_id_oracle.sh`, `tests/oracle/fixture_registry.json`（root串行登记）, `docs/TODO_BOARD.md` | 锁定进程级ID表；`0`只表示遍历结束，`ATTRIB_UNKNOWN=159`、`ELEM_UNKNOWN=289`，不得按输入/注册顺序动态发号。fixture覆盖完整name↔id、unknown、重复初始化及SPACE=20/SIZE=19；不在本原子修改Translate | 2026-08-14 |

...
| `MARSHAL-ID-0001` | P0 | READY | unassigned | 12.0.4 `AttributeId`/`ElementId` 固定表、reverse lookup 与结束哨兵 | `src/marshal.rs`, `docs/api/marshal.md`, `tests/oracle/marshal_id_1204.{cc,rs,metadata.json}`, `tools/run_marshal_id_oracle.sh`, `tests/oracle/fixture_registry.json`（root串行登记）, `docs/TODO_BOARD.md` | 锁定进程级ID表；`0`只表示遍历结束，`ATTRIB_UNKNOWN=159`、`ELEM_UNKNOWN=289`，不得按输入/注册顺序动态发号。fixture覆盖完整name↔id、unknown、重复初始化及SPACE=20/SIZE=19；不在本原子修改Translate | 2026-08-14 |
...
| `CSPEC-RANGEPROPS-0001` | P0 | READY | unassigned | `RangeProperties::decode` 的space/range/register状态 | `src/address.rs`, `docs/api/address.md`, `tests/oracle/range_properties_1204.{cc,rs,metadata.json}`, `tools/run_range_properties_oracle.sh`, `tests/oracle/fixture_registry.json`（root串行登记）, `docs/TODO_BOARD.md` | 锁定`GH12-F-87faea0d5fa6006d5091`；补`spaceName,first,last,isRegister,seenLast`及ordered decode/error状态。仅批准字符串space投影；动态AddrSpace应用继续依赖`SPACE-0001`/`ADDRESS-0001` | 2026-08-14 |

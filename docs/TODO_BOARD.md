...
| `VARNODE-INIT-0001` | P0 | IN_PROGRESS | varnode_init_wave | `Varnode` constructor + `VarnodeBank` xref/ordering/cover/canonical caller closure + `termOrder` | `src/{varnode,funcdata,heritage}.rs`, `docs/api/{varnode,funcdata,heritage}.md`, `tests/oracle/varnode_init_1204.{cc,rs,metadata.json}`, `tools/run_varnode_init_oracle.sh`, `docs/TODO_BOARD.md` | 首轮独立复核 REJECT 后扩为真实高扇出地基：修 constructor flags/type、unique起点、Loc/Def comparator、xref duplicate canonical Arc/descendant重接、lazy cover、remove→mutate→reinsert、setInput/setDef/makeFree/destroy 及**全部**生产/测试调用闭包；phase3必须先snapshot/drop op read guard并消费canonical返回，禁止 `let _`/unwrap；public destroy返回错误，只有 locked guard 已证明 detached+bank-owned 的内部路径可走带debug断言的prevalidated helper；同步闭合 `termOrder` 的 INT_MULT系数剥离与space+offset键供 `RULE-COLLECTTERMS-0001`。`pcodeparse` 的27个Result告警已核实是独立parser标点缺口，不属于本租约。fail-closed runner需在最终调用闭包冻结后刷新并重新独立复核；Cover endpoint残差保持MISMATCH，模块不升L3 | 2026-08-13 |
...
| `HERITAGE-OWNERSHIP-0001` | P0 | BLOCKED | unassigned | `Funcdata::opHeritage` + `Heritage` explicit-borrow ownership boundary | `src/{heritage,funcdata}.rs`, `docs/api/{heritage,funcdata}.md`, `tests/oracle/heritage_ownership_1204.{cc,rs,metadata.json}`, `tools/run_heritage_ownership_oracle.sh`, `tests/oracle/fixture_registry.json`, `docs/TODO_BOARD.md` | 依赖 `VARNODE-INIT-0001` 提交释放同文件租约；移除 Heritage 对 Funcdata Weak/RwLock 的递归升级，persistent Heritage用`mem::take`暂移、同一`&mut Funcdata`连续3次单pass且pass=0→1→2→3，完整state direct diff并证明无死锁。此原子不切生产Action、不宣称GetStr或Heritage整体MATCH | 2026-08-13 |
...
| `PROTO-EFFECT-MODEL-0001` | P0 | IN_PROGRESS | param_scope_next_wave | `FuncProto::{setModel,hasEffect}` resolved `ProtoModelFull` identity/fallback | `src/fspec.rs`, `docs/api/fspec.md`, `tests/oracle/funcproto_effect_1204.{cc,rs,metadata.json}`, `tools/run_funcproto_effect_oracle.sh` | FuncProto持同一resolved model引用而非复制effect表；local effectlist空时委托model，非空只查override。fixture覆盖RBP/RBX/RSP UNAFFECTED、return/killed/unique、space区分、exact/contained/partial/unknown与copy identity；禁止硬编码SysV。后继由Funcdata property/cspec loader消费，当前不冒充已修GetStr | 2026-08-13 |
| `SCOPE-CAT0-0001` | P0 | IN_PROGRESS | scope_cat0_wave | `Scope::{getCategorySize,getCategorySymbol,setCategory}` category-0 storage | `src/database.rs`, `docs/api/database.md`, `tests/oracle/scope_category_1204.{cc,rs,metadata.json}`, `tools/run_scope_category_oracle.sh` | locked direct完整状态fixture覆盖category-0 slot/holes、cat>0 append、NO_CATEGORY、replace/delete/order/identity；修当前retain+push重排。它是ProtoStoreSymbol前置，不改printer或fspec，不凭文本结果宣称参数已对齐 | 2026-08-13 |
...
| `VARMAP-GATHEROFFSET-0001` | P0 | REVIEW | varmap_mask_wave | `AliasChecker::gatherOffset` 8-byte `calc_mask` boundary | `src/varmap.rs`, `docs/api/varmap.md`, `tests/oracle/varmap_gather_offset_1204.{cc,rs,metadata.json}`, `tools/run_varmap_gather_offset_oracle.sh`, `docs/TODO_BOARD.md` | locked direct stdout已零差异：copy8=`0xfedcba9876543210`、add8回绕=`0x25`、相邻add7 mask=`0x25`，完整前后状态SHA=`92476add…`；实现者正把Rust comparand改为完整crate/build-input隔离快照，禁止live workspace/shared rlib，随后重跑并由独立核心reviewer签字；未获APPROVE前不得提交或升L3 | 2026-08-13 |
...
| `RULE-COLLECTTERMS-0001` | P0 | REVIEW | rule_collectterms_wave | `RuleCollectTerms::applyOp` + `AddTreeState/TermOrder` 调用闭包 | `src/{ruleaction,expression}.rs`, `docs/api/{ruleaction,expression}.md`, `tests/oracle/rule_collect_terms_1204.{cc,rs,metadata.json}`, `tools/run_rule_collect_terms_oracle.sh`, `docs/TODO_BOARD.md` | 系数64位回绕后storage mask、constant phase倒序、ADD-output lone-descend与`Varnode::termOrder`调用闭包均已落；locked 7 cases/14 records stdout direct diff=0，SHA=`c45835fd…`，合法BlockBasic定向测试1/1。当前authoritative runner为PARTIAL_MATCH且等待`VARNODE-INIT-0001`最终crate冻结后刷新pins/重跑/独立APPROVE；OPBANK/ARCH/TYPE残差不得规范化或升L3 | 2026-08-13 |
...
| `SLEIGH-FLOW-REL-0001` | P0 | BLOCKED | sleigh_relative_flow_wave | const-space relative `BRANCH/CBRANCH` → `FlowInfo` internal p-code edges | `src/{disasm/sleigh_lift,flow,funcdata}.rs`, paired `docs/api`, `tests/oracle/sleigh_flow_relative_1204.{cc,rs,metadata.json}`, `tools/run_sleigh_flow_relative_oracle.sh`, `docs/TODO_BOARD.md` | 依赖`VARNODE-INIT-0001`释放funcdata租约；真实x86 `0f a2 c3`当前Rust把33个Const内部branch改Ram并当机器地址，得到78 ops/34 blocks/0 edges且漏RET，locked Ghidra为81 ops/33 Const branches/34 blocks及完整双向边。先做只读oracle/fixture，租约释放后保留VN space、按SeqNum time解析relative target并仅非Const目标入机器worklist；ABI identity不得误映射成Arc alias | 2026-08-13 |
...
| `PIPE-ACTION-COUNT-0001A` | P0 | IN_PROGRESS | block_entry_wave | `ActionStartTypes` count bridge + 6个prototype/call leaves once flags | `src/coreaction.rs`, `docs/api/coreaction.md`, `tests/oracle/action_leaf_count_1204.{cc,rs,metadata.json}`, `tools/run_action_leaf_count_oracle.sh` | 仅补StartTypes `take_count_delta` 与 DefaultParams/ExtraPopSetup/PrototypeTypes/FuncLink/FuncLinkOutOnly/InternalStorage 的 `RULE_ONCEPERFUNC`；fixture覆盖repeat组两轮、reset保留统计及zero-work once lifecycle，不以零边界批准非零遍历 | 2026-08-13 |

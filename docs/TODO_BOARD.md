...
| `MERGE-PERSISTENT-STATE-0001` | P1 | IN_PROGRESS | merge_persistent_wave | 独立 merge Action 的 cover/testCache 前提 | `src/merge.rs`, `src/funcdata.rs`（仅持久 Merge 挂载点）, `docs/api/{merge,funcdata}.md`, `tests/oracle/merge_persistent_1204.{cc,rs,metadata.json}`, `tools/run_merge_persistent_oracle.sh`, docs/TODO_BOARD.md 该行 | 依赖 merge.rs 租约释放（已释放）；PIPE-MERGETYPE wave 发现：Ghidra `data.getMerge()` 为 Funcdata 持久对象（testCache/covers 跨 Action 保持），Rugra 每 Action 新建 `Merge::new()`——重排后独立 merge Action（mergerequired/mergecopy/mergeadjacent）在空 live_set/covers 前提下运行；需等价持久 cover 前提（fixture 现以常量输入规避 divergent 路径，E2E 当前无损） | 2026-08-15 |

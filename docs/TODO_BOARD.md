...
| `OP-INSERT-0001` | P0 | IN_PROGRESS | op_insert_wave | `Funcdata::opInsert*` / `PcodeOpBank` alive + `BlockBasic` parent/ordered list 原子更新 | `src/funcdata.rs`, `docs/api/funcdata.md`, `tests/oracle/op_insert_1204.{cc,rs,metadata.json}`, `tools/run_op_insert_oracle.sh` | 独占 `funcdata`；锁定 `funcdata_op.cc` / `op.cc` 完整函数；验证 before/after/begin/end、parent、block 顺序、alive/dead、双向 xref 的完整 state diff；不得绕过 `ADDRESS/SEQNUM/OPBANK` 其余缺口，未覆盖分支保持 L2 | 2026-08-13 |
| `BLOCK-ENTRY-0001` | P0 | IN_PROGRESS | block_entry_wave | `FlowInfo::generateBlocks` entry block 标志与首块身份 | `src/flow.rs`, `docs/api/flow.md`, `tests/oracle/block_entry_1204.{cc,rs,metadata.json}`, `tools/run_block_entry_oracle.sh` | 与 `PIPE-REACH-0001` raw/CFG 输入相同；当前首块 Ghidra flag=`0x200`、Rugra=`0`；对拍块顺序、entry flag、边、address range，禁止只改 snapshot 文本 | 2026-08-13 |
| `FLOW-TARGET-BOUND-0001` | P0 | READY | unassigned | `FlowInfo::{setFallthruBound,fallthru}` visited join `STARTBASIC` | `src/flow.rs`, `docs/api/flow.md`, `tests/oracle/flow_target_boundary_1204.{cc,rs,metadata.json}`, `tools/run_flow_target_boundary_oracle.sh`, `docs/TODO_BOARD.md` | 依赖 `BLOCK-ENTRY-0001`；锁定 `flow.cc:489-513,545-580` 在已访问target/精确bound命中时标 STARTBASIC；当前只pop导致 `CBRANCH → CALL fallthrough → visited join` 汇合块丢失与fallthrough edge消失；对拍block划分、STARTBASIC、edge/reverse slot顺序 | 2026-08-13 |
| `VARNODE-INIT-0001` | P0 | IN_PROGRESS | varnode_init_wave | `Varnode` constructor / initial flags / type / cover-dirty state | `src/varnode.rs`, `docs/api/varnode.md`, `tests/oracle/varnode_init_1204.{cc,rs,metadata.json}`, `tools/run_varnode_init_oracle.sh` | 锁定 `varnode.cc/.hh` constructor 与 bank create 调用闭包；GetStr 首 VN 当前 `0x1000030/xunknown8` ↔ `0x30/None`；必须对拍 flags/type/storage/def/cover/state，不得以 printer 补类型 | 2026-08-13 |
...
| `PIPE-LIFECYCLE-0001` | P0 | IN_PROGRESS | pipeline_lifecycle_wave | `ActionStart` / `ActionStop` / `ActionPrototypeTypes` locked-input creation | `src/coreaction.rs`, `docs/api/coreaction.md`, `tests/oracle/pipeline_lifecycle_1204.{cc,rs,metadata.json}`, `tools/run_pipeline_lifecycle_oracle.sh` | 独占 `coreaction`；锁定 `coreaction.hh/.cc` 与 `funcdata.cc`；对拍 processing/heritage init/dead-op cleanup、locked input Varnode storage/flags/type/parameter identity；主管线 Action，提交前须独立 Cross-Review + curl differential | 2026-08-13 |
| `HERITAGE-DRIVER-0001` | P0 | AUDITED | heritage_driver_audit | `Heritage::heritage` canonical driver ↔ `ActionHeritage` | 只读审计：`src/{heritage,coreaction,funcdata}.rs`、locked oracle；输出 `docs/alignment_audit/HERITAGE_DRIVER_2026-08-13.md` | evidence=`this commit`；确认生产路径错误执行 direct place/rename 两遍并夹 DeadCode，绕过 canonical single-pass/guard/ADT/refinement；GetStr 缺 20 call INDIRECT + 23 MULTIEQUAL 已按 2 calls×10 ABI ranges 与 block2/5 的 11+12 phi 精确归因；直接切换 nominal driver 会重入 RwLock 写锁，实施须按文档 DAG 自底向上，模块仍 L2/MISMATCH | 2026-08-13 |
| `PIPE-TREE-0001` | P0 | IN_PROGRESS | pipeline_tree_audit | 12.0.4 universal/default action tree 与 Rugra 实际 perform 顺序 | 只读审计：`src/{action,coreaction}.rs`、locked oracle；输出 `docs/alignment_audit/PIPELINE_TREE_2026-08-13.md` | 产出完整有序树首差异、重复/平铺/阶段死代码与依赖；禁止凭最终 C 猜顺序；后续写者须等 `PIPE-LIFECYCLE-0001` 释放 `coreaction` | 2026-08-13 |
| `PARAM-BIND-0001` | P0 | AUDITED | param_bind_audit | locked `FuncProto` input storage → SSA Varnode → Symbol/PrintC 参数名 | 只读审计：`src/{debugproto,fspec,funcdata,coreaction,varmap,printc}.rs`、locked oracle；输出 `docs/alignment_audit/PARAM_BIND_2026-08-13.md` | evidence=`this commit`；首差异在 Funcdata 构造缺 Scope+ProtoStoreSymbol，参数导入再丢 AddrSpace/category-0 Symbol，ActionPrototypeTypes 无 locked-input loop，Heritage 绕过 property/effect；RBP/RBX 因缺 UNAFFECTED 被 Merge input-register guard 错命名 `param_35/param_33`；按文档层级 fixtures 自底向上修，禁止 printer 文本替换 | 2026-08-13 |
| `PRINT-RESIDUAL-0001` | P1 | AUDITED | print_residual_audit | upstream 修复后 `PrintC` 未声明变量/类型/表达式残差分类 | 只读审计：`src/{printc,printlanguage,prettyprint,varmap}.rs`、locked oracle；输出 `docs/alignment_audit/PRINT_RESIDUAL_2026-08-13.md` | evidence=`this commit`；GetStr 在 raw Fspec↔Iop 与 VN flags/type 即分叉，Heritage 156/273↔105/287、Action 14/23↔131/439、structure 1↔3，故先修 upstream；真实 emitter residual 为 callspec消费、声明/命名、RPN/CBRANCH、类型写回及 semantic postprocess；2/23 仅 sink诊断，非对齐率 | 2026-08-13 |
| `VARMAP-GATHEROFFSET-0001` | P0 | READY | unassigned | `AliasChecker::gatherOffset` 8-byte `calc_mask` boundary | `src/varmap.rs`, `docs/api/varmap.md`, locked oracle fixture, `docs/TODO_BOARD.md` | `GetStr` debug stage触发 `(1u64 << 64)` panic；锁定 `varmap.cc:817-855` 返回 `retval & calc_mask(vn->getSize())`，8-byte必须全1 mask；需同输入 offset/alias/state fixture + visible-output differential + 独立核心复核，修复前 snapshot固定release且该分支=`UNTESTED` | 2026-08-13 |
...
| `RULE-COLLECTTERMS-0001` | P0 | READY | unassigned | `RuleCollectTerms::applyOp` coefficient addition modulo storage width | `src/ruleaction.rs`, `docs/api/ruleaction.md`, locked oracle fixture, `docs/TODO_BOARD.md` | `GetStr` debug Action触发 Rust unsigned-add panic；锁定 `ruleaction.cc:107-178` 的 `uintb newcoef = (coef1 + coef2) & mask` 按无符号机器数回绕；需溢出/非溢出完整 IR mutation fixture、curl differential与独立主管线 Rule 复核；未批准前不合入 wrapping 修复 | 2026-08-13 |
...
| `PIPE-0000` | P0 | IN_PROGRESS | root | `Action::{reset,perform}` + group/root execution | `src/action.rs`, `src/bin/rugra.rs`, `examples/{curl_decompile,httpd_decompile,getstr_stage_snapshot,debug_my_fwrite,rugra_decompile_func,debug_cfg,decompile_demo}.rs`, `docs/api/{action,bin/rugra}.md`, `tests/oracle/action_perform_1204.{cc,rs,metadata.json}`, `tools/run_action_perform_oracle.sh`, `docs/TODO_BOARD.md` | 子 Action 一律走 `perform`；root 统一 `reset→perform`，不得直接 `apply`；对拍 status/count/lcount/repeat/once-per-func、group member顺序与 reset mutation；主管线改动须独立 cross-review + GetStr/curl differential，任何 nonconvergence 必须定位错误 reporter 而非加迭代上限 | 2026-08-13 |
| `PIPE-ACTION-COUNT-0001` | P0 | READY | unassigned | core Action 叶节点的 protected `count` 与 rule flags | `src/coreaction.rs`, `src/{merge,blockaction}.rs`（按独立原子批次拆租约）, paired `docs/api`, locked leaf fixtures, `docs/TODO_BOARD.md` | 依赖 `PIPE-0000`；逐类对拍 `ActionStartTypes`/DoNothing/ConstantPtr/MultiCse/ShadowVar/Deindirect 等真实突变后 count，补齐 DefaultParams/ExtraPop/PrototypeTypes/FuncLink/InternalStorage/DoNothing/SetCasts 等约27处 repeat/once flags；禁止按返回值猜变化，主管线每批需独立复核+differential | 2026-08-13 |

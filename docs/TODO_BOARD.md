...
| `RULE-MULTICOLLAPSE-0001` | P0 | IN_PROGRESS | rule_multicollapse_wave | `RuleMultiCollapse::applyOp` 根/嵌套 `MULTIEQUAL` collapse | `src/ruleaction.rs`, `docs/api/ruleaction.md`, `tests/oracle/rule_multi_collapse_1204.{cc,rs,metadata.json}`, `tools/run_rule_multi_collapse_oracle.sh`, `docs/TODO_BOARD.md` | `RULE-COLLECTTERMS-0001`依赖已满足；当前Rust成功分支跳过根output却返回CHANGE，导致main等至少9函数simplifypool不收敛。必须按locked `ruleaction.cc:3234-3343`覆盖absolute root、loop mark、nested skiplist、functional CSE/原位改写/reinsert，完整比较opbank/def-use/marks/order；禁止迭代上限、禁Rule或仅改返回0 | 2026-08-13 |

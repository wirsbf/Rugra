...
`HERITAGE-ADT-RENAME-0001` | P0 | REWORK (交付完成待 root 集成/复核) | heritage_adt_rename_wave | `buildADT/placeMultiequals/rename` → 23 `MULTIEQUAL` | `src/heritage.rs`, `docs/api/heritage.md`, `tests/oracle/heritage_adt_rename_1204.{cc,rs,metadata.json}`, `tools/run_heritage_adt_rename_oracle.sh`, TODO 行 | evidence=`pending root commit`；authoritative runner（pinned base=194901d+src/heritage.rs overlay，锁定 oracle 全量重建）exit0：`MATCH covered_projection=3/3 overall=PARTIAL_MATCH stdout_sha256=52e1096e…`。canonical `place_multiequals`（cc:2599-2645）逐段消费 disjoint TaskList（collect MemRange 形式含 write-mask/marker/clearProperty NEW、refinement fencepost+remove13+tasklist/globaldisjoint 重写、guardInput 补 write-mask+concat、guard new_addresses 门控、calcMultiequals 改吃 write varnode 列表、phi 经 new_op+create_def_with_space+op_set_input+op_insert_begin 落块首）；`rename`/`rename_recurse`（cc:2479-2593）block-0 唯一根、单趟 op 序、精确 op_set_input、phi 环 old-marker skip（cc:2538），不再走 rename_direct（direct 生产路径保持不切）。fixture 3 case 双侧逐字节：diamond 3 phi（块首倒序+reverse predecessor slot+输入 def-opcode 身份）、oldmark（ownership 受限案例升格全投影）、two-join `seq=6,3`（pq/augment merge 序 ≠ 块索引序 witness）。E2E All 124/124 processed/76 decompiled/0 fail；20×All sha 一致（6ddd8133…）、20×compare-main 全 MATCH；lib 5 域外失败经 pinned-base+overlay worktree A/B 证实预存；annotations/refs strict OK；Evidence 4/4 dry-run OK。残差：collect 全 bank 偏移窗口（跨空间碰撞→DRIVER-SWITCH 硬前置）、refinement/guardInput concat/removeRevisitedMarkers 分支 UNTESTED、heritage_ownership/callguard 两 fixture pin 需 root 重钉、fixture_registry 登记归 root | 2026-08-16 |
...
---
| `FUNCDATA-LINKSYMBOL-TYPED-0001` | P0 | IN_PROGRESS | linksymbol_wave | `Funcdata::linkSymbol` 忠实化：SSA 寄存器临时也建带 HighVariable 类型的 ScopeLocal Symbol | `src/{funcdata,coreaction,varmap,merge}.rs`, paired docs, fixture | 认领 2026-08-15；write-set 严格限定：`src/funcdata.rs`、`src/coreaction.rs`、`src/varmap.rs`、`src/merge.rs`（仅 :1280 附近 per-High 命名挪到 Action 期）、`docs/api/{funcdata,coreaction,varmap,merge}.md`、`tests/oracle/linksymbol_typed_1204.*`、`tools/run_linksymbol_typed_oracle.sh`、TODO 行。TYPED-DECL-GAP-0001 诊断（2026-08-15）：Ghidra funcdata_varnode.cc:1156-1184 在无重叠且非 persist 时 `addSymbol("", high->getType(), addr, usepoint)`（golden 的 bVar1/pcVar5 全部由此）；Rugra funcdata.rs:734-763 替换为往 HashMap 塞无类型 `local_XX` 自创名（:758）——`local_XX` 名直接来源；coreaction.rs:4453 还把无名 HighVariable 提前 continue（Ghidra hasName 语义=可命名）。依赖 SCOPE-CAT0/VARMAP-NAMING（DONE）+ COREACTION-SCOPE-CALLER（持久 scope）。验收=probe glob_word 后声明块出现 bVar/pcVar 族 | 2026-08-15 |
| `PRINTC-SYMBOL-DECL-0001` | P1 | READY | unassigned | PrintC 声明改 Symbol 驱动（吸收 PRINTC-SCOPE-RESTRUCT-0001） | `src/printc.rs`, `docs/api/printc.md`, fixture | TYPED-DECL-GAP-0001：移植 emitLocalVarDecls/emitScopeVarDecls/emitVarDecl（printc.cc:2260/2518/2497），声明=sym.dtype+name；删除 doc_variable_decls_from_funcdata GLUE（printc.rs:2903-3072，类型来自打印期 used_varnode_types 的 xunknown8 + 硬编码 is_declarable 白名单 + long/int 兜底）与打印期 restructure 兜底（:4970-4990）；依赖上两项；验收=glob_word 声明块=golden 7 行结构、numbering 归零 | 2026-08-15 |

...
| `COVER-REBUILD-COVERAGE-0002` | P2 | IN_PROGRESS | cover_coverage_wave | cover_rebuild_1204 补 "MULTIEQUAL 读 implied 中间变量" 用例 | `tests/oracle/cover_rebuild_1204.{cc,rs,metadata.json}`, `tools/run_cover_rebuild_oracle.sh` | 独立 cross-review 指出 root-vs-current 恒等修复（addRefPoint 恒以 root 匹配 MULTIEQUAL 槽）的判别场景未被 fixture 观察（implied_chain 的 reader 是 INT_OR）；补一个 implied 中间变量被 MULTIEQUAL 读取的 case，把 metadata coverage 的 `multiequal_reader_of_implied_intermediate` 从 UNTESTED 升 MATCH；重 pin hash 并重跑 runner | 2026-08-15 |
...
| `TYPEFACTORY-UNDEFNAME-0001` | P2 | READY | unassigned | `xunknownN`→Ghidra 数据组织命名 `undefinedN` | `src/type_system/typefactory.rs`, `src/type_system/datatype.rs`, paired docs | TYPED-DECL-GAP-0001：typefactory.rs:103 的 SLEIGH standalone 命名 + datatype print_name_base 取首字符产生 xVar 家族；依赖 TYPE-0001/cspec 数据组织；验收=输出无 xunknown/xVar | 2026-08-15 |

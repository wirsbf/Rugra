...
| `RULE-COLLECTTERMS-0001` | P0 | IN_PROGRESS | rule_collectterms_wave | `RuleCollectTerms::applyOp` + `AddTreeState/TermOrder` 调用闭包 | `src/{ruleaction,expression}.rs`, `docs/api/{ruleaction,expression}.md`, `tests/oracle/rule_collect_terms_1204.{cc,rs,metadata.json}`, `tools/run_rule_collect_terms_oracle.sh`, `docs/TODO_BOARD.md` | 首轮真实oracle/独立复核REJECT并锁定四个地基差异：系数加法须64位回绕后storage mask、constant phase倒序；`AddTreeState::collect`须检查ADD output lone-descend，`sortTerms`须用 `Varnode::termOrder`且常量在尾；`termOrder`由 `VARNODE-INIT-0001` 同步修。隔离快照应用三处底层修案后7 cases/14 records已零差异，但必须在共享冻结树重跑fail-closed runner、curl differential并获独立APPROVE后才能提交 | 2026-08-13 |

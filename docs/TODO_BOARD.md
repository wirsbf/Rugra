...
| `RULE-COLLECTTERMS-0001` | P0 | REVIEW | op_insert_wave | `RuleCollectTerms::applyOp` + `AddTreeState/TermOrder` 调用闭包 | `src/{ruleaction,expression}.rs`, `docs/api/{ruleaction,expression}.md`, `tests/oracle/rule_collect_terms_1204.{cc,rs,metadata.json}`, `tools/run_rule_collect_terms_oracle.sh`, `docs/TODO_BOARD.md` | `VARNODE-INIT-0001` 已满足；系数64位回绕后storage mask、constant phase倒序、ADD-output lone-descend与`Varnode::termOrder`调用闭包均已落，locked 7 cases/14 records direct diff=0，合法BlockBasic定向测试1/1。须基于本次稳定提交刷新crate pin、重跑并取得原reviewer终签后提交；OPBANK/ARCH/TYPE残差不得规范化或升L3 | 2026-08-13 |
| `RULE-MULTICOLLAPSE-0001` | P0 | BLOCKED | rule_multicollapse_wave | `RuleMultiCollapse::applyOp` 根/嵌套 `MULTIEQUAL` collapse | `src/ruleaction.rs`, `docs/api/ruleaction.md`, `tests/oracle/rule_multi_collapse_1204.{cc,rs,metadata.json}`, `tools/run_rule_multi_collapse_oracle.sh`, `docs/TODO_BOARD.md` | 依赖`RULE-COLLECTTERMS-0001`提交释放同文件租约；当前Rust成功分支跳过根output却返回CHANGE，导致main等至少9函数simplifypool不收敛。必须按locked `ruleaction.cc:3234-3343`覆盖absolute root、loop mark、nested skiplist、functional CSE/原位改写/reinsert，完整比较opbank/def-use/marks/order；禁止迭代上限、禁Rule或仅改返回0 | 2026-08-13 |
| `RUNTIME-TIMEOUT-0001` | P0 | IN_PROGRESS | timeout_driver_wave | curl逐函数分析超时隔离与进程回收 | `examples/curl_decompile.rs`, `tools/run_curl_timeout_isolation.sh`, `docs/TODO_BOARD.md` | 当前`recv_timeout`仅停止等待而不取消worker，每个超时遗留满CPU分析线程+joiner并污染后续结果；改为可终止的逐函数子进程/进程组，严格区分timeout/disconnect/panic并验证超时后零子进程、下一函数独立运行。此项只修测量器，不替代`RULE-MULTICOLLAPSE-0001`算法修复 | 2026-08-13 |

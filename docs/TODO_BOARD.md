...
| `SLEIGH-0002B` | P0 | IN_PROGRESS | root + sleigh_abi_fixture | 动态无截断 C ABI + owned image + typed result/error | `sleigh_shim/rugra_sleigh.cpp`, `src/sleigh_ffi.rs`, `docs/api/sleigh_ffi.md`, `examples/sleigh_test.rs`, `tests/oracle/sleigh_decode_1204.{cc,rs,metadata.json}`, `tools/run_sleigh_decode_oracle.sh`, `docs/TODO_BOARD.md` | 依赖 `SLEIGH-0002A`（`87aaef2`，已满足）；root 独占 shim/Rust/API/example，sleigh_abi_fixture 独占 oracle 三文件/runner；一次 oneInstruction 原子返回 step+全序 ops+全部 inputs+typed error；CPUID 78 ops、零-op、BadData/DataUnavail 同输入差分；实现后须独立 Cross-Review | 2026-08-12 |

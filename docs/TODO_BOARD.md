...
| `GATE-SCANNER-0001` | P0 | DONE | annotation_checker_review | Rust fn item scanner + commit/PreToolUse 共用 scope | `tools/rust_fn_scanner.py`, `tools/check_ghidra_annotations.py`, `.zcode/align_gate.py` | evidence commit=this commit；共享 scanner 覆盖 const/extern/restricted visibility/单行 impl、item macro、精确 cfg(test) scope、trait `;` 终点、多编辑重叠与逐次读取 freshness；94 文件零违规；独立终审 APPROVE | 2026-08-12 |

...
| `ACTION-OPCODE-REDISPATCH-0001` | P0 | IN_PROGRESS | next_url_panic | `ActionPool::processOp` 每条 Rule 后 opcode/dead 检查与新 opcode 从 rule index 0 重派发 | `src/action.rs`, `docs/api/action.md`, `tests/oracle/action_opcode_redispatch_1204.{cc,rs,metadata.json}`, `tools/run_action_opcode_redispatch_oracle.sh`, `docs/TODO_BOARD.md` | locked `action.cc:822-875`；当前 Rust 在整份旧 opcode rule list 完成后才检测 opcode，`RuleSubExtComm` 将 SUBPIECE 改为 INT_ZEXT/删除slot1后仍误派给旧 `RuleSubvarSubpiece`，导致 `next_url` panic。须逐rule立即检查并重派发，禁止给Rule加guard或给executor加cap；fixture覆盖正变化及返回0却改opcode的warning路径，完整比较rule调用序、返回/count、opcode/inputs/dead与诊断；修后重跑`next_url`及完整curl | 2026-08-14 |

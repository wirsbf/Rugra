...
| `BLOCK-INDEX-ASSIGN-0001` | P1 | READY | unassigned | `BlockGraph::findSpanningTree` reverse-post-order `FlowBlock::index` assignment | `src/block.rs`, `docs/api/block.md`, `tests/oracle/block_index_assign_1204.{cc,rs,metadata.json}`, `tools/run_block_index_assign_oracle.sh`, `tests/oracle/fixture_registry.json`（root串行登记）, `docs/TODO_BOARD.md` | 由 cover_rebuild_1204 fixture 发现：Ghidra `FlowBlock::index` 仅在 `BlockGraph::findSpanningTree`（block.cc:1004-1120，RPO `curbl->index=rpostcount`）赋值，ctor 全 0；Rust `find_spanning_tree` 是 blockaction.rs 私有方法且不写 `FlowBlock.index`，生产无 RPO index 赋值路径。须移植公共 `BlockGraph::find_spanning_tree`（含 rootlist swap/两遍 extraroots/irreducible 跳过/numdesc/visitcount/copymap 副作用全对齐），fixture 覆盖单/多入口、不可达块、irreducible 边；`RUGRA-GLUE` 不得引入新 index 语义 | 2026-08-15 |

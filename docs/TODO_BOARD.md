...
| `ANN-A` | P0 | IN_PROGRESS | ann_options | `options.rs` 44 个（37×`name` + parse/collection/default helpers） | `src/options.rs`, `docs/api/options.md` | 精确集合=`find_fn_violations`@fcc5142；验收指定文件 annotation+refs+doc-sync；evidence commit=pending | 2026-08-11 |
| `ANN-B` | P0 | IN_PROGRESS | ann_flow_callgraph | `flow.rs` 18 个 + `callgraph.rs` 2 个 accessor/helper | `src/{flow,callgraph}.rs`, `docs/api/{flow,callgraph}.md` | 精确集合=`find_fn_violations`@fcc5142；验收指定文件 annotation+refs+doc-sync；evidence commit=pending | 2026-08-11 |
| `ANN-C` | P0 | IN_PROGRESS | ann_fspec | `fspec.rs` 18 个 parameter/default/helper 函数 | `src/fspec.rs`, `docs/api/fspec.md` | 精确集合=`find_fn_violations`@fcc5142；验收指定文件 annotation+refs+doc-sync；evidence commit=pending | 2026-08-11 |
| `ANN-D` | P0 | IN_PROGRESS | ann_signature_memstate | `signature.rs` 12 个 + `memstate.rs` 14 个 container/accessor 函数 | `src/{signature,memstate}.rs`, `docs/api/{signature,memstate}.md` | 精确集合=`find_fn_violations`@fcc5142；验收指定文件 annotation+refs+doc-sync；evidence commit=pending | 2026-08-11 |
| `ANN-E` | P0 | IN_PROGRESS | ann_cover_funcdata | `cover.rs` 12 个 + `funcdata.rs` 7 个 CFG/helper 函数 | `src/{cover,funcdata}.rs`, `docs/api/{cover,funcdata}.md` | 精确集合=`find_fn_violations`@fcc5142；验收指定文件 annotation+refs+doc-sync；evidence commit=pending | 2026-08-11 |
| `ANN-F` | P0 | IN_PROGRESS | ann_core_heritage | `coreaction.rs::{newparam_push_unique,seed_output_trials,derive_func_output_map}` + `heritage.rs::guard_calls_range_with_space` | `src/{coreaction,heritage}.rs`, `docs/api/{coreaction,heritage}.md` | 只做真实映射/GLUE 分类，不改行为；指定文件 annotation+refs+doc-sync；evidence commit=pending | 2026-08-11 |
| `ANN-G` | P0 | IN_PROGRESS | ann_pcode_grammar | `pcodeparse.rs` 22 个 lexer/decoder helper + `grammar.rs::new_impl` | `src/{pcodeparse,grammar}.rs`, `docs/api/{pcodeparse,grammar}.md` | 精确集合=`find_fn_violations`@fcc5142；验收指定文件 annotation+refs+doc-sync；evidence commit=pending | 2026-08-11 |
| `ANN-H` | P0 | IN_PROGRESS | ann_print_opbehavior | `printc.rs` 6 个 + `opbehavior.rs` 36 个 output/behavior helper | `src/{printc,opbehavior}.rs`, `docs/api/{printc,opbehavior}.md` | 精确集合=`find_fn_violations`@fcc5142；仅注释不触发输出行为变更；指定文件门禁；evidence commit=pending | 2026-08-11 |
| `ANN-I` | P0 | IN_PROGRESS | ann_variable_dynamic_union | `variable.rs` 14 个 + `dynamic.rs::default` + `unionresolve.rs` 6 个 helper | `src/{variable,dynamic,unionresolve}.rs`, `docs/api/{variable,dynamic,unionresolve}.md` | 精确集合=`find_fn_violations`@fcc5142；验收指定文件门禁；evidence commit=pending | 2026-08-11 |
| `ANN-J` | P0 | IN_PROGRESS | ann_translate_types | `translate.rs` 6 + `constseq.rs` 5 + `typefactory.rs::insert` + `datatype.rs` 18 个 helper | `src/{translate,constseq}.rs`, `src/type_system/{typefactory,datatype}.rs`, paired `docs/api/*` | 精确集合=`find_fn_violations`@fcc5142；不得把 `TypeFactory::insert` 简化覆盖标成 MATCH；验收指定文件门禁；evidence commit=pending | 2026-08-11 |
...
| `GATE-REF-RULEACTION` | P0 | IN_PROGRESS | ref_ruleaction | `ruleaction.rs` 1 个 `RuleTransformCpool::applyOp` ref | `src/ruleaction.rs`, `docs/api/ruleaction.md` | 锁定 12.0.4 完整函数体；只改引用则行为无变化；strict refs + differential 回归；evidence commit=pending | 2026-08-11 |

...
| `FUNC-GLOBRANGE-HANG-0001` | P0 | READY | unassigned | `glob_range`(0x4d60) 挂起（最后 trace：erase_descend 循环） | `src/varnode.rs`, `docs/api/varnode.md`, timeout isolation 日志 | E2E（2026-08-15）：worker >10s 超时、直跑 120s exit=124，最后 trace 为 erase_descend 循环；08-14 基线工件中成功。须先复现并拿到循环内 mutating 状态（descend 列表是否被并发缩短导致迭代器失效/死循环），对照 Ghidra `Varnode::eraseDescend`/descend list 语义修复；与 `VARNODE-BANK-KEY-LIVE-0001` 同域（varnode.rs），租约串行 | 2026-08-15 |

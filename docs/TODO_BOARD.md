...
| `HERITAGE-DRIVER-0001` | P0 | IN_PROGRESS | heritage_driver_audit | `Heritage::heritage` canonical driver ↔ `ActionHeritage` | 只读审计：`src/{heritage,coreaction,funcdata}.rs`、locked oracle；输出 `docs/alignment_audit/HERITAGE_DRIVER_2026-08-13.md` | 本轮只读，不与 `PIPE-LIFECYCLE-0001` 争写；列出 canonical buildADT→joins→ranges→phi→rename→store/load guard→pass++ 的依赖 DAG、锁/别名/遍历/计数/排序语义及最小 fixtures；实现依赖本 wave 地基集成 | 2026-08-13 |
...
| `PARAM-BIND-0001` | P0 | IN_PROGRESS | param_bind_audit | locked `FuncProto` input storage → SSA Varnode → Symbol/PrintC 参数名 | 只读审计：`src/{debugproto,fspec,funcdata,coreaction,varmap,printc}.rs`、locked oracle；输出 `docs/alignment_audit/PARAM_BIND_2026-08-13.md` | 追踪 GetStr `RDI/RSI` storage、locked input 创建、heritage rename、symbol map、PrintC lookup；定位 `param_35/param_33` 与未声明 unique 的首个状态差异，给分层 fixture，不做文本替换 | 2026-08-13 |
| `PRINT-RESIDUAL-0001` | P1 | IN_PROGRESS | print_residual_audit | upstream 修复后 `PrintC` 未声明变量/类型/表达式残差分类 | 只读审计：`src/{printc,printlanguage,prettyprint,varmap}.rs`、locked oracle；输出 `docs/alignment_audit/PRINT_RESIDUAL_2026-08-13.md` | 以当前 GetStr/CURL gcc 21/23 失败为基线，区分 upstream IR/parent/type/symbol 缺口与真正 emitter mismatch；先建立归因矩阵，禁止打印层掩盖 SSA/Action 错误 | 2026-08-13 |

...
| `FLOW-TARGET-BOUND-0001` | P0 | IN_PROGRESS | block_entry_wave | `FlowInfo::{setFallthruBound,fallthru}` visited join `STARTBASIC` | `src/flow.rs`, `docs/api/flow.md`, `tests/oracle/flow_target_boundary_1204.{cc,rs,metadata.json}`, `tools/run_flow_target_boundary_oracle.sh`, `docs/TODO_BOARD.md` | `BLOCK-ENTRY-0001` 首轮独立复核 REJECT：窄 ret/self-loop probe MATCH，但 GetStr CALL@0x36e4 的 fallthrough edge 1→2 从1变0；锁定 `flow.cc:489-513,545-580` 在已访问target/精确bound命中时标 STARTBASIC，当前只pop。修复后必须同时重跑窄entry fixture与 GetStr完整block/edge/reverse-slot identity/order，禁止保留heuristic provisional edge | 2026-08-13 |

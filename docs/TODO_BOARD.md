...
| `SUBFLOW-OUTVN-UNWRAP-0001` | P0 | READY | unassigned | `SubvariableFlow` create_link 路径 `outvn.unwrap()` 无守卫 | `src/subflow.rs`, `docs/api/subflow.md`, `tests/oracle/subflow_outvn_1204.{cc,rs,metadata.json}`, `tools/run_subflow_outvn_oracle.sh` | E2E（2026-08-15，ee29a32 后工作树）新发现：`my_get_token`(0x3720) worker panic 于 `src/subflow.rs:1737:73` `Option::unwrap()` on None——COPY/MULTIEQUAL/INT_{NEGATE,XOR,OR,AND} 分支的 `create_link` 调用在 `outvn` 为 None 时无守卫。须对照锁定 oracle `subflow.cc` 对应 createLink 调用点确认该 op 类 outvn 为 None 是否合法（Ghidra 侧 getOut 引用语义），按 Ghidra 行为修复而非简单加 unwrap_or；fixture 覆盖每分支 outvn None/Some 两种前置 | 2026-08-15 |

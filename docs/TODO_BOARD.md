...
| `BLKACT-FINALSTRUCT-COUNT-0001` | P1 | READY | unassigned | `ActionFinalStructure::apply` 硬编码 `changed += 1` | `src/blockaction.rs`, `docs/api/blockaction.md`, action_merge_order fixture 复跑 | PIPE-MERGETYPE wave 发现：锁定 blockaction.cc:2186 apply 返回 0 不增 count，Rust 硬编码 bump 导致 action_merge_order_1204 唯一残差行（count=1/apply=1/res=1 vs oracle 全 0）；删掉后该 fixture 无需改动即转 exit 0 MATCH（metadata overall 同步 MISMATCH→MATCH） | 2026-08-15 |

...
---
| `FUNCDATA-LINKSYMBOL-TYPED-0001` | P0 | READY | unassigned | `Funcdata::linkSymbol` 忠实化：SSA 寄存器临时也建带 HighVariable 类型的 ScopeLocal Symbol | `src/{funcdata,coreaction,varmap,merge}.rs`, paired docs, fixture | TYPED-DECL-GAP-0001 诊断（2026-08-15）：Ghidra funcdata_varnode.cc:1156-1184 在无重叠且非 persist 时 `addSymbol("", high->getType(), addr, usepoint)`（golden 的 bVar1/pcVar5 全部由此）；Rugra funcdata.rs:734-763 替换为往 HashMap 塞无类型 `local_XX` 自创名（:758）——`local_XX` 名直接来源；coreaction.rs:4453 还把无名 HighVariable 提前 continue（Ghidra hasName 语义=可命名）。依赖 SCOPE-CAT0/VARMAP-NAMING（DONE）+ COREACTION-SCOPE-CALLER（持久 scope）。验收=probe glob_word 后声明块出现 bVar/pcVar 族 | 2026-08-15 |

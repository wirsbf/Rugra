...
| `DEADCODE-SELFLOOP-0001` | P0 | IN_PROGRESS | remaining_timeout_small | `ActionDeadCode::{apply,pushConsumed}` 自环 `MULTIEQUAL` 引用/锁与 consumed 传播 | `src/coreaction.rs`, `docs/api/coreaction.md`, `tests/oracle/action_deadcode_selfloop_1204.{cc,rs,metadata.json}`, `tools/run_action_deadcode_selfloop_oracle.sh`, `docs/TODO_BOARD.md` | locked `coreaction.cc:3556-3568,3962-4010`；当前 Rust `apply` 持 output `RwLockReadGuard` 时对同一 self-loop VN 进入`push_consumed`写锁，令`parseconfig.constprop.0`确定性死锁。须按Ghidra裸指针状态序解除借用后再突变，不能跳过self-loop；fixture完整对拍合法phi自环的flags/consumed/return/count、op/VN/def-use与遍历顺序，修后单函数与完整curl复跑 | 2026-08-14 |

...
| `CSPEC-PARAMMODEL-0001` | P0 | IN_PROGRESS | cspec_parammodel_wave | `ParamListStandard` `<input>/<pentry>` decode、参数RangeList与Architecture默认`ProtoModelFull` | `src/{fspec,arch}.rs`, `docs/api/{fspec,arch}.md`, `tests/oracle/cspec_param_model_1204.{cc,rs,metadata.json}`, `tools/run_cspec_param_model_oracle.sh`, `docs/TODO_BOARD.md` | `next_url`实证`FuncProto.model=None`且当前5个负栈local未被x86-64 gcc参数范围`[8,507]`过滤；先按locked `fspec.cc`/`architecture.cc`完整解析真实cspec并保留共享model identity，禁止硬编码SysV或只注入范围常量。fixture需固定cspec/space index/default model/input pentry顺序、RangeList exact/in-range/boundary/flipped-stack与decode异常；依赖`PROTO-EFFECT-MODEL-0001`已满足 | 2026-08-14 |
| `FUNCPROTO-MODEL-BIND-0001` | P0 | BLOCKED | unassigned | `Funcdata`/DWARF prototype绑定Architecture resolved default model | `src/{funcdata,debugproto,arch}.rs`, `docs/api/{funcdata,debugproto,arch}.md`, `tests/oracle/funcproto_model_bind_1204.{cc,rs,metadata.json}`, `tools/run_funcproto_model_bind_oracle.sh`, `docs/TODO_BOARD.md` | 依赖`CSPEC-PARAMMODEL-0001`且当前`src/funcdata.rs`由`DEADCODE-SELFLOOP-0001`独占；Funcdata构造和driver必须先绑定同一Architecture/default model Arc，DWARF overlay保留该model再锁，不得clone fresh `model=None`。fixture观察model身份、lock/extrapop/effects/param range及失败回滚 | 2026-08-14 |
| `SCOPELOCAL-STATE-0001` | P0 | BLOCKED | scope_local_state_audit | persistent `ScopeLocal/ScopeInternal` range、category、query/addSymbol状态 | `src/{database,varmap,coreaction}.rs`, `docs/api/{database,varmap,coreaction}.md`, `tests/oracle/scope_local_state_1204.{cc,rs,metadata.json}`, `tools/run_scope_local_state_oracle.sh`, `docs/TODO_BOARD.md` | 依赖`FUNCPROTO-MODEL-BIND-0001`与`SCOPE-CAT0-0001`；Action必须复用Funcdata持久ScopeLocal，禁止每pass新建并clear symbols。fixture覆盖category-0 locked representative、queryProperties/usepoint、range identity、addSymbol冲突/警告与重复Action状态；PrintC阶段不得兜底重构 | 2026-08-14 |
...
| `PARSECONFIG-RULE-PAIRS-0001` | P0 | AUDITING | parseconfig_rule_panic | parseconfig越过DeadCode后`ruleaction.rs`空pair panic | 只读诊断先锁定Rule/输入状态与Ghidra调用闭包；实现write-set待根因确认 | `DEADCODE-SELFLOOP-0001`候选使parseconfig不再自锁，随后在`src/ruleaction.rs:5085`出现`res==1`但`result.pairs`为空；必须用固定prestate确认具体Rule与上游状态，禁止在unwrap处加默认值。诊断需记录locked Ghidra函数、四类语义、最小fixture与依赖DAG后再认领源码 | 2026-08-14 |
...
| `DEADCODE-SELFLOOP-0001` | P0 | IN_PROGRESS | remaining_timeout_small | `ActionDeadCode::{apply,pushConsumed,propagateConsumed}` + Varnode consume状态机 + `Funcdata::opUnsetOutput`目标闭包 | `src/{coreaction,varnode,funcdata}.rs`, `docs/api/{coreaction,varnode,funcdata}.md`, `tests/oracle/action_deadcode_selfloop_1204.{cc,rs,metadata.json}`, `tools/run_action_deadcode_selfloop_oracle.sh`, `docs/TODO_BOARD.md` | locked `coreaction.cc:3556-3650,3925-4010`、`varnode.hh` consume accessors、`funcdata_op.cc:52-64`；恢复call/nonassignment/autolive seed、VAC/LIS/LIFO与完整opcode传播，不能跳过self-loop；opUnsetOutput须按op→NULL、bank makeFree、clearCover顺序闭合输出状态。fixture完整对拍普通assignment/self-loop/autolive的flags/consume/op/VN/def-use及unset output，nullable input/opDestroy等残差保持overall MISMATCH。实测glob_word/glob_set已从timeout降至<1s；parseconfig越过死锁后暴露独立Rule panic | 2026-08-14 |
| `MERGE-DATATYPE-SCALE-0001` | P0 | IN_PROGRESS | my_get_line_diag | `Merge::{mergeByDatatype,mergeLinear,mergeTestBasic,mergeTestSpeculative}` 精确候选/排序/cache闭包 | `src/merge.rs`, `docs/api/merge.md`, `tests/oracle/merge_datatype_scale_1204.{cc,rs,metadata.json}`, `tools/run_merge_datatype_scale_oracle.sh`, `tests/oracle/fixture_registry.json`, `docs/TODO_BOARD.md` | locked `merge.cc:102-292,359-401,1565-1575` + `merge.hh:72-177`；`my_get_line`当前post-fullloop形成39960 high，Rust漏free/Basic过滤、updateHigh+精确sort、Speculative/HighIntersect cache并反复clone/reaggregate growing head，单轮O(N²)超时。必须完整移植过滤、排序、first-candidate、双向cache/move及HighVariable突变，不得cap/提前退/近似；fixture覆盖过滤、type pointer identity、四级排序、spec guards、cover/shadow边界和1/2/32/1024规模，完整状态direct diff；随后`my_get_line@0x3840`<10s并对拍最终IR/C | 2026-08-14 |

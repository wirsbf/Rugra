...
| `PRINTC-FORMAT-0001` | P1 | IN_PROGRESS | printc_format_wave | `PrintC::docFunction` 头部/花括号/参数空格 + `EmitXml` 缩进纯格式层 ↔ `src/{printc,prettyprint}.rs` | `src/printc.rs`, `src/prettyprint.rs`, `docs/api/{printc,prettyprint}.md`, `tests/oracle/printc_format_1204.{cc,rs,metadata.json}`, `tools/run_printc_format_oracle.sh`, 本 TODO 行 | 依赖 `ORACLE-0002`（已满足，12.0.4 canonical golden 就位）；只动格式层（花括号换行、参数空格、缩进、空行），不动语义/命名/类型；验收=cover_rebuild fixture 双侧逐字节一致 + `compare_ghidra.py result/curl_cur.c tests/golden/ghidra_curl_1204.c --summary-only` skeleton 显著低于 4530 基线且 numbering/defects 不劣化 | 2026-08-15 |

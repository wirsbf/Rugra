...
| `MERGE-PERSISTENT-STATE-0001` | P1 | BLOCKED | unassigned | 独立 merge Action 的 cover/testCache 前提 | `src/merge.rs`, `docs/api/merge.md`, action_merge_order fixture 扩展 | 依赖 merge.rs 租约释放；PIPE-MERGETYPE wave 发现：Ghidra `data.getMerge()` 为 Funcdata 持久对象（testCache/covers 跨 Action 保持），Rugra 每 Action 新建 `Merge::new()`——重排后独立 merge Action（mergerequired/mergecopy/mergeadjacent）在空 live_set/covers 前提下运行；需等价持久 cover 前提（fixture 现以常量输入规避 divergent 路径，E2E 当前无损） | 2026-08-15 |
...
| `DWARF-TYPE-IMPORT-0001` | P1 | READY | unassigned | DWARF 全局变量类型导入（golden 的 `URLGlob *pUVar2` 来源） | `examples/curl_decompile.rs`, DWARF 前端相关 src, docs | TYPED-DECL-GAP-0001：Rugra 目前只导函数原型+仅 `::config` 全局（curl_decompile.rs:37），golden 需要 `glob_expand:URLGlob*` 等全局类型；验收=输出出现 `URLGlob *` | 2026-08-15 |
| `FULL-CORPUS-0001` | P1 | READY | unassigned | curl example 喂全 124 函数语料 | `examples/curl_decompile.rs`, docs | 12.0.4 golden 有 124 函数，Rugra driver 只处理 21-25 个子集——全量对齐前必须扩语料；注意超时隔离与逐函数 abort 可见性（a0c1634）配合 | 2026-08-15 |

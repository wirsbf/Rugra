...
| `LEDGER-0001` | P0 | IN_PROGRESS | root | 114 `.cc/.hh` 全函数 ↔ `src/**/*.rs` + 协议表/依赖 DAG | `tools/generate_function_ledger.py`, `docs/alignment_audit/{FUNCTION_LEDGER.json,FUNCTION_MAP.generated.md,PROTOCOL_TABLE.json,DEPENDENCY_DAG.json}`, `docs/TODO_BOARD.md` | ctags 基线：5691 `.cc` definitions + 101 prototypes；3803 `.hh` inline definitions + 6216 prototypes；稳定 ID、精确 annotation 映射、未映射状态、协议常量与模块依赖可重复生成；生成器 `--check` 零漂移 | 2026-08-12 |
| `PERF-BUILD-0001` | P0 | IN_PROGRESS | root | Rust/C++ 可复现快速构建 | `Cargo.toml`, `Cargo.lock`, `build.rs`, `tools/rugra_build.py`, `tools/README.md`, `docs/TODO_BOARD.md` | `cc` parallel feature；可选 sccache/ccache 自动发现且无工具时安全回退；`fast-release` profile；离线 cold/warm 构建记录命中与耗时；不得改变 release 产物语义 | 2026-08-12 |
| `PERF-GATE-0001` | P0 | READY | root | edit/commit/wave/nightly 四级门禁 | `tools/rugra_gate.py`, `tests/oracle/fixture_registry.json`, `tools/README.md`, `docs/VERIFICATION_GUIDE.md`, `docs/TODO_BOARD.md` | 依赖 `PERF-BUILD-0001`；每级命令、超时、输入 hash、exit 状态结构化记录；commit 级覆盖 changed fixture，wave/nightly 保留全量冷门禁 | 2026-08-12 |
| `IMPACT-0001` | P0 | READY | root | changed Rust function/path → oracle fixture/门禁 | `tools/select_fixtures.py`, `tests/oracle/fixture_registry.json`, `tools/README.md`, `docs/VERIFICATION_GUIDE.md`, `docs/TODO_BOARD.md` | 依赖 `LEDGER-0001`；基于 git diff function span、显式 path/function/module selector；无法证明影响域时 fail-closed 选择全量；自测覆盖 rename/add/delete/顶层编辑 | 2026-08-12 |
| `ORACLE-CACHE-0001` | P0 | READY | root | 内容寻址 oracle 产物与阶段快照 | `tools/oracle_cache.py`, `tools/README.md`, `docs/VERIFICATION_GUIDE.md`, `docs/TODO_BOARD.md` | key 覆盖 oracle commit、arch/cspec/options/input/tool/comparand hash；原子写、逐文件校验、只读 restore；默认 `.rugra-cache/` 不入库；命中不得跳过 provenance 校验 | 2026-08-12 |
| `DIFF-BISECT-0001` | P1 | READY | root | pipeline stage hash/首差异定位 | `tools/stage_diff.py`, `tools/README.md`, `docs/VERIFICATION_GUIDE.md`, `docs/TODO_BOARD.md` | 依赖 `ORACLE-CACHE-0001`；比较有序 stage manifest，报告首个状态/hash/schema 差异和缺失阶段；不规范化顺序/别名/CFG；自测零差异/内容差异/重排/缺失 | 2026-08-12 |
| `REDUCE-0001` | P1 | READY | root | oracle mismatch 输入 delta reduction | `tools/reduce_fixture.py`, `tools/README.md`, `docs/VERIFICATION_GUIDE.md`, `docs/TODO_BOARD.md` | JSON list/object-field/byte-hex ddmin；predicate 命令无 shell 注入、超时/重试/结果缓存；输出保留原输入指纹和最小化 trace；自测确定性与 1-minimal | 2026-08-12 |

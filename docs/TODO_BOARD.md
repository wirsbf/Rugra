...
| `SLEIGH-0002A` | P0 | IN_PROGRESS | root | 锁定 x86-64 spec 资产与可重现生成 | `tools/build_locked_x86_64_sla.sh`, `sleigh_specs/{x86-64.sla,x86-64-gcc.cspec,x86-64.spec-metadata.json}`, `docs/TODO_BOARD.md` | 从 `e40ed130` Git 对象提取完整 language tree；双构建 byte-identical SHA `d5adc314…`；pspec/cspec hash 固定；example smoke 通过但状态不升级 | 2026-08-12 |

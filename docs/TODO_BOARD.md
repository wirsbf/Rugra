...
| `PARSECONFIG-RULE-PAIRS-0001` | P0 | IN_PROGRESS | parseconfig_rule_panic | `functionalEqualityLevel` raw out-buffer/positive-prefix contract | `src/expression.rs`, `docs/api/expression.md`, `tests/oracle/functional_equality_level_1204.{cc,rs,metadata.json}`, `tools/run_functional_equality_level_oracle.sh`, `tests/oracle/fixture_registry.json`, `docs/TODO_BOARD.md` | locked `expression.cc:404-512`；parseconfig真实prestate为MULTIEQUAL两侧不同unary INT_ZEXT，Rust命中`num=1,testval=1`却返回`code=1,pairs=[]`，而Ghidra先写`res1[0]/res2[0]`再返回1。须同时修四个positive-code空pair分支，并以预填sentinel的两槽raw buffers覆盖code0/-1残留写入、原pair优先与commutative交换；`RulePushMulti`索引合法，禁止加guard。修后重跑parseconfig调用闭包，Rule自身bb/CSE残差仍独立MISMATCH | 2026-08-14 |

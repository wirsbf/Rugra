...
| `ANN-L` | P0 | IN_PROGRESS | isolated_candidate_b | expanded scanner: `ffi.rs` 10 个 extern ABI 函数 | `src/ffi.rs`, `docs/api/ffi.md` | 依赖 `GATE-SCANNER-0001` 最终验收；FFI bridge 不冒充 Ghidra 算法映射，零行为改动 | 2026-08-12 |
...
| `ANN-N` | P0 | IN_PROGRESS | small_foundation_1204 | expanded scanner: address 2 + marshal 1 + signature 2 + types 13 | `src/{address,marshal,signature,types}.rs`, paired `docs/api/{address,marshal,signature,types}.md` | 依赖 `GATE-SCANNER-0001` 最终验收；真实 inline/constructor 与 Rust glue 分别分类，零行为改动 | 2026-08-12 |

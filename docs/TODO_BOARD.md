...
| `COVER-REBUILD-SELFLOCK-0001` | P0 | REVIEW | cover_rebuild_wave | `Varnode::updateCover` → `Cover::rebuild/addRefPoint` root identity与worklist闭包 | `src/{cover,varnode,merge}.rs`, `docs/api/{cover,varnode,merge}.md`, `tests/oracle/cover_rebuild_1204.{cc,rs,metadata.json}`, `tools/run_cover_rebuild_oracle.sh`, `tests/oracle/fixture_registry.json`（root串行登记）, `docs/TODO_BOARD.md` | evidence=pending commit；authoritative runner exit0：`MATCH covered_projection=8/8 overall=PARTIAL_MATCH stdout_sha256=00dba3d1…`（pinned base=235b91b+3文件overlay，完整重建锁定Ghidra）。修复：`update_cover_locked`持root写锁窗口内快照def/descend/implied；`add_ref_point_full`用`Arc::ptr_eq`在op锁窗口快照MULTIEQUAL匹配槽后释放块锁再递归；descendant op输出=root时用快照implied避免写锁重入；input sentinel根因修正为uindex域0（`getUIndex(2)=0`，add_def_point_full与contain_varnode_def_at同步）。8 case含slot2 self-ref/双槽读/implied链/setAll填充/no-cover-object；residual=MULTIEQUAL-tip旧stop判别、INDIRECT目标order（metadata coverage表）；E2E与cross-review由独立agent进行中，通过后本行转DONE并释放`src/varnode.rs`租约给`VARNODE-BANK-KEY-LIVE-0001` | 2026-08-15 |

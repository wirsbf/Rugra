...
| `ANN-K` | P0 | DONE | print_pipeline_1204 | expanded scanner: `opbehavior.rs` 42 个 const constructors | `src/opbehavior.rs`, `docs/api/opbehavior.md` | evidence commit=this commit；37 个锁定 inline constructor + 5 个 Translate/macro GLUE；纯 provenance；expanded scoped scan/refs/doc/check 通过 | 2026-08-12 |
...
| `ANN-P` | P0 | IN_PROGRESS | transform_subflow_1204 | expanded scanner: `unify.rs` 10 个单行 impl functions | `src/unify.rs`, `docs/api/unify.md` | 依赖 `GATE-SCANNER-0001` 最终验收；scanner 行内 item 覆盖，逐函数 provenance，零行为改动 | 2026-08-12 |

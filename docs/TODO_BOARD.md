...
| `FUNC-TYPEPROP-SETTLE-0001` | P0 | READY | unassigned | `match_url`(0x5220)/`__libc_csu_init`(0x5400) 类型传播不收敛 | `src/funcdata.rs`, `docs/api/funcdata.md`, timeout isolation 日志 | E2E（2026-08-15）：两函数 worker >10s 超时，直跑 120s 同样 exit=124，最后 trace 均为 `Type propagation algorithm not settling`；二者在 08-14 基线工件中成功——疑管线走得更远后新暴露或回归。须对照锁定 oracle `funcdata.cc` 类型传播 settle 条件（迭代上限/不动点判据），先诊断收敛判据与 Ghidra 差异再修，禁止调大上限掩盖 | 2026-08-15 |

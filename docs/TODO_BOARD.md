...
| `VARMAP-GATHEROFFSET-0001` | P0 | IN_PROGRESS | varmap_mask_wave | `AliasChecker::gatherOffset` 8-byte `calc_mask` boundary | `src/varmap.rs`, `docs/api/varmap.md`, `tests/oracle/varmap_gather_offset_1204.{cc,rs,metadata.json}`, `tools/run_varmap_gather_offset_oracle.sh`, `docs/TODO_BOARD.md` | `GetStr` debug stage触发 `(1u64 << 64)` panic；锁定 `varmap.cc:817-855` 返回 `retval & calc_mask(vn->getSize())`，8-byte必须全1 mask；需同输入 offset/alias/state fixture + visible-output differential + 独立核心复核，修复前 snapshot固定release且该分支=`UNTESTED` | 2026-08-13 |
...
| `RULE-COLLECTTERMS-0001` | P0 | IN_PROGRESS | rule_collectterms_wave | `RuleCollectTerms::applyOp` coefficient addition modulo storage width | `src/ruleaction.rs`, `docs/api/ruleaction.md`, `tests/oracle/rule_collect_terms_1204.{cc,rs,metadata.json}`, `tools/run_rule_collect_terms_oracle.sh`, `docs/TODO_BOARD.md` | `GetStr` debug Action触发 Rust unsigned-add panic；锁定 `ruleaction.cc:107-178` 的 `uintb newcoef = (coef1 + coef2) & mask` 按无符号机器数回绕；需溢出/非溢出完整 IR mutation fixture、curl differential与独立主管线 Rule 复核；未批准前不合入 wrapping 修复 | 2026-08-13 |

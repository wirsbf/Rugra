...
| `UPSTREAM-OUTVN-DEADWIRE-0001` | P0 | IN_PROGRESS/AUDITED | deadwire_diag | alive 无输出 op 滞留 descend 列表 + `free varnode gets multiple descendants` WARN（E2E 340 条）归因 | `src/{varnode,funcdata}.rs`, paired docs, E2E 复现 | SUBFLOW wave 移交（2026-08-15）：E2E my_get_token 中 MULTIEQUAL@0x3828(t564)/COPY@0x3759(t281) alive（dead=false、parent_alive=true）但 output=None 且仍在 descend 列表，伴随 `[VN] WARN: erase_descend not in descend list` 风暴——Ghidra 不变量链（funcdata_op.cc:52-66/82-84/203-217：opDestroy 逐槽 opUnsetInput→eraseDescend；opUnsetOutput 调用方补输出或销毁）下该状态不可达。修复后 subflow 的 None-abort 应变为不可达。与 `FUNC-GLOBRANGE-HANG-0001`（死 Weak descend 条目）同域，排查时合并审计 op_destroy/mark_dead/varnode-descend 记账 | 2026-08-15 |
...
| `MERGE-PREEXISTING-GATES-0001` | P2 | IN_PROGRESS | merge_gates_wave | 复核确认的 merge 家族预存在门/方向缺口（不在 MERGE-PERSISTENT diff 内） | `src/merge.rs`, `docs/api/merge.md`, `tests/oracle/merge_gates_1204.{cc,rs,metadata.json}`, `tools/run_merge_gates_oracle.sh`, TODO 行 | 第三轮复核预存在观察（2026-08-16 登记）：①mergeIndirect 缺 isAddrForce 门/输入方存活语义（merge.cc:846-887）+ allocateCopyTrim 兜底；②buildDominantCopy 用带 cache 的 merge 而 oracle :1236 传 null cache；③merge_multi_entry 缺 :937 mergeTestRequired 门（setMergeProblems/setUnmerged 计数路径）；④compare_just_loc 缺 space 维（variable.cc:439-443 为 Addr 全序）；⑤merge_highs (Some,Some)→false vs oracle throw（Ghidra 侧不可达，保守降级）。验收：runner exit0 + merge:: 4/4 基线 + merge_persistent_1204 17/17 复跑 + E2E 124/124 | 2026-08-16 |

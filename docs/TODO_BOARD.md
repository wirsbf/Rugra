...
| `MERGE-PERSISTENT-STATE-0001` | P1 | PARTIAL_MATCH | merge_persistent_wave | 独立 merge Action 的 cover/testCache 前提 | `src/merge.rs`, `src/funcdata.rs`（仅持久 Merge 挂载点 4 hunks）, `docs/api/{merge,funcdata}.md`, `tests/oracle/merge_persistent_1204.{cc,rs,metadata.json}`, `tools/run_merge_persistent_oracle.sh`, docs/TODO_BOARD.md 该行 | evidence=`pending root commit`；Ghidra 生命周期映射：`Merge covermerge`=Funcdata 按值成员（funcdata.hh:96/cc:39）、`getMerge()` 共享（funcdata.hh:440）、仅 `Funcdata::clear()` 清（cc:108→merge.cc:1580-1587）；Rugra 落地：`Funcdata::merge_state: MergePersistentState`（testCache/copyTrims/live_set）+ merge.rs attach/detach 接线全部 Action 级入口（attached 守卫防嵌套往返）+ `set_high_level` 补 assignHigh calcCover 前提（funcdata_varnode.cc:52-53，此前缺失=独立 merge Action 空前提根因）+ `merge_speculative` 重写为忠实 `Merge::merge`（merge.cc:1565-1575 testCache/moveIntersectTests/updateCover，替换 aggregate-cover 空前提）。权威 runner（pinned base=ec03e2f79136+merge.rs overlay+funcdata.rs 四 hunk 确定性重建，锁定 oracle 全量重建）exit0：**8 行投影 6 行逐字节一致**（disjoint_copy 全 4 阶段 MATCH+ladder assignhigh/mergerequired MATCH），2 行 MISMATCH=`VARNODE-COPYSHADOW-ARC-0001`（既有 bug 被揭出钉死）。merge:: 4/4；全 lib 失败=pristine 基线集减一（既有 merge_multi_entry 反被修复）；E2E 124/124 全绿 0 timeout/panic，12.0.4 差分 defects=0、skeleton 4388→4181、numbering 7→26（copy_shadow 残差可见代价，修复后应回收敛，见报告 Differential）。残差：copyShadow 传递族（→`VARNODE-COPYSHADOW-ARC-0001`）、merge_test_with_list 粗测（marker trim 分叉）、copyTrims/protoPartial/testCache 计数不可观测（Ghidra 默认 private）、clear 生命周期 oracle 级 UNTESTED | 2026-08-15 |
...
| `MERGE-HIGHCOVER-UNION-0001` | P1 | READY | unassigned | merge 合并后 high cover 未按 oracle 并集重建 | `src/merge.rs`, `docs/api/merge.md`, merge_persistent_1204 fixture 扩展 | COPYSHADOW wave 发现（2026-08-16）：mergecopy 把 Q1/Q2 并入 P 的 high 后，cover 未按 merge.cc:1571-1572 `high1->merge(...); high1->updateCover();`（+testCache purge）并集重建——Rugra 相交判定仍用 P 单实例 cover [b..op4]，TB 的 def@op5 实为 Q1 cover [op2..op6] 内点应拒而未拒；gdb+双侧 diag trace 定位。修复后 merge_persistent_1204 应 8/8（当前 7/8，mergeadjacent 行残差），需 root 重 pin（base 前移+expected sha 重算+末行 summary 同步）。依赖 merge.rs 租约释放（MERGE-PERSISTENT 复核进行中） | 2026-08-16 |

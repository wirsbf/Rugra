...
| `CSPEC-TEXT-INGEST-0001` | P0 | READY | unassigned | production `.cspec` bytes → DOM/TreeDecoder → `Architecture::parseCompilerConfig` | `src/marshal.rs`, `docs/api/marshal.md`, `src/arch.rs`, `docs/api/arch.md`, `tests/oracle/cspec_text_ingest_1204.{cc,rs,metadata.json}`, `tools/run_cspec_text_ingest_oracle.sh`, `tests/oracle/fixture_registry.json`, `docs/TODO_BOARD.md` | `CSPEC-PARAMMODEL-0001`依赖已满足并释放`arch.rs`。必须从锁定真实cspec bytes按文档序解析/dispatch stackpointer、default/prototype/eval并在全部成功后才发布Architecture；worker携带immutable bytes+SHA与language/compiler-spec identity，禁止按cwd临时重读或硬编码ABI。错误矩阵覆盖malformed XML、unknown register、duplicate/multiple/no default、invalid pentry及未发布回滚；`<modelrule>`未实现须保持MISMATCH | 2026-08-14 |

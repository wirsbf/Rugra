...
| `SLEIGH-0002` | P0 | IN_PROGRESS | root | raw SLEIGH 资产/ABI/context/lifecycle 闭包 | 由 `SLEIGH-0002A`…`D` 精确租约组成 | 依赖链 `2A → 2B → 2C → 2D → SLEIGH-FLOW-0001`；禁止以单次 `load_pspec` 冒充修复；完整 raw fixture 前保持 MISMATCH/NO_ORACLE | 2026-08-12 |
| `SLEIGH-0002A` | P0 | DONE | root | 锁定 x86-64 spec 资产与可重现生成 | `.gitattributes`, `tools/build_locked_x86_64_sla.sh`, `sleigh_specs/{x86-64.sla,x86-64-gcc.cspec,x86-64.spec-metadata.json}`, `docs/TODO_BOARD.md` | evidence commit=this commit；从 `e40ed130` Git 对象临时提取 cpp+language tree；双编译 SHA `d5adc314…`/487659；pspec/cspec/ldefs hash 固定；example 3-byte/COPY；Cross-Review APPROVE；仅资产 provenance，仍 NO_ORACLE | 2026-08-12 |
| `SLEIGH-0002B` | P0 | BLOCKED | unassigned | 动态无截断 C ABI + owned image + typed result/error | `sleigh_shim/rugra_sleigh.cpp`, `src/sleigh_ffi.rs`, paired API/example/oracle runner | 依赖 `SLEIGH-0002A`；一次 oneInstruction 原子返回 step+全序 ops+全部 inputs+typed error；CPUID 78 ops、零-op、BadData/DataUnavail 同输入差分 | 2026-08-12 |
| `SLEIGH-0002C` | P0 | BLOCKED | unassigned | Ghidra XmlDecode 完整 pspec context/tracked ranges | `sleigh_shim/rugra_sleigh.cpp`, `src/sleigh_ffi.rs`, paired API/oracle | 依赖 `SLEIGH-0002B`；`.sla initialize` 后调用 `ContextInternal::decodeFromSpec`；保留范围/mask/order/tracked DF/异常，禁止字符串扫 `<set>` | 2026-08-12 |

...
> **2026-09-26 后记（root 集成期转入,DUALSLEIGH 车道）**: 层 1 已由 wt/dualsleigh 修复
> （PERF-DUAL-SLEIGH-INIT-0001）——hermetic 子进程 `x86-64.sla` 反序列化 2→1
> （`SleighLifter::from_ctx` 领养,sleigh_arch.cc:174 buildTranslator 复用形态）,
> sqlite3 小函数交错 A/B **−25.3%**（~99ms/函数,20/20 全快,地板 221→132ms）,
> canon curl user −16%,canon 双语料字节恒等。固定开销残差（单载后地板 ~132ms vs
> oracle 85–180ms）剩余项 = 进程 spawn + cspec/pspec 解析与 TypeFactory 等 arch 构建
> 面,归 PAREVAL-ARCH-BUILD-COST-0001 续作。

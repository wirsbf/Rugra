...
pub mod block_flags {
    ...
    /// Ghidra f_label_bumpup = 0x1000 (block.hh:99). Label on this block needs
    /// to be bumped up to be visible outside the block's print body. Set by
    /// BlockDoWhile/BlockInfLoop::markLabelBumpUp (block.cc:3406, 3445) so the
    /// loop's exit label is emitted at the loop header, not the trailing goto.
    pub const LABEL_BUMPUP: u32 = 0x1000;
    /// Ghidra f_donothing_loop = 0x2000 (block.hh:100). Set when a BlockWhileDo
    /// or BlockDoWhile is generated by structuring for an unstructured goto
    /// whose body is empty (block.cc:3351, 3417). The loop prints no body.
    pub const DONOTHING_LOOP: u32 = 0x2000;
    /// Ghidra f_whiledo_overflow = 0x8000 (block.hh:102). Set by
    /// BlockWhileDo::setOverflowSyntax (block.cc:3531) when the C `while(cond)`
    /// syntax would not faithfully represent the loop's overflow side-effect,
    /// forcing the printer to fall back to an explicit `for(;;)` plus a break.
    pub const WHILEDO_OVERFLOW: u32 = 0x8000;
...
///
/// Stored on `BlockGoto::gototype` and `BlockIf::gototype` to distinguish an
/// unstructured `goto` from a `break` or `continue` produced by structuring
/// (`scope_break`, block.cc:2873-2880). Used by `PrintC` to emit the correct
/// keyword for unstructured exits.
pub mod goto_type {
    /// Ordinary unstructured `goto target;` (block.hh:89).
    pub const GOTO_GOTO: u32 = 1;
    /// Goto that lands on the enclosing loop's exit — printed as `break;`
    /// (block.hh:90). Set by `BlockGoto::scopeBreak` (block.cc:2873) when the
    /// goto target is the loop's exit block.
    pub const BREAK_GOTO: u32 = 2;
    /// Goto that lands on the enclosing loop's iterate header — printed as
    /// `continue;` (block.hh:91). Set by `BlockGoto::scopeBreak` (block.cc:2880).
>>>>>>> Stashed changes
...
=======
    /// Ghidra `BlockCopy::printHeader` (block.cc:2835-2840): emit
    /// `"Basic(copy) block <index>"`. Rugra returns the constructed header
    /// string; the printer concatenates it. Faithful to the C++ which streams
    /// the literal then defers to `FlowBlock::printHeader` for the index.
    // Ghidra: block.cc:2835 BlockCopy::printHeader
    pub fn print_header(&self) -> String {
        format!("Basic(copy) block {}", self.index)
    }

    /// Ghidra `BlockCopy::printTree` (block.cc:2842-2846): defer to the wrapped
    /// copy block. Rugra delegates to the original `BlockBasic`, which does not
    /// expose a tree printer, so we emit a single indented header line for the
    /// copy and let the printer walk the original's ops separately.
    // Ghidra: block.cc:2842 BlockCopy::printTree
    pub fn print_tree(&self, level: i32) -> String {
        let indent: String = "  ".repeat(level as usize);
        format!("{}{}\n", indent, self.print_header())
    }

    /// Ghidra `BlockCopy::encodeHeader` (block.cc:2848-2854): write the base
    /// header then an `altindex` attribute holding the wrapped block's index.
    /// Rugra returns `(base_index, alt_index)` so the encoder layer can
    /// serialize both fields. Faithful to the C++ which calls
    /// `FlowBlock::encodeHeader` then `writeSignedInteger(ATTRIB_ALTINDEX, ...)`.
    // Ghidra: block.cc:2848 BlockCopy::encodeHeader
    pub fn encode_header(&self) -> (i32, i32) {
        // altindex = copy->getIndex() in Ghidra.
        let altindex = self.original.read().unwrap().index;
>>>>>>> Stashed changes
        (self.index, altindex)
    }
}
...
=======
    /// `GOTO_GOTO`; mutated by `scopeBreak` (block.cc:2873) to `BREAK_GOTO`
    /// when the goto lands on the enclosing loop's exit.
>>>>>>> Stashed changes
    pub goto_type: u32,
    pub incoming: Vec<BlockEdge>,
    pub outgoing: Vec<BlockEdge>,

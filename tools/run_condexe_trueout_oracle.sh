#!/usr/bin/env bash
# CONDEXE-TRUEOUT-0002 — locked Ghidra 12.0.4 oracle gate for the purely
# positional getTrueOut/getFalseOut semantics (block.hh:299-300) and their
# condexe consumers. Pin-base schema2, modeled on
# tools/run_transform_multiequal_insert_oracle.sh with these deltas: no BFD,
...
rust_fixture="$repo_root/tests/oracle/condexe_trueout_1204.rs"
...
cp "$blockaction_overlay" "$snapshot_root/src/blockaction.rs"
cp "$cpp_fixture" "$snapshot_root/tests/oracle/condexe_trueout_1204.cc"
cp "$rust_fixture" "$snapshot_root/tests/oracle/condexe_trueout_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/condexe_trueout_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_condexe_trueout_oracle.sh"
...
(
    repo_raw, metadata_raw, cpp_raw, rust_raw, condexe_raw, block_raw,
    double_raw, blockaction_raw, runner_sha, oracle_commit, oracle_tag,
    cpp_tree, makefile_blob, source_commit, source_tree, source_src_tree,
    condexe_blob, block_blob, double_blob, blockaction_blob,
...
require("fixture", metadata["fixture_id"], "CONDEXE-TRUEOUT-0002")
...
for label, actual, expected in (
    ...
    ("source block blob", source["base_block_blob"], block_blob),
    ("source double_precis blob", source["base_double_precis_blob"], double_blob),
    ("source blockaction blob", source["base_blockaction_blob"], blockaction_blob),
...
for key, path in (
    ...
    ("double_precis_overlay_sha256", pathlib.Path(double_raw)),
    ("blockaction_overlay_sha256", pathlib.Path(blockaction_raw)),
):
...
expected_matches = {
    "trueout_helper_positional", "findinit_init2a_true",
    "verify_camethruposta_slot", "zeropath_is_true",
}
expected_residuals = {
    "execute_and_flow_split", "heritage_gating", "pullback_storage",
    "error_channels", "orpredicate_applyop_end_to_end",
}
...
require("residual TODO", residual["todo_id"], "CONDEXE-TRUEOUT-RESIDUAL-0001")
...
set +e
"$oracle_tmp/condexe_trueout_1204_cpp" \
...
expected_order = [
    "helper:H1", "helper:H2", "helper_neg:H3", "helper_neg:H4",
    "findinit:F1", "findinit:F2", "findinit:F3", "findinit:F4",
    "findinit:F5", "findinit:F6", "findinit:F7",
    "verify:V1", "verify:V2", "verify:V3", "verify:V4",
    "zeropath:Z1", "zeropath:Z2", "zeropath:Z3", "zeropath:Z4",
    "zeropath:Z5", "zeropath:Z6", "zeropath:Z7",
]
...
printf 'condexe_trueout_1204: covered_projection=4/4 projection_status=MATCH overall_status=UNTESTED residual=CONDEXE-TRUEOUT-RESIDUAL-0001\n'

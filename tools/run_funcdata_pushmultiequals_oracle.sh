...
rugra_source_commit=de4309fd1aee876e698f18380193b8cf4d2e9dd1
rugra_source_tree=d66fc1d7b33bf15a17bc154863cc42bf8556f913
rugra_source_src_tree=1a1d55fe121d0696ecb865da684eeaa8ea7de14b
rugra_source_funcdata_blob=6e91051defe685be4df5c8bcbb8957b4e88a78f4
...
rugra_input_commit=de4309fd1aee876e698f18380193b8cf4d2e9dd1
...
(
    ...
    source_funcdata_blob, source_cargo_toml_blob, source_cargo_lock_blob,
...
require("fixture", metadata["fixture_id"], "FUNCDATA-PUSHMULTIEQUALS-0001")
...
if not isinstance(comparand, dict):
    raise SystemExit("comparand must be an object")
for key, path in (
    ("cpp_fixture_sha256", pathlib.Path(cpp_raw)),
    ("rust_fixture_sha256", pathlib.Path(rust_raw)),
    ("funcdata_overlay_sha256", pathlib.Path(overlay_raw)),
):
...
expected_matches = {"reader_beyond", "dead_edge_only", "addrtied_unique"}
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/funcdata_pushmultiequals_1204.cc" \
  ...
  "$bfd_library" -lz -o "$oracle_tmp/funcdata_pushmultiequals_1204_cpp"

# Persistent cargo target for the snapshot build, on /home rather than the
# /tmp tmpfs (32G, shared with concurrent agents' scratch: repeated quota
...
printf 'funcdata_pushmultiequals_1204: covered_projection=3/3 projection_status=MATCH overall_status=MATCH residual=none\n'

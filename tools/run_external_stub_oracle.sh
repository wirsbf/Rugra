...
runner="$repo_root/tools/run_external_stub_oracle.sh"
...
cpp_fixture="$repo_root/tests/oracle/external_stub_1204.cc"
rust_fixture="$repo_root/tests/oracle/external_stub_1204.rs"
space_rs="$repo_root/src/space.rs"
translate_rs="$repo_root/src/translate.rs"
space_doc="$repo_root/docs/api/space.md"
translate_doc="$repo_root/docs/api/translate.md"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-external-stub-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-external-stub-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/external_stub_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/external_stub_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/external_stub_1204.rs",
    pathlib.Path(space_raw): repo / "src/space.rs",
    pathlib.Path(translate_raw): repo / "src/translate.rs",
    pathlib.Path(space_doc_raw): repo / "docs/api/space.md",
    pathlib.Path(translate_doc_raw): repo / "docs/api/translate.md",
}
...
crate_hasher.update(b"rugra-external-stub-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/external_stub_1204.cc"),
    pathlib.Path("tests/oracle/external_stub_1204.rs"),
    pathlib.Path("tests/oracle/external_stub_1204.metadata.json"),
    pathlib.Path("docs/api/space.md"),
    pathlib.Path("docs/api/translate.md"),
    pathlib.Path("tools/run_external_stub_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "EXTERNAL-STUB-1204")
require(
    "stable function id",
...
observed_hashes = {
    ...
    "translate_rs_sha256": sha(crate_bytes["src/translate.rs"]),
    "space_doc_sha256": sha(special[special_paths[3].as_posix()]),
    "translate_doc_sha256": sha(special[special_paths[4].as_posix()]),
...
require(
    ...
    "sha256 of rugra-external-stub-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("decode_spaces_default_registration", "MATCH"),
    ("decode_space_overlay_marks_base", "MATCH"),
    ("decode_space_base_stack_contain", "MATCH"),
    ("decode_space_unknown_base", "MATCH"),
    ("decode_spaces_bad_default", "MATCH"),
    ("external_named_space_registration", "MATCH"),
    ("deadcodedelay_default_from_delay", "MATCH"),
    ("printc_halt_baddata_arm", "UNTESTED"),
    ("driver_stub_projection", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-external-stub-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/external_stub_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/external_stub_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/external_stub_1204.rs"
...
cpp_binary="$oracle_tmp/external_stub_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "external_stub_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/external_stub_1204_rust"
...
if len(lines) != 36:
    raise SystemExit(f"expected thirty-six fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=EXTERNAL-STUB-1204|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "decode_spaces_default_registration", "decode_space_overlay_marks_base",
    "decode_space_base_stack_contain", "decode_space_unknown_base",
    "decode_spaces_bad_default", "external_named_space_registration",
    "deadcodedelay_default_from_delay",
]
...
metadata = json.loads(
    (repo / "tests/oracle/external_stub_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_external_stub_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/external_stub_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/external_stub_1204.rs").read_bytes()),
    comparand["rust_fixture_sha256"],
)
for label, relative, key in (
    ("space implementation", "src/space.rs", "space_rs_sha256"),
    ("translate implementation", "src/translate.rs", "translate_rs_sha256"),
    ("space API document", "docs/api/space.md", "space_doc_sha256"),
    ("translate API document", "docs/api/translate.md", "translate_doc_sha256"),
):
    require(label, sha((repo / relative).read_bytes()), comparand[key])

overlay_files = {"src/space.rs", "src/translate.rs"}
...
hasher.update(b"rugra-external-stub-base-overlay-v1\0")
...
echo "external_stub_1204: MATCH covered_projection=7/7 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

...
# Authoritative oracle runner for VARIABLE-GETTYPE-LAZY-UPDATETYPE-0001:
# the lazy updateType() trigger inside HighVariable::getType (variable.hh:
# 174) and the const isTypeLock re-derivation (variable.hh:222), driven on
# construction-dirty, externally-dirtied (Varnode::copySymbol cc:496/501),
# isolated, and finalized (variable.cc:551-566 + cc:405-407 guard) paths.
#
# Comparand model: pinned Rugra base commit plus the live overlay files of
# this lease's write-set (src/variable.rs and its compile-required v_type
# accessor adaptations src/analysis/type_infer.rs, src/coreaction.rs,
# src/merge.rs), so concurrent WIP in other agents' domains never leaks into
# this comparand.
...
runner="$repo_root/tools/run_variable_lazytype_oracle.sh"
...
rugra_base_commit=25f8f83ab6f28a3818f3ac10bb7b2f4381446dfb
rugra_base_tree=12821f723bd26406201ad127de542f1950fa801e
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/variable_lazytype_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/variable_lazytype_1204.cc"
rust_fixture="$repo_root/tests/oracle/variable_lazytype_1204.rs"
variable_rs="$repo_root/src/variable.rs"
type_infer_rs="$repo_root/src/analysis/type_infer.rs"
coreaction_rs="$repo_root/src/coreaction.rs"
merge_rs="$repo_root/src/merge.rs"
variable_doc="$repo_root/docs/api/variable.md"
type_infer_doc="$repo_root/docs/api/analysis/type_infer.md"
coreaction_doc="$repo_root/docs/api/coreaction.md"
merge_doc="$repo_root/docs/api/merge.md"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$variable_rs" "$type_infer_rs" "$coreaction_rs" "$merge_rs" \
  "$variable_doc" "$type_infer_doc" "$coreaction_doc" "$merge_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-variable-lazytype-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-variable-lazytype-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S - \
  ...
  "$metadata" "$cpp_fixture" "$rust_fixture" "$variable_rs" \
  "$type_infer_rs" "$coreaction_rs" "$merge_rs" \
  "$variable_doc" "$type_infer_doc" "$coreaction_doc" "$merge_doc" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, variable_raw, type_infer_raw,
    coreaction_raw, merge_raw, variable_doc_raw, type_infer_doc_raw,
    coreaction_doc_raw, merge_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/variable_lazytype_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/variable_lazytype_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/variable_lazytype_1204.rs",
    pathlib.Path(variable_raw): repo / "src/variable.rs",
    pathlib.Path(type_infer_raw): repo / "src/analysis/type_infer.rs",
    pathlib.Path(coreaction_raw): repo / "src/coreaction.rs",
    pathlib.Path(merge_raw): repo / "src/merge.rs",
    pathlib.Path(variable_doc_raw): repo / "docs/api/variable.md",
    pathlib.Path(type_infer_doc_raw): repo / "docs/api/analysis/type_infer.md",
    pathlib.Path(coreaction_doc_raw): repo / "docs/api/coreaction.md",
    pathlib.Path(merge_doc_raw): repo / "docs/api/merge.md",
}
...
overlay_files = {
    ...
    pathlib.Path("src/analysis/type_infer.rs"),
...
crate_hasher.update(b"rugra-variable-lazytype-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/variable_lazytype_1204.cc"),
    pathlib.Path("tests/oracle/variable_lazytype_1204.rs"),
    pathlib.Path("tests/oracle/variable_lazytype_1204.metadata.json"),
    pathlib.Path("docs/api/variable.md"),
    pathlib.Path("docs/api/analysis/type_infer.md"),
    pathlib.Path("docs/api/coreaction.md"),
    pathlib.Path("docs/api/merge.md"),
    pathlib.Path("tools/run_variable_lazytype_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "VARIABLE-GETTYPE-LAZY-UPDATETYPE-0001")
require(
    "stable function id",
...
observed_hashes = {
    ...
    "type_infer_rs_sha256": sha(crate_bytes["src/analysis/type_infer.rs"]),
    "coreaction_rs_sha256": sha(crate_bytes["src/coreaction.rs"]),
    "merge_rs_sha256": sha(crate_bytes["src/merge.rs"]),
    "variable_doc_sha256": sha(special[special_paths[3].as_posix()]),
    "type_infer_doc_sha256": sha(special[special_paths[4].as_posix()]),
    "coreaction_doc_sha256": sha(special[special_paths[5].as_posix()]),
    "merge_doc_sha256": sha(special[special_paths[6].as_posix()]),
...
require(
    ...
    "sha256 of rugra-variable-lazytype-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("lazytype_ctor_dirty_rederive", "MATCH"),
    ("lazytype_external_settype_rederive", "MATCH"),
    ("lazytype_instance_isolation", "MATCH"),
    ("lazytype_finalized_guard", "MATCH"),
    ("lazytype_raw_bit_clear_by_gettype", "NO_ORACLE"),
    ("varnode_update_type_high_dirty_leg", "NO_ORACLE"),
):
...
residual_ids = metadata["residual_todo_ids"]
require(
    "residual todo ids",
    set(residual_ids),
    {
        "VARIABLE-GETTYPE-LAZY-UPDATETYPE-0001",
        "VARNODE-UPDATETYPE-HIGHDIRTY-0001",
        "VARNODE-SETSYMBOLENTRY-HIGHLEG-0001",
    },
)
mismatch_entries = [
    value for value in coverage.values() if value.split(":", 1)[0] == "MISMATCH"
]
require(
    "every MISMATCH entry cites a registered residual TODO",
    all(any(residual in value for residual in residual_ids) for value in mismatch_entries),
    True,
)
require("MISMATCH entries none", len(mismatch_entries), 0)
...
registry_hasher.update(b"rugra-variable-lazytype-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/variable_lazytype_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/variable_lazytype_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/variable_lazytype_1204.rs"
...
cpp_binary="$oracle_tmp/variable_lazytype_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "variable_lazytype_1204: GHIDRA_LOCKED_OUTPUT_OK covered_projection=MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/variable_lazytype_1204_rust"
...
if lines != metadata["expected_lines"]:
    raise SystemExit(f"fixture observations are not the exact pinned lines: {lines!r}")
PY
...
metadata = json.loads(
    (repo / "tests/oracle/variable_lazytype_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_variable_lazytype_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
for relative, key in (
    ("tests/oracle/variable_lazytype_1204.cc", "cpp_fixture_sha256"),
    ("tests/oracle/variable_lazytype_1204.rs", "rust_fixture_sha256"),
    ("src/variable.rs", "variable_rs_sha256"),
    ("src/analysis/type_infer.rs", "type_infer_rs_sha256"),
    ("src/coreaction.rs", "coreaction_rs_sha256"),
    ("src/merge.rs", "merge_rs_sha256"),
    ("docs/api/variable.md", "variable_doc_sha256"),
    ("docs/api/analysis/type_infer.md", "type_infer_doc_sha256"),
    ("docs/api/coreaction.md", "coreaction_doc_sha256"),
    ("docs/api/merge.md", "merge_doc_sha256"),
):
    require(relative, sha((repo / relative).read_bytes()), comparand[key])

overlay_files = {"src/variable.rs", "src/analysis/type_infer.rs", "src/coreaction.rs", "src/merge.rs"}
...
hasher.update(b"rugra-variable-lazytype-base-overlay-v1\0")
...
echo "variable_lazytype_1204: covered_projection=4/4 MATCH residual=VARIABLE-GETTYPE-LAZY-UPDATETYPE-0001,VARNODE-UPDATETYPE-HIGHDIRTY-0001,VARNODE-SETSYMBOLENTRY-HIGHLEG-0001 overall=UNTESTED stdout_sha256=$actual_stdout_sha"

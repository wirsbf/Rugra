...
rugra_base_commit=1fc8af3df85ddac9bc87ff4606a27672fd97f675
rugra_base_tree=b2effc9338f71d37d62ee55ec85cfa4bb36907ac
rugra_base_src_tree=8b32e893ffe956565db26a1f0a1e5d5ac1ccd446
rugra_base_action_blob=22c29595478b5c6a186ac7b4fb540cdcd723d3b9
rugra_base_ruleaction_blob=342e21169fd102cdae9f9f22f43a7055a01a800e
...
rust_fixture="$repo_root/tests/oracle/pool_purity_1204.rs"
...
for required in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$action_overlay" "$ruleaction_overlay" "$runner"; do
...
(
    repo_raw, metadata_raw, cpp_raw, rust_raw, action_raw, ruleaction_raw,
    runner_sha, oracle_commit, oracle_tag, cpp_tree, makefile_blob,
    base_commit, base_tree, base_src_tree, base_action_blob,
    base_ruleaction_blob, base_cargo_toml_blob, base_cargo_lock_blob,
    base_build_rs_blob,
) = sys.argv[1:]
...
require("fixture", metadata["fixture_id"], "PIPE-POOL-LOCAL-RULES-0001")
...
for label, actual, expected in (
    ...
    ("base ruleaction blob", source["base_ruleaction_blob"], base_ruleaction_blob),
...
for key, path in (
    ...
    ("ruleaction_overlay_sha256", pathlib.Path(ruleaction_raw)),
):
...
require("coverage pool projection", coverage["pool_rule_projection"]["status"], "MATCH")
require("coverage name projection", coverage["rule_name_projection"]["status"], "MATCH")
...
cp "$rust_fixture" "$snapshot_root/tests/oracle/pool_purity_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/pool_purity_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_pool_purity_oracle.sh"
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare -m64 \
  -I"$oracle_cpp" "$snapshot_root/tests/oracle/pool_purity_1204.cc" \
  ...
  -o "$oracle_tmp/pool_purity_1204_cpp"
...
set +e
"$oracle_tmp/pool_purity_1204_cpp" \
...
if len(lines) != 317:
    raise SystemExit(f"expected 317 fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=PIPE-POOL-LOCAL-RULES-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
if lines[1] != "root=all" or lines[159] != "root=decompile":
    raise SystemExit("root block markers mismatch")
pool_lines = [line for line in lines if line.startswith("pool|")]
rule_lines = [line for line in lines if line.startswith("rule|")]
if len(pool_lines) != 6 or len(rule_lines) != 308:
    raise SystemExit(
        f"projection size mismatch: pools={len(pool_lines)} rules={len(rule_lines)}"
    )
for pool_line, expected_count in (
    ("pool|path=universal:fullloop:mainloop:stackstall:oppool1|name=oppool1|count=134", 2),
    ("pool|path=universal:fullloop:mainloop:oppool2|name=oppool2|count=5", 2),
    ("pool|path=universal:cleanup|name=cleanup|count=15", 2),
):
    if sum(1 for line in pool_lines if line == pool_line) != expected_count:
        raise SystemExit(f"pool line missing or duplicated: {pool_line}")
all_block = lines[2:159]
decompile_block = lines[160:]
if all_block != decompile_block:
...
if len(trivial) != 2 or any(
    ...
    raise SystemExit(
        "trivialarith must be registered exactly once per root, at oppool1 "
...
printf 'pool_purity_1204: pool_rule_projection=MATCH overall=MATCH pools=3x2 rules=308 stdout_sha256=%s\n' \

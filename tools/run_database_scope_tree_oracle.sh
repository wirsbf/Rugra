...
# Immutable runner for DATABASE-SCOPE-TREE-FIXTURE-0001 (MIGW1-DATABASE-0005
# phase 2).  The Ghidra oracle, Rugra base commit, leased database.rs, both
...
metadata="$repo_root/tests/oracle/database_scope_tree_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/database_scope_tree_1204.cc"
rust_fixture="$repo_root/tests/oracle/database_scope_tree_1204.rs"
runner="$repo_root/tools/run_database_scope_tree_oracle.sh"
...
if metadata["fixture_id"] != "DATABASE-SCOPE-TREE-FIXTURE-0001":
...
if metadata["observation"]["overall_status"] != "MATCH":
    raise SystemExit("scope tree fixture must be MATCH")
...
oracle_tmp=$(mktemp -d /tmp/rugra-database-scope-tree-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-database-scope-tree-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare -Wno-exceptions -m64 \
  ...
  "$bfd_library" -lz \
  -o "$oracle_tmp/database_scope_tree_ghidra"
...
ghidra_status=0
"$oracle_tmp/database_scope_tree_ghidra" \
...
cp "$rust_fixture" "$rugra_snapshot/src/bin/database_scope_tree_1204.rs"
...
CARGO_TARGET_DIR="$oracle_tmp/cargo-target" \
  ...
  --bin database_scope_tree_1204
rugra_status=0
"$oracle_tmp/cargo-target/debug/database_scope_tree_1204" \
...
printf 'database_scope_tree_1204: covered_oracle=MATCH rugra=MATCH overall=MATCH\n'

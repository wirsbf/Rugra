...
runner="$repo_root/tools/run_merge_gates_oracle.sh"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-merge-gates-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-merge-gates-1204.??????) /usr/bin/rm -rf -- "$oracle_tmp" ;;
...
# The pinned base commit already carries the persistent Funcdata merge_state
# mount (7046998), so only the two leased sources are overlaid: the working
# tree's src/merge.rs (MERGE-PREEXISTING-GATES-0001 fixes) and src/variable.rs
# (the compare_just_loc space dimension).
/usr/bin/mkdir -p "$oracle_tmp/candidate"
for candidate in \
  tests/oracle/merge_gates_1204.metadata.json \
  tests/oracle/merge_gates_1204.cc \
  tests/oracle/merge_gates_1204.rs \
  src/merge.rs src/variable.rs Cargo.toml Cargo.lock build.rs; do
  /usr/bin/cp -- "$repo_root/$candidate" "$oracle_tmp/candidate/$(/usr/bin/basename "$candidate")"
done
metadata="$oracle_tmp/candidate/merge_gates_1204.metadata.json"
cpp_fixture="$oracle_tmp/candidate/merge_gates_1204.cc"
rust_fixture="$oracle_tmp/candidate/merge_gates_1204.rs"
merge_source="$oracle_tmp/candidate/merge.rs"
variable_source="$oracle_tmp/candidate/variable.rs"
...
# from the pinned base commit with only the two leased source candidates
# overlaid (src/merge.rs + src/variable.rs).
...
/usr/bin/cp -- "$variable_source" "$oracle_tmp/rugra/src/variable.rs"
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S - \
  "$metadata" "$cpp_fixture" "$rust_fixture" "$merge_source" "$variable_source" \
...
(
    metadata_name, cpp_name, rust_name, merge_name, variable_name, runner_name,
...
for gate in ("indirect_addrforce_gate", "dominant_copy_null_cache_merge",
    ...
    if metadata["coverage"][gate] != "MATCH":
        raise SystemExit(f"coverage gate must be MATCH: {gate}")
...
paths = {
    ...
    "variable_rs_sha256": pathlib.Path(variable_name),
...
/usr/bin/env -i HOME="$HOME" RUSTUP_HOME="$HOME/.rustup" \
  ...
  "$rust_fixture" -o "$oracle_tmp/merge_gates_rust"

/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/merge_gates_cpp" >"$oracle_tmp/ghidra.stdout" \
  2>"$oracle_tmp/ghidra.stderr"
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/merge_gates_rust" >"$oracle_tmp/rugra.stdout" \
  2>"$oracle_tmp/rugra.stderr"
test ! -s "$oracle_tmp/ghidra.stderr"
test ! -s "$oracle_tmp/rugra.stderr"
test "$(wc -l < "$oracle_tmp/ghidra.stdout")" -eq 14
test "$(wc -l < "$oracle_tmp/rugra.stdout")" -eq 14
...
print(f"covered_projection={match_lines}/14 lines byte-identical")
for g, r in zip(ghidra_lines, rugra_lines):
    if g != r:
        print(f"MISMATCH ghidra: {g}")
        print(f"MISMATCH rugra:  {r}")
if match_lines != 14:
    raise SystemExit("fixture regression: projection no longer 14/14")
PYVERDICT

cat "$oracle_tmp/ghidra.stdout"
printf 'merge_gates_1204: covered_projection=14/14 overall_status=MATCH\n'
printf 'gates=MATCH: mergeIndirect isAddrForce gate + input-surviving merges + allocateCopyTrim fallback (merge.cc:846-882); buildDominantCopy null-testCache speculative merge (:1236, pinned by mergeGroup class 1); mergeMultiEntry mergeTestRequired gate with setMergeProblems/setUnmerged + warningHeader (:936-961); compareJustLoc space-major total order (variable.cc:439-443)\n'
printf 'residuals=merge_highs (Some,Some) piece arm: NO_ORACLE (Ghidra-side unreachable through Merge::merge callers; debug_assert + conservative skip, variable.cc:699-711 mergeGroups path unported while group_partials is a no-op); newUnique assignHigh gap worked around in merge.rs wire_unique_high (funcdata_varnode.cc:89)\n'

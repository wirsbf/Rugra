...
# (8c26662, Varnode::getLocalType full port + localType dispatch table) via
# a complete git archive, never from the live crate; no source overlays are
# applied.
...
rugra_base_commit=8c26662b742dac535cffdba98d9e970f90d29545
rugra_base_tree=6861795b811ac519edee7bf35f79481c39f0000e
rugra_base_src_tree=fbe806f04e70238c1f2235a0ff70be0bad3edf1c
...
actual_varnode_cc_blob=$(/usr/bin/env -i PATH="$clean_path" LC_ALL=C GIT_CONFIG_NOSYSTEM=1 \
  "$host_git_bin" -C "$ghidra_root" rev-parse \
  "$oracle_commit:Ghidra/Features/Decompiler/src/decompile/cpp/varnode.cc")
actual_typeop_cc_blob=$(/usr/bin/env -i PATH="$clean_path" LC_ALL=C GIT_CONFIG_NOSYSTEM=1 \
  "$host_git_bin" -C "$ghidra_root" rev-parse \
  "$oracle_commit:Ghidra/Features/Decompiler/src/decompile/cpp/typeop.cc")
...
require(
    ...
    "immutable complete crate snapshot: git archive of the frozen production commit 8c26662 with the Varnode::getLocalType full port, STOP early-return, and the localType dispatch table; no source overlays",
)
require(
    "archive paths",

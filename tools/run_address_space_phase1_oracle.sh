...
runner="$repo_root/tools/run_address_space_phase1_oracle.sh"
...
metadata="$repo_root/tests/oracle/address_space_phase1_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/address_space_phase1_1204.cc"
rust_fixture="$repo_root/tests/oracle/address_space_phase1_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-address-phase1-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-address-phase1-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/address_space_phase1_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/address_space_phase1_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/address_space_phase1_1204.rs",
...
crate_hasher.update(b"rugra-address-phase1-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/address_space_phase1_1204.cc"),
    pathlib.Path("tests/oracle/address_space_phase1_1204.rs"),
    pathlib.Path("tests/oracle/address_space_phase1_1204.metadata.json"),
    pathlib.Path("docs/api/address.md"),
    pathlib.Path("tools/run_address_space_phase1_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "ADDRESS-SPACE-PHASE1-1204")
require(
    "stable function id",
...
require(
    ...
    "sha256 of rugra-address-phase1-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("none_compat_fallback", "MATCH"),
    ("space_ordering", "MATCH"),
    ("tag_identity", "MATCH"),
    ("wrap_overlap_tagged", "MATCH"),
    ("none_arith_overlap", "UNTESTED"),
    ("none_display_legacy", "UNTESTED"),
    ("maximal_sentinel_bridge", "UNTESTED"),
...
registry_hasher.update(b"rugra-address-phase1-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/address_space_phase1_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/address_space_phase1_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/address_space_phase1_1204.rs"
...
cpp_binary="$oracle_tmp/address_space_phase1_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "address_space_phase1_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/address_space_phase1_1204_rust"
...
if len(lines) != 35:
    raise SystemExit(f"expected thirty-five fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=ADDRESS-SPACE-PHASE1-1204|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "none_compat_fallback", "space_ordering", "tag_identity",
    "wrap_overlap_tagged",
]
...
metadata = json.loads(
    (repo / "tests/oracle/address_space_phase1_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_address_space_phase1_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/address_space_phase1_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/address_space_phase1_1204.rs").read_bytes()),
...
hasher.update(b"rugra-address-phase1-base-overlay-v1\0")
...
echo "address_space_phase1_1204: MATCH covered_projection=4/4 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

#!/usr/bin/env bash
# CONDEXE-ERROR-0006 — locked Ghidra 12.0.4 oracle gate for the
...
# pullback-integration commit e6b4ec0 (trueout+pullback already in), so the
# only source overlay is src/condexe.rs itself; no BFD, no loader binary, no
# SLEIGH assets (synthetic FixtureArchitecture).
set -euo pipefail
...
rugra_source_commit=e6b4ec073045e70a3d05afb45b35d5416bc76fd9
rugra_source_tree=f69bf3b5669fe04a4a97bc11a9bfbc6966012c2f
rugra_source_src_tree=e6451db85ad85f8bcedb882fd5f46eae58052e1e
rugra_condexe_blob=b1bec547cfbcd0832ea43e12322186687d8ee989
...
metadata="$repo_root/tests/oracle/condexe_error_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/condexe_error_1204.cc"
rust_fixture="$repo_root/tests/oracle/condexe_error_1204.rs"
condexe_overlay="$repo_root/src/condexe.rs"
runner="$repo_root/tools/run_condexe_error_oracle.sh"

oracle_tmp=$(mktemp -d /tmp/rugra-condexe-error-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-condexe-error-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
cp "$cpp_fixture" "$snapshot_root/tests/oracle/condexe_error_1204.cc"
cp "$rust_fixture" "$snapshot_root/tests/oracle/condexe_error_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/condexe_error_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_condexe_error_oracle.sh"
...
require("fixture", metadata["fixture_id"], "CONDEXE-ERROR-0006")
...
rustc --edition=2021 -O \
  ...
  "$snapshot_root/tests/oracle/condexe_error_1204.rs" \
  -o "$oracle_tmp/condexe_error_1204_rust"

set +e
"$oracle_tmp/condexe_error_1204_cpp" \
  >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
ghidra_status=$?
"$oracle_tmp/condexe_error_1204_rust" \

...
runner="$repo_root/tools/run_block_calcloop_oracle.sh"
...
metadata="$repo_root/tests/oracle/block_calcloop_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/block_calcloop_1204.cc"
rust_fixture="$repo_root/tests/oracle/block_calcloop_1204.rs"
block_rs="$repo_root/src/block.rs"
blockaction_rs="$repo_root/src/blockaction.rs"
block_doc="$repo_root/docs/api/block.md"
blockaction_doc="$repo_root/docs/api/blockaction.md"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$block_rs" "$blockaction_rs" "$block_doc" "$blockaction_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-block-calcloop-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-block-calcloop-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C GIT_CONFIG_NOSYSTEM=1 \
  ...
  "$block_rs" "$blockaction_rs" "$block_doc" "$blockaction_doc" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, block_raw, blockaction_raw,
    block_doc_raw, blockaction_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/block_calcloop_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/block_calcloop_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/block_calcloop_1204.rs",
    pathlib.Path(block_raw): repo / "src/block.rs",
    pathlib.Path(blockaction_raw): repo / "src/blockaction.rs",
    pathlib.Path(block_doc_raw): repo / "docs/api/block.md",
    pathlib.Path(blockaction_doc_raw): repo / "docs/api/blockaction.md",
}
...
crate_hasher.update(b"rugra-block-calcloop-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/block_calcloop_1204.cc"),
    pathlib.Path("tests/oracle/block_calcloop_1204.rs"),
    pathlib.Path("tests/oracle/block_calcloop_1204.metadata.json"),
    pathlib.Path("docs/api/block.md"),
    pathlib.Path("docs/api/blockaction.md"),
    pathlib.Path("tools/run_block_calcloop_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "BLOCK-CALCLOOP-0001")
require(
    "stable function id",
    metadata["stable_function_id"],
    "GH12-F-3bc919ab99b4782145f5",
)
...
observed_hashes = {
    ...
    "block_rs_sha256": sha(crate_bytes["src/block.rs"]),
    "blockaction_rs_sha256": sha(crate_bytes["src/blockaction.rs"]),
    "block_doc_sha256": sha(special[special_paths[3].as_posix()]),
    "blockaction_doc_sha256": sha(special[special_paths[4].as_posix()]),
...
require(
    ...
    "sha256 of rugra-block-calcloop-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("reducible_backedge_labeling", "MATCH"),
    ("nested_loops_labeling", "MATCH"),
    ("self_loop_slot_order", "MATCH"),
    ("loopout_skip_on_rerun", "MATCH"),
    ("visited_truncate", "MATCH"),
    ("irreducible_endtoend_calcloop", "MATCH"),
    ("structure_reset_chain_dominator", "MATCH"),
    ("production_pipeline_wiring", "MATCH"),
    ("copymap_numdesc_indeterminate", "MATCH"),
    ("edgeflag_bit7_collision", "UNTESTED"),
    ("empty_graph_early_return", "UNTESTED"),
    ("stale_loop_label_skip_within_call", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-block-calcloop-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/block_calcloop_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/block_calcloop_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/block_calcloop_1204.rs"
...
cpp_binary="$oracle_tmp/block_calcloop_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "block_calcloop_1204: GHIDRA_LOCKED_OUTPUT_OK overall=MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/block_calcloop_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=BLOCK-CALCLOOP-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "reducible_backedge_labeled", "nested_loops_inner_outer",
    "self_loop_then_child", "calc_loop_twice_loopout_skip",
    "visited_truncate_no_label", "irreducible_endtoend_calcloop",
    "irreducible_endtoend_calcloop_roots", "structure_reset_chain_dominator",
    "structure_reset_chain_dominator_roots",
]
...
metadata = json.loads(
    (repo / "tests/oracle/block_calcloop_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_block_calcloop_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/block_calcloop_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/block_calcloop_1204.rs").read_bytes()),
    comparand["rust_fixture_sha256"],
)
require(
    "block implementation",
    sha((repo / "src/block.rs").read_bytes()),
    comparand["block_rs_sha256"],
)
require(
    "blockaction implementation",
    sha((repo / "src/blockaction.rs").read_bytes()),
    comparand["blockaction_rs_sha256"],
)
require(
    "block API document",
    sha((repo / "docs/api/block.md").read_bytes()),
    comparand["block_doc_sha256"],
)
require(
    "blockaction API document",
    sha((repo / "docs/api/blockaction.md").read_bytes()),
    comparand["blockaction_doc_sha256"],
)

overlay_files = {"src/block.rs", "src/blockaction.rs"}
...
hasher.update(b"rugra-block-calcloop-base-overlay-v1\0")
...
echo "block_calcloop_1204: MATCH covered_projection=7/7 overall=MATCH stdout_sha256=$actual_stdout_sha"

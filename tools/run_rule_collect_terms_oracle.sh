...
metadata="$repo_root/tests/oracle/rule_collect_terms_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/rule_collect_terms_1204.cc"
rust_fixture="$repo_root/tests/oracle/rule_collect_terms_1204.rs"
runner="$repo_root/tools/run_rule_collect_terms_oracle.sh"
bfd_include=/tmp/rugra-ghidra-bfd-2.38/usr/include
bfd_header="$bfd_include/bfd.h"
bfd_library=/usr/lib/x86_64-linux-gnu/libbfd-2.38-system.so

oracle_tmp=$(mktemp -d /tmp/rugra-rule-collect-terms-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-rule-collect-terms-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
expected_paths = {
    metadata_path: repo_root / "tests/oracle/rule_collect_terms_1204.metadata.json",
    cpp_fixture_path: repo_root / "tests/oracle/rule_collect_terms_1204.cc",
    rust_fixture_path: repo_root / "tests/oracle/rule_collect_terms_1204.rs",
    runner_path: repo_root / "tools/run_rule_collect_terms_oracle.sh",
}
...
crate_hasher.update(b"rugra-rule-collect-terms-lib-snapshot-v1\0")
crate_bytes = {}
...
special_paths = [
    pathlib.Path("tests/oracle/rule_collect_terms_1204.cc"),
    pathlib.Path("tests/oracle/rule_collect_terms_1204.rs"),
    pathlib.Path("tests/oracle/rule_collect_terms_1204.metadata.json"),
    pathlib.Path("tools/run_rule_collect_terms_oracle.sh"),
]
special = {path.as_posix(): snapshot_file(path) for path in special_paths}
require(
    "runner snapshot/live copy",
    sha256(special["tools/run_rule_collect_terms_oracle.sh"]),
    runner_sha,
)
...
metadata = json.loads(special["tests/oracle/rule_collect_terms_1204.metadata.json"])
require("metadata schema", metadata["schema_version"], 1)
require("fixture id", metadata["fixture_id"], "RULE-COLLECTTERMS-0001")
...
observed = {
    "cpp_fixture_sha256": sha256(special["tests/oracle/rule_collect_terms_1204.cc"]),
    "rust_fixture_sha256": sha256(special["tests/oracle/rule_collect_terms_1204.rs"]),
...
require(
    ...
    "sha256 of rugra-rule-collect-terms-lib-snapshot-v1 plus sorted length-prefixed relative paths and contents",
)
...
require(
    ...
    "PARTIAL_MATCH: seven target-relevant structural projections match; full-state input/output remains MISMATCH through OPBANK-0001 and ARCH-0001, TYPE-UNKNOWN-0001 and explicitly listed branches remain UNTESTED",
)
for key in (
    "like_term_nonoverflow",
    "like_term_storage_mask_wrap",
    "like_term_uintb_wrap",
    "like_term_zero_result",
    "constant_reverse_lastconst_and_zeroing",
    "constant_storage_mask_wrap",
    "constant_uintb_wrap",
):
    require(
        f"coverage.{key}",
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/rule_collect_terms_1204.cc" \
...
printf 'rule_collect_terms_1204: PARTIAL_MATCH target_structural_cases=7 records=14 dependencies=OPBANK-0001,ARCH-0001 untested=TYPE-UNKNOWN-0001\n'

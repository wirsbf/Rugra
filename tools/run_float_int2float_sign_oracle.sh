...
# FLOAT-OPINT2FLOAT-SIGN-0001 oracle runner (pin-base schema2, immutable fd).
...
# float_int2float_sign_1204 fixture pair, and requires byte-identical
...
runner="$repo_root/tools/run_float_int2float_sign_oracle.sh"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-float-int2float-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-float-int2float-1204.??????) /usr/bin/rm -rf -- "$oracle_tmp" ;;
...
for candidate in \
  tests/oracle/float_int2float_sign_1204.metadata.json \
  tests/oracle/float_int2float_sign_1204.cc \
  tests/oracle/float_int2float_sign_1204.rs \
...
metadata="$oracle_tmp/candidate/float_int2float_sign_1204.metadata.json"
cpp_fixture="$oracle_tmp/candidate/float_int2float_sign_1204.cc"
rust_fixture="$oracle_tmp/candidate/float_int2float_sign_1204.rs"
...
require("fixture id", metadata["fixture_id"], "FLOAT-OPINT2FLOAT-SIGN-0001")
...
expected_coverage = {
    "int2float_signed_negatives", "int2float_sign_extend_discard",
    "int2float_boundary_rounding", "float2float_widening",
    "float2float_narrowing_rounding", "float2float_special_clamps",
}
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_cxx_bin" \
  ...
  -o "$oracle_tmp/float_int2float_sign_cpp"
...
/usr/bin/env -i HOME="$resolved_user_home" RUSTUP_HOME="$resolved_user_home/.rustup" \
  ...
  "$rust_fixture" -o "$oracle_tmp/float_int2float_sign_rust"

/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/float_int2float_sign_cpp" >"$oracle_tmp/ghidra.stdout" \
  2>"$oracle_tmp/ghidra.stderr"
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/float_int2float_sign_rust" >"$oracle_tmp/rugra.stdout" \
...
/usr/bin/printf 'float_int2float_sign_1204: covered_projection=36/36 cases (37 lines incl. banner) projection_status=MATCH overall_status=MATCH residual=none\n'

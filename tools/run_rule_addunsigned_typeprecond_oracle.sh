...
runner="$repo_root/tools/run_rule_addunsigned_typeprecond_oracle.sh"
...
for candidate in \
  tests/oracle/rule_addunsigned_typeprecond_1204.metadata.json \
  tests/oracle/rule_addunsigned_typeprecond_1204.cc \
  tests/oracle/rule_addunsigned_typeprecond_1204.rs \
...
metadata="$oracle_tmp/candidate/rule_addunsigned_typeprecond_1204.metadata.json"
cpp_fixture="$oracle_tmp/candidate/rule_addunsigned_typeprecond_1204.cc"
rust_fixture="$oracle_tmp/candidate/rule_addunsigned_typeprecond_1204.rs"
...
for case_name in ("default_unknown_ff_reject", "canonical_uint_ff_transform",
                  "canonical_uint_7f_high_quarter_reject", "getoplist_int_add_only"):
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_cxx_bin" \
  ...
  -o "$oracle_tmp/rule_addunsigned_cpp"
...
/usr/bin/printf 'rule_addunsigned_typeprecond_1204: covered_projection=7/7 projection_status=MATCH overall_status=MISMATCH\n'
/usr/bin/printf 'residuals=RULE-ADDUNSIGNED-TYPEPRECOND-0001(None/op-aware-union/char/equate/enum/rule-name/pool-repeat)\n'

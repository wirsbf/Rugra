...
rust_fixture="$repo_root/tests/oracle/fspec_space_identity_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-fspec-space-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-fspec-space-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/fspec_space_identity_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/fspec_space_identity_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/fspec_space_identity_1204.rs",
...
crate_hasher.update(b"rugra-fspec-space-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/fspec_space_identity_1204.cc"),
    pathlib.Path("tests/oracle/fspec_space_identity_1204.rs"),
...
require("fixture id", metadata["fixture_id"], "FSPEC-SPACE-IDENTITY-1204")
require(
    "stable function id",
...
require(
    ...
    "sha256 of rugra-fspec-space-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("registration_lookup", "MATCH"),
    ("same_offset_discrimination", "MATCH"),
    ("print_raw_forms", "MATCH"),
    ("encode_decode_roundtrip", "MATCH"),
    ("cross_space_exceptions", "MATCH"),
    ("packed_special_space_codec", "UNTESTED"),
    ("iop_print_raw", "UNTESTED"),
    ("join_space_codecs", "UNTESTED"),
    ("register_name_decode", "UNTESTED"),
...
registry_hasher.update(b"rugra-fspec-space-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/fspec_space_identity_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/fspec_space_identity_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/fspec_space_identity_1204.rs"
...
cpp_binary="$oracle_tmp/fspec_space_identity_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "fspec_space_identity_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/fspec_space_identity_1204_rust"
...
expected_cases = [
    "registration_lookup", "same_offset_discrimination", "print_raw_forms",
    "encode_decode_roundtrip", "cross_space_exceptions",
]
...
metadata = json.loads(
    (repo / "tests/oracle/fspec_space_identity_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "C++ fixture",
    sha((repo / "tests/oracle/fspec_space_identity_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/fspec_space_identity_1204.rs").read_bytes()),
...
hasher.update(b"rugra-fspec-space-base-overlay-v1\0")
...
echo "fspec_space_identity_1204: MATCH covered_projection=5/5 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

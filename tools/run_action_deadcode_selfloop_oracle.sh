...
runner="$repo_root/tools/run_action_deadcode_selfloop_oracle.sh"
...
rugra_source_commit=5f6f34ffcb13378189f9fe962d08e4d999184fc5
rugra_source_tree=d180210e9f8691d817ed95428f53228c5d3f42c8
...
rugra_source_varnode_blob=a041d0b2130eaf0d47b8974e6686e6bbd2879100
rugra_source_funcdata_blob=43195dd9ac36e391986b6bf08f26c392df478129
...
metadata="$repo_root/tests/oracle/action_deadcode_selfloop_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/action_deadcode_selfloop_1204.cc"
rust_fixture="$repo_root/tests/oracle/action_deadcode_selfloop_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-action-deadcode-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-action-deadcode-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S - \
  ...
  "$rugra_source_varnode_blob" "$rugra_source_funcdata_blob" \
  "$rugra_source_coreaction_blob" \
...
(
    ...
    source_tree, source_src_tree, source_sleigh_tree, source_varnode_blob,
    source_funcdata_blob, source_coreaction_blob, cargo_toml_blob,
...
base_hasher.update(b"rugra-action-deadcode-base-v1\0")
...
special_paths = [
    "tests/oracle/action_deadcode_selfloop_1204.cc",
    "tests/oracle/action_deadcode_selfloop_1204.rs",
    "tests/oracle/action_deadcode_selfloop_1204.metadata.json",
    "tools/run_action_deadcode_selfloop_oracle.sh",
]
special = {path: live_file(path) for path in special_paths}
metadata = json.loads(special["tests/oracle/action_deadcode_selfloop_1204.metadata.json"])
...
require("source funcdata blob", source["base_funcdata_blob"], source_funcdata_blob)
...
for relative in ("src/varnode.rs", "src/funcdata.rs", "src/coreaction.rs"):
...
coreaction = (snapshot / "src/coreaction.rs").read_text()
for needle in ("    fn push_consumed(\n", "    fn propagate_consumed(\n"):
    require(f"visibility transform count {needle.strip()}", coreaction.count(needle), 1)
    coreaction = coreaction.replace(needle, needle.replace("fn ", "pub fn "))
(snapshot / "src/coreaction.rs").write_text(coreaction)
rust_transform = (
    "ActionDeadCode::push_consumed and ActionDeadCode::propagate_consumed "
    "made public in isolated snapshot only\n"
).encode()
...
observed = {
    "cpp_fixture_sha256": sha(special["tests/oracle/action_deadcode_selfloop_1204.cc"]),
    "rust_fixture_sha256": sha(special["tests/oracle/action_deadcode_selfloop_1204.rs"]),
    "runner_sha256": runner_sha,
    "rust_visibility_transform_sha256": sha(rust_transform),
}
require("runner snapshot/live", sha(special["tools/run_action_deadcode_selfloop_oracle.sh"]), runner_sha)
...
package_blocks = base_bytes["Cargo.lock"].decode().split("[[package]]")[1:]
packages = []
...
# Temporary, deterministic Ghidra test visibility/read-only instrumentation.
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S - \
...
patches = {
    "coreaction.cc": [
        (
            ...
            "void ActionDeadCode::pushConsumed(uintb val,Varnode *vn,vector<Varnode *> &worklist)",
        ),
    ],
    "coreaction.hh": [
        (
    ...
    "op.hh": [
        (
            ...
            "  uint4 fixtureGetFlags(void) const { return flags; }\n"
            "  uint4 fixtureGetAdditionalFlags(void) const { return addlflags; }\n"
            "  bool isDead(void) const { return ((flags&PcodeOp::dead)!=0); }",
        ),
    ],
    "varnode.hh": [
        (
            ...
            "  uint4 getFlags(void) const { return flags; } ///< Get all the boolean attributes\n"
            "  uint2 fixtureGetAdditionalFlags(void) const { return addlflags; }\n"
            "  void fixtureCalcCover(void) const { calcCover(); }\n"
            "  bool fixtureHasCoverObject(void) const { return cover != (Cover *)0; }",
        ),
    ],
    "heritage.hh": [
        (
    ...
    "funcdata.hh": [
        (
            ...
            "  bool fixtureHasDefVarnode(Varnode *vn) const {\n"
            "    for(VarnodeDefSet::const_iterator i=vbank.beginDef();i!=vbank.endDef();++i)\n"
            "      if (*i == vn) return true;\n"
            "    return false;\n"
            "  }",
        ),
...
hasher.update(b"ghidra-action-deadcode-instrumentation-v1\0")
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_cxx_bin" \
  ...
  "$snapshot_root/tests/oracle/action_deadcode_selfloop_1204.cc" \
  ...
  "$bfd_library" -lz -o "$oracle_tmp/action_deadcode_selfloop_1204_cpp"
...
if ! (
  ...
  RUSTC="$host_rustc_bin" "$host_cargo_bin" build --quiet --locked --offline --lib
); then
...
/usr/bin/env -i HOME="$HOME" RUSTUP_HOME="$HOME/.rustup" \
  ...
  "$snapshot_root/tests/oracle/action_deadcode_selfloop_1204.rs" \
  -o "$oracle_tmp/action_deadcode_selfloop_1204_rust"

set +e
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$host_timeout_bin" --signal=TERM --kill-after=2s 10s \
  "$oracle_tmp/action_deadcode_selfloop_1204_cpp" \
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$host_timeout_bin" --signal=TERM --kill-after=2s 10s \
  "$oracle_tmp/action_deadcode_selfloop_1204_rust" \
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S - \
  "$snapshot_root/tests/oracle/action_deadcode_selfloop_1204.metadata.json" \
...
for key, path in paths.items():
    ...
    if key == "rugra_stderr_normalized_sha256":
        data, count = re.subn(rb"op=0x[0-9a-f]+", b"op=<ptr>", data)
        if count != 1:
            raise SystemExit(f"Rugra stderr pointer record count drifted: {count}")
...
apply_cases = [
    "ordinary_chain_no_seed", "selfloop_repeated_live", "two_phi_cycle_live",
    "autolive_input_output", "callother_without_spec_hold_output",
    "callother_dead_output_unset_only", "direct_op_unset_output",
    "direct_op_set_output", "direct_new_varnode_out",
    "last_chance_load_pass1", "last_chance_load_pass2_gate",
]

def validate_records(label, lines):
    case_lines = [line for line in lines if line.startswith("case=")]
    if len(case_lines) != 2 * len(apply_cases):
        raise SystemExit(f"{label} apply record count mismatch: {len(case_lines)}")
    observed = [line.split("|", 1)[0][5:] for line in case_lines[::2]]
    if observed != apply_cases:
        raise SystemExit(f"{label} apply case order mismatch: {observed}")
    if any("|stage=after|result=0|count=0|" not in line for line in case_lines[1::2]):
        raise SystemExit(f"{label} apply return/count mismatch")
    trace_lines = [line for line in lines if line.startswith("trace=")]
    if len(trace_lines) != 14:
        raise SystemExit(f"{label} helper trace record count mismatch: {len(trace_lines)}")
    return case_lines + trace_lines

ghidra_records = validate_records("Ghidra", ghidra_lines)
rugra_records = validate_records("Rugra", rugra_lines)

def target_projection(records):
    # Full raw flags/addl are retained. The fixture itself renders the named
    # LOAD space annotation as the semantic AddrSpace index on both sides.
    result = list(records)
    expected_stages = [
        "case=last_chance_load_pass1|stage=before|",
        "case=last_chance_load_pass1|stage=after|",
        "case=last_chance_load_pass2_gate|stage=before|",
    ]
    pattern = r"space\{present=1,space=const,size=8,offset=3,"
    for line in records:
        count = len(re.findall(pattern, line))
        if count:
            if not (line.startswith("case=last_chance_load_pass1|") or
                    line.startswith("case=last_chance_load_pass2_gate|")):
                raise SystemExit("LOAD space-reference normalization escaped its cases")
            if count != 1:
                raise SystemExit("LOAD space-reference normalization count drifted")
            stage = next((item for item in expected_stages if line.startswith(item)), None)
            if stage is None:
                raise SystemExit("LOAD space-reference stage drifted")
            expected_stages.remove(stage)
        if line.startswith("case=last_chance_load_pass2_gate|stage=after|"):
            if line.count("space{present=0}") != 1 or "space{present=1" in line:
                raise SystemExit("pass2-after LOAD space tombstone drifted")
    if expected_stages:
        raise SystemExit(f"LOAD space-reference stages missing: {expected_stages}")
    return result

ghidra_projection = target_projection(ghidra_records)
rugra_projection = target_projection(rugra_records)
if ghidra_projection != rugra_projection:
    import difflib
    print("\n".join(difflib.unified_diff(
        ghidra_projection, rugra_projection,
...
printf 'action_deadcode_selfloop_1204: %s target_projection=MATCH lines=36 rust_exit=%s timeout=%s raw_diff=%s\n' \

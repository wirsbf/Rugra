...
runner="$repo_root/tools/run_varmap_localwindow_oracle.sh"
...
rugra_base_commit=dfd27ad7fb53f1ebb878b491f6b289db22917440
rugra_base_tree=8ccc92a6cf24d9bbd71c16e6bd64c25533de874d
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/varmap_localwindow_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/varmap_localwindow_1204.cc"
rust_fixture="$repo_root/tests/oracle/varmap_localwindow_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$varmap_rs" "$varmap_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-varmap-localwindow-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-varmap-localwindow-1204.?????? ]]; then
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, varmap_raw, varmap_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/varmap_localwindow_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/varmap_localwindow_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/varmap_localwindow_1204.rs",
...
crate_hasher.update(b"rugra-varmap-localwindow-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/varmap_localwindow_1204.cc"),
    pathlib.Path("tests/oracle/varmap_localwindow_1204.rs"),
    pathlib.Path("tests/oracle/varmap_localwindow_1204.metadata.json"),
    pathlib.Path("docs/api/varmap.md"),
    pathlib.Path("tools/run_varmap_localwindow_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "VARMAP-LOCALWINDOW-0001")
require(
    "stable function id",
...
require(
    ...
    "sha256 of rugra-varmap-localwindow-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
host_values = {
    ...
    "rust_toolchain": host_rust_toolchain,
...
for required_key, required_prefix in (
    ("default_windows_union_tree", "MATCH"),
    ("negative_offset_gate", "MATCH"),
    ("open_array_extension", "MATCH"),
    ("endpoint_bounding", "MATCH"),
    ("naming_branch_split", "MATCH"),
    ("loadguard_addguard_open_hints", "UNTESTED"),
    ("gather_symbols_reinput", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-varmap-localwindow-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/varmap_localwindow_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/varmap_localwindow_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/varmap_localwindow_1204.rs"
...
cpp_binary="$oracle_tmp/varmap_localwindow_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "varmap_localwindow_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/varmap_localwindow_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=VARMAP-LOCALWINDOW-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "default_windows", "gate_and_symbols",
    "open_array", "naming_branches",
]
...
metadata = json.loads(
    (repo / "tests/oracle/varmap_localwindow_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_varmap_localwindow_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/varmap_localwindow_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/varmap_localwindow_1204.rs").read_bytes()),
...
hasher.update(b"rugra-varmap-localwindow-base-overlay-v1\0")
...
echo "varmap_localwindow_1204: MATCH covered_projection=5/5 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

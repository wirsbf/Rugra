#!/usr/bin/env bash
# BLOCKSTRUCT-MULTIGOTO-0001 bilateral runner (B2-MG-RESID-1/2 closure).
#
# Builds the locked Ghidra 12.0.4 oracle fixture (block.hh:573-593
# BlockMultiGoto; block.cc:1720-1753 newBlockMultiGoto / 2918-2951
# scopeBreak+printHeader+nextFlowAfter; blockaction.cc:1456-1458
# ruleBlockGoto isSwitchOut arm; block.cc:3548-3553 grabCaseBasic
# t_multigoto append arm) against the oracle cpp tree, builds the Rugra
# comparand against the crate rlib, runs both, and diffs the sorted
# per-family observation projection (multigoto facts, switch case facts,
# direct-peel contract lines).
set -euo pipefail
...
cpp_fixture="$repo_root/tests/oracle/blockmultigoto_1204.cc"
rust_fixture="$repo_root/tests/oracle/blockmultigoto_1204.rs"
metadata="$repo_root/tests/oracle/blockmultigoto_1204.metadata.json"
...
oracle_tmp=$(mktemp -d /tmp/rugra-blockmultigoto-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-blockmultigoto-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
g++ -std=c++11 -O2 -I"$cpp_root" -I"$bfd_root/usr/include" \
  ...
  -o "$oracle_tmp/blockmultigoto_1204"
...
rustc --edition=2021 -O -L "dependency=$fixture_target/debug/deps" \
  ...
  -o "$oracle_tmp/blockmultigoto_rugra"

"$oracle_tmp/blockmultigoto_1204" >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
"$oracle_tmp/blockmultigoto_rugra" >"$oracle_tmp/rugra.stdout" 2>"$oracle_tmp/rugra.stderr"

if diff -u "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout" \
    >"$oracle_tmp/bilateral.diff"; then
  cat "$oracle_tmp/ghidra.stdout"
  printf 'blockmultigoto_1204: MATCH\n'
else
  cat "$oracle_tmp/bilateral.diff"
  printf 'blockmultigoto_1204: MISMATCH (%s diff lines)\n' \

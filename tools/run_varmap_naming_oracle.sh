...
runner="$repo_root/tools/run_varmap_naming_oracle.sh"
...
rugra_base_commit=ee29a32487702a8044d910202903e83a23996c9e
rugra_base_tree=5539a3cef4797b1452b62d2d97e656b916e14a3e
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/varmap_naming_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/varmap_naming_1204.cc"
rust_fixture="$repo_root/tests/oracle/varmap_naming_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$varmap_rs" "$printc_rs" "$varmap_doc" "$printc_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-varmap-naming-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-varmap-naming-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C GIT_CONFIG_NOSYSTEM=1 \
  ...
  "$varmap_rs" "$printc_rs" "$varmap_doc" "$printc_doc" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, varmap_raw, printc_raw,
    varmap_doc_raw, printc_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/varmap_naming_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/varmap_naming_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/varmap_naming_1204.rs",
    pathlib.Path(varmap_raw): repo / "src/varmap.rs",
    pathlib.Path(printc_raw): repo / "src/printc.rs",
...
crate_hasher.update(b"rugra-varmap-naming-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/varmap_naming_1204.cc"),
    pathlib.Path("tests/oracle/varmap_naming_1204.rs"),
    pathlib.Path("tests/oracle/varmap_naming_1204.metadata.json"),
    pathlib.Path("docs/api/varmap.md"),
    pathlib.Path("docs/api/printc.md"),
    pathlib.Path("tools/run_varmap_naming_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "VARMAP-NAMING-0001")
require(
    "stable function id",
...
observed_hashes = {
    ...
    "printc_doc_sha256": sha(special[special_paths[4].as_posix()]),
...
require(
    ...
    "sha256 of rugra-varmap-naming-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("shared_base_counter_across_prefixes", "MATCH"),
    ("cross_category_param_naming", "MATCH"),
    ("nametree_walk_order_and_dedup", "MATCH"),
    ("locked_display_name_preservation", "MATCH"),
    ("stack_y_x_plain_paths", "MATCH"),
    ("assign_idempotence", "MATCH"),
    ("builddefaultname_vn_path", "UNTESTED"),
    ("translate_register_table_plumbing", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-varmap-naming-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/varmap_naming_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/varmap_naming_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/varmap_naming_1204.rs"
...
cpp_binary="$oracle_tmp/varmap_naming_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "varmap_naming_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/varmap_naming_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=VARMAP-NAMING-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "cross_category_shared_base", "sequential_increment_shared_base",
    "typelock_display_and_bump", "scopelocal_stack_paths",
    "direct_flag_paths", "undef_dedup_and_shared_base",
]
...
metadata = json.loads(
    (repo / "tests/oracle/varmap_naming_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_varmap_naming_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/varmap_naming_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/varmap_naming_1204.rs").read_bytes()),
    comparand["rust_fixture_sha256"],
)
for label, relative in (
    ("varmap implementation", "src/varmap.rs"),
    ("printc implementation", "src/printc.rs"),
    ("varmap API document", "docs/api/varmap.md"),
    ("printc API document", "docs/api/printc.md"),
):
...
overlay_files = {"src/varmap.rs", "src/printc.rs"}
...
hasher.update(b"rugra-varmap-naming-base-overlay-v1\0")
...
echo "varmap_naming_1204: MATCH covered_projection=6/6 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

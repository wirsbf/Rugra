...
runner="$repo_root/tools/run_setcasts_output_bank_oracle.sh"
...
metadata="$repo_root/tests/oracle/setcasts_output_bank_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/setcasts_output_bank_1204.cc"
rust_fixture="$repo_root/tests/oracle/setcasts_output_bank_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$varnode_rs" "$coreaction_rs" "$varnode_doc" "$coreaction_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-setcasts-output-bank-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-setcasts-output-bank-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C GIT_CONFIG_NOSYSTEM=1 \
  ...
  "$varnode_rs" "$coreaction_rs" "$varnode_doc" "$coreaction_doc" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, varnode_raw, coreaction_raw,
    varnode_doc_raw, coreaction_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/setcasts_output_bank_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/setcasts_output_bank_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/setcasts_output_bank_1204.rs",
...
crate_hasher.update(b"rugra-setcasts-output-bank-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/setcasts_output_bank_1204.cc"),
    pathlib.Path("tests/oracle/setcasts_output_bank_1204.rs"),
    pathlib.Path("tests/oracle/setcasts_output_bank_1204.metadata.json"),
    pathlib.Path("docs/api/varnode.md"),
    pathlib.Path("docs/api/coreaction.md"),
    pathlib.Path("tools/run_setcasts_output_bank_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "VARNODE-BANK-KEY-LIVE-0001")
require(
    "stable function id",
...
require(
    ...
    "sha256 of rugra-setcasts-output-bank-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("opsetoutput_swap_key_lifecycle", "MATCH"),
    ("makefree_under_inplace_key_drift", "MATCH"),
    ("stale_def_tree_position_observation", "MATCH"),
    ("destroy_after_transition_cycle", "MATCH"),
    ("rebind_same_output_noop", "MATCH"),
    ("setinput_transition_fresh_input", "MATCH"),
    ("foreign_handle_ownership_guard", "NO_ORACLE"),
    ("cast_output_union_resolution_closure", "UNTESTED"),
    ("makefree_on_input_class_varnode", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-setcasts-output-bank-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/setcasts_output_bank_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/setcasts_output_bank_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/setcasts_output_bank_1204.rs"
...
cpp_binary="$oracle_tmp/setcasts_output_bank_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "setcasts_output_bank_1204: GHIDRA_LOCKED_OUTPUT_OK overall=MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/setcasts_output_bank_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=VARNODE-BANK-KEY-LIVE-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "opsetoutput_swap", "makefree_inplace_key_drift",
    "destroy_after_transitions", "rebind_same_output_noop",
]
...
metadata = json.loads(
    (repo / "tests/oracle/setcasts_output_bank_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_setcasts_output_bank_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/setcasts_output_bank_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/setcasts_output_bank_1204.rs").read_bytes()),
    comparand["rust_fixture_sha256"],
)
for label, relative in (
    ("varnode implementation", "src/varnode.rs"),
    ("coreaction implementation", "src/coreaction.rs"),
    ("varnode API document", "docs/api/varnode.md"),
    ("coreaction API document", "docs/api/coreaction.md"),
):
...
overlay_files = {"src/varnode.rs", "src/coreaction.rs"}
...
hasher.update(b"rugra-setcasts-output-bank-base-overlay-v1\0")
...
echo "setcasts_output_bank_1204: MATCH covered_projection=4/4 overall=MATCH stdout_sha256=$actual_stdout_sha"

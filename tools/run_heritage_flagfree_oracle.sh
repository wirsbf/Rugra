...
runner="$repo_root/tools/run_heritage_flagfree_oracle.sh"
...
rugra_base_commit=17c64d3437966bb180312fcac92006d1074bd93e
rugra_base_tree=11212874e9732da30d1c8de7e487deda58fefad0
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/heritage_flagfree_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/heritage_flagfree_1204.cc"
rust_fixture="$repo_root/tests/oracle/heritage_flagfree_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-heritage-flagfree-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-heritage-flagfree-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/heritage_flagfree_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/heritage_flagfree_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/heritage_flagfree_1204.rs",
...
crate_hasher.update(b"rugra-heritage-flagfree-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/heritage_flagfree_1204.cc"),
    pathlib.Path("tests/oracle/heritage_flagfree_1204.rs"),
    pathlib.Path("tests/oracle/heritage_flagfree_1204.metadata.json"),
    pathlib.Path("docs/api/heritage.md"),
    pathlib.Path("tools/run_heritage_flagfree_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "HERITAGE-FLAGFREE-SSA-0001")
...
require(
    ...
    "sha256 of rugra-heritage-flagfree-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("canonical_written_link", "MATCH"),
    ("canonical_unwritten_input_promotion", "MATCH"),
    ("canonical_diamond_phi_link", "MATCH"),
    ("free_with_reader_census", "MATCH"),
    ("direct_route_invariants", "UNTESTED"),
...
registry_hasher.update(b"rugra-heritage-flagfree-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/heritage_flagfree_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/heritage_flagfree_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/heritage_flagfree_1204.rs"
...
cpp_binary="$oracle_tmp/heritage_flagfree_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "heritage_flagfree_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/heritage_flagfree_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=HERITAGE-FLAGFREE-SSA-0001|"
...
if "|bool_in=R206:1:W+INT_SUB|" not in lines[1]:
    raise SystemExit("written case: BOOL input must link to the INT_SUB write")
if lines[1].find("n.BOOL_NEGATE(R206:1:W+INT_SUB)") == -1:
    raise SystemExit("written case op projection must show the replaced read")
if "|bool_in=R207:1:I|" not in lines[2]:
    raise SystemExit("unwritten case: BOOL input must be the promoted input (cc:2499-2502)")
if "|bool_in=R206:1:W+MULTIEQUAL|" not in lines[3]:
    raise SystemExit("diamond case: BOOL input must be the phi output")
...
if "s0<R206:1:W+INT_SUB>#p2" not in lines[3] or "s1<R206:1:W+INT_OR>#p1" not in lines[3]:
...
metadata = json.loads(
    (repo / "tests/oracle/heritage_flagfree_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_heritage_flagfree_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
for label, relative, key in (
    ("C++ fixture", "tests/oracle/heritage_flagfree_1204.cc", "cpp_fixture_sha256"),
    ("Rust fixture", "tests/oracle/heritage_flagfree_1204.rs", "rust_fixture_sha256"),
...
hasher.update(b"rugra-heritage-flagfree-base-overlay-v1\0")
...
echo "heritage_flagfree_1204: MATCH covered_projection=3/3 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

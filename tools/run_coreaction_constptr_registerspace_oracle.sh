...
rugra_input_commit=2a12c22629c55ec57b0600c78c644d887f847fd7
rugra_input_blob=76d9343ea3add321aa4134856323663b36365807
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/coreaction_constptr_registerspace_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/coreaction_constptr_registerspace_1204.cc"
rust_fixture="$repo_root/tests/oracle/coreaction_constptr_registerspace_1204.rs"
runner="$repo_root/tools/run_coreaction_constptr_registerspace_oracle.sh"
bfd_include=/tmp/rugra-ghidra-bfd-2.38/usr/include
bfd_header="$bfd_include/bfd.h"
bfd_library=/usr/lib/x86_64-linux-gnu/libbfd-2.38-system.so

oracle_tmp=$(mktemp -d /tmp/rugra-coreaction-constptr-registerspace-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-coreaction-constptr-registerspace-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
expected_paths = {
    metadata_path: repo_root / "tests/oracle/coreaction_constptr_registerspace_1204.metadata.json",
    cpp_fixture_path: repo_root / "tests/oracle/coreaction_constptr_registerspace_1204.cc",
    rust_fixture_path: repo_root / "tests/oracle/coreaction_constptr_registerspace_1204.rs",
    runner_path: repo_root / "tools/run_coreaction_constptr_registerspace_oracle.sh",
}
...
] + source_files("src") + source_files("crates")
# SLEIGH-RUSTIFY-PHASE2 (applied at MERGEBATCH17): build.rs and
# sleigh_shim/ are retired from the live tree (the vendored kuna-sleigh
# engine is the production decoder); they are dropped from the snapshot
# list. Pinned-commit materialization of the retired files remains
# possible from git history when auditing pre-retirement states.
crate_files = sorted(set(crate_files), key=lambda value: value.as_posix())
crate_hasher = hashlib.sha256()
crate_hasher.update(b"rugra-coreaction-constptr-registerspace-lib-snapshot-v1\0")
crate_bytes = {}
...
special_paths = [
    pathlib.Path("tests/oracle/coreaction_constptr_registerspace_1204.cc"),
    pathlib.Path("tests/oracle/coreaction_constptr_registerspace_1204.rs"),
    pathlib.Path("tests/oracle/coreaction_constptr_registerspace_1204.metadata.json"),
    pathlib.Path("tools/run_coreaction_constptr_registerspace_oracle.sh"),
]
special = {path.as_posix(): snapshot_file(path) for path in special_paths}
require(
    "runner snapshot/live copy",
    sha256(special["tools/run_coreaction_constptr_registerspace_oracle.sh"]),
    runner_sha,
)
...
metadata = json.loads(special["tests/oracle/coreaction_constptr_registerspace_1204.metadata.json"])
require("metadata schema", metadata["schema_version"], 1)
require("fixture id", metadata["fixture_id"], "HTTPDMAIN-F4-WEBTYPE-0001")
...
for dependency, status in (
    ("ARCH-0001", "MISMATCH"),
    ("CALLSPECS-0001", "UNTESTED"),
    ("SCOPELOCAL-0001", "UNTESTED"),
):
    require(
...
observed = {
    "cpp_fixture_sha256": sha256(special["tests/oracle/coreaction_constptr_registerspace_1204.cc"]),
    "rust_fixture_sha256": sha256(special["tests/oracle/coreaction_constptr_registerspace_1204.rs"]),
...
require(
    ...
    "sha256 of rugra-coreaction-constptr-registerspace-lib-snapshot-v1 plus sorted length-prefixed relative paths and contents",
)
...
require(
    ...
    "MATCH: inferPtrSpaces registry (register-space filter) + 3 ActionConstantPtr cases (sz8 container hit converts to PTRSUB charPrint-pointer, sz4 flag-web constant stays bare at the same address, sz8 no-container miss) are byte-identical under the locked oracle; ARCH-0001 enum-space stand-in remains a known MISMATCH at the full-state level and CALLSPECS-0001/SCOPELOCAL-0001 tails remain UNTESTED",
)
for key in (
    "registry_inferptr_spaces",
    "constptr_sz8_container",
    "constptr_sz4_container",
    "constptr_sz8_nocontainer",
):
    require(
        f"coverage.{key}",
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/coreaction_constptr_registerspace_1204.cc" \
  ...
  -o "$oracle_tmp/coreaction_constptr_registerspace_1204_cpp"

CARGO_TARGET_DIR="$oracle_tmp/cargo-target" \
  cargo build --offline --locked --quiet --manifest-path "$snapshot_root/Cargo.toml" --lib
rustc --edition=2021 "$snapshot_root/tests/oracle/coreaction_constptr_registerspace_1204.rs" \
  --extern rugra="$oracle_tmp/cargo-target/debug/librugra.rlib" \
  -L "dependency=$oracle_tmp/cargo-target/debug/deps" \
  -o "$oracle_tmp/coreaction_constptr_registerspace_1204_rust"

"$oracle_tmp/coreaction_constptr_registerspace_1204_cpp" \
  "$snapshot_root/sleigh_specs" "$snapshot_root/examples/curl" \
  >"$oracle_tmp/ghidra.stdout"
"$oracle_tmp/coreaction_constptr_registerspace_1204_rust" >"$oracle_tmp/rugra.stdout"
diff -u "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout"

python3 -I -S - "$snapshot_root/tests/oracle/coreaction_constptr_registerspace_1204.metadata.json" \
...
printf 'coreaction_constptr_registerspace_1204: MATCH cases=4 records=4 dependencies=ARCH-0001 untested=CALLSPECS-0001,SCOPELOCAL-0001'

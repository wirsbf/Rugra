...
# HERITAGE-SUBPIECE-CONST-1204 oracle runner (pin-base schema2): rebuilds
# the locked Ghidra 12.0.4 decompiler and the pinned Rugra base plus the
# candidate src/heritage.rs overlay in an isolated snapshot, runs both
# fixtures byte-for-byte, and verifies every comparand hash in the
# metadata. Covered projection: the production
# Heritage::guardOutputOverlapStack SUBPIECE truncate constants
# (heritage.cc:1336/cc:1358 endian-routed via Address::justifiedContain)
# over every front/back trigger geometry incl. the cc:1329 pre-existing
# output branch, the cc:1327 insertPoint concat ordering and the cc:1340/
# cc:1362 opSetOutput def wiring, plus the cc:1336/cc:1358 call shapes in
# both endian routings (HERITAGE-GUARD-SUBPIECE-CONST-0001).
...
runner="$repo_root/tools/run_heritage_subpiece_const_oracle.sh"
...
rugra_base_commit=3f07f9657b32c662654b52794e07384a5c7ba80f
rugra_base_tree=9621e5acd794aa7a32a26bdc7074870d8725ef00
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/heritage_subpiece_const_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/heritage_subpiece_const_1204.cc"
rust_fixture="$repo_root/tests/oracle/heritage_subpiece_const_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-heritage-subpiece-const-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-heritage-subpiece-const-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/heritage_subpiece_const_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/heritage_subpiece_const_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/heritage_subpiece_const_1204.rs",
...
crate_hasher.update(b"rugra-heritage-subpiece-const-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/heritage_subpiece_const_1204.cc"),
    pathlib.Path("tests/oracle/heritage_subpiece_const_1204.rs"),
    pathlib.Path("tests/oracle/heritage_subpiece_const_1204.metadata.json"),
    pathlib.Path("docs/api/heritage.md"),
    pathlib.Path("tools/run_heritage_subpiece_const_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "HERITAGE-SUBPIECE-CONST-1204")
...
require(
    ...
    "sha256 of rugra-heritage-subpiece-const-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("stack_guard_full", "MATCH"),
    ("front_back_constant", "MATCH"),
    ("be_space_guard_staging", "UNTESTED"),
    ("production_entry_wiring", "UNTESTED"),
    ("negative_piece_geometry", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-heritage-subpiece-const-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/heritage_subpiece_const_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/heritage_subpiece_const_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/heritage_subpiece_const_1204.rs"
...
cpp_binary="$oracle_tmp/heritage_subpiece_const_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "heritage_subpiece_const_1204: GHIDRA_LOCKED_OUTPUT_OK covered=MATCH overall=UNTESTED stdout_sha256=$actual_stdout_sha"
  exit 0
fi

fixture_target=/tmp/rugra-target-heritage-subpiece/heritage-subpiece-const
...
rust_binary="$oracle_tmp/heritage_subpiece_const_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=HERITAGE-SUBPIECE-CONST-1204|"
...
echo "heritage_subpiece_const_1204: OK covered=MATCH overall=UNTESTED ghidra=$actual_stdout_sha"

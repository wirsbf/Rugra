...
runner="$repo_root/tools/run_action_pool_clone_filter_oracle.sh"
...
(metadata_name, cpp_name, rust_name, action_name, doc_name, runner_name,
 oracle_commit, oracle_tag, cpp_tree, makefile_blob, base_commit, base_tree,
 host_cxx, host_rustc, host_cargo, host_platform) = sys.argv[1:]
metadata = json.loads(pathlib.Path(metadata_name).read_text(encoding="utf-8"))
if metadata["fixture_id"] != "ACTIONPOOL-CLONE-FILTER-0001":
...
for field, filename in {
    ...
    "action_rs_sha256": action_name, "action_doc_sha256": doc_name,
...
if [[ "$ghidra_only" == true ]]; then
  ...
  echo "ACTIONPOOL-CLONE-FILTER-0001 locked Ghidra projection SHA256 $actual_sha" >&2
  exit 0
fi
...
echo "ACTIONPOOL-CLONE-FILTER-0001 B2 covered projection MATCH ($records records) SHA256 $actual_sha"

...
runner="$repo_root/tools/run_block_findirreducible_oracle.sh"
...
metadata="$repo_root/tests/oracle/block_findirreducible_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/block_findirreducible_1204.cc"
rust_fixture="$repo_root/tests/oracle/block_findirreducible_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-block-findirred-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-block-findirred-1204.?????? ]]; then
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, block_raw, block_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/block_findirreducible_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/block_findirreducible_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/block_findirreducible_1204.rs",
...
crate_hasher.update(b"rugra-block-findirred-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/block_findirreducible_1204.cc"),
    pathlib.Path("tests/oracle/block_findirreducible_1204.rs"),
    pathlib.Path("tests/oracle/block_findirreducible_1204.metadata.json"),
    pathlib.Path("docs/api/block.md"),
    pathlib.Path("tools/run_block_findirreducible_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "BLOCK-FINDIRREDUCIBLE-0001")
require(
    "stable function id",
    metadata["stable_function_id"],
    "GH12-F-7bfea0ee3fe8e4d3fdfc",
)
...
require(
    ...
    "sha256 of rugra-block-findirred-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("reducible_zero_marks", "MATCH"),
    ("self_loop_head_exclusion", "MATCH"),
    ("back_edge_pair_promotion_forward", "MATCH"),
    ("parallel_edge_double_count", "MATCH"),
    ("nested_find_reuse_collapse", "MATCH"),
    ("multi_root_cross_promotion", "MATCH"),
    ("structure_loops_driver_wiring", "MATCH"),
    ("copymap_collapse_projection", "MATCH"),
    ("mark_cleared_projection", "MATCH"),
    ("irreducible_tree_edge_rebuild", "UNTESTED"),
    ("is_irreducible_in_skip", "UNTESTED"),
    ("calc_loop_body", "UNTESTED"),
    ("production_pipeline_wiring", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-block-findirred-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/block_findirreducible_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/block_findirreducible_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/block_findirreducible_1204.rs"
...
cpp_binary="$oracle_tmp/block_findirreducible_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "block_findirreducible_1204: GHIDRA_LOCKED_OUTPUT_OK overall=MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/block_findirreducible_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=BLOCK-FINDIRREDUCIBLE-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "reducible_diamond_zero_marks", "self_loop_head_skipped",
    "classic_two_entry", "parallel_edges_double_count",
    "nested_irreducible_find_reuse", "multi_root_cross_to_irreducible",
    "structure_loops_reducible_endtoend",
]
...
metadata = json.loads(
    (repo / "tests/oracle/block_findirreducible_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_block_findirreducible_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/block_findirreducible_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/block_findirreducible_1204.rs").read_bytes()),
...
hasher.update(b"rugra-block-findirred-base-overlay-v1\0")
...
echo "block_findirreducible_1204: MATCH covered_projection=7/7 overall=MATCH stdout_sha256=$actual_stdout_sha"

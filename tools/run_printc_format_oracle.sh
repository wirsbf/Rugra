...
runner="$repo_root/tools/run_printc_format_oracle.sh"
...
cpp_fixture="$repo_root/tests/oracle/printc_format_1204.cc"
rust_fixture="$repo_root/tests/oracle/printc_format_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$printc_rs" "$prettyprint_rs" \
  "$printc_doc" "$prettyprint_doc" \
  "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-printc-format-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-printc-format-1204.?????? ]]; then
...
(
    ...
    metadata_raw, cpp_raw, rust_raw,
    printc_rs_raw, prettyprint_rs_raw, printc_doc_raw, prettyprint_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/printc_format_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/printc_format_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/printc_format_1204.rs",
    pathlib.Path(printc_rs_raw): repo / "src/printc.rs",
    pathlib.Path(prettyprint_rs_raw): repo / "src/prettyprint.rs",
    pathlib.Path(printc_doc_raw): repo / "docs/api/printc.md",
    pathlib.Path(prettyprint_doc_raw): repo / "docs/api/prettyprint.md",
}
...
overlay_files = {
    pathlib.Path("src/printc.rs"),
    pathlib.Path("src/prettyprint.rs"),
}
crate_files = [
...
crate_hasher.update(b"rugra-printc-format-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/printc_format_1204.cc"),
    pathlib.Path("tests/oracle/printc_format_1204.rs"),
    pathlib.Path("tests/oracle/printc_format_1204.metadata.json"),
    pathlib.Path("docs/api/printc.md"),
    pathlib.Path("docs/api/prettyprint.md"),
    pathlib.Path("tools/run_printc_format_oracle.sh"),
]
special = {}
...
observed_hashes = {
    ...
    "printc_rs_sha256": sha(crate_bytes["src/printc.rs"]),
    "prettyprint_rs_sha256": sha(crate_bytes["src/prettyprint.rs"]),
    "printc_doc_sha256": sha(special[special_paths[3].as_posix()]),
    "prettyprint_doc_sha256": sha(special[special_paths[4].as_posix()]),
...
require(
    ...
    "sha256 of rugra-printc-format-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("void_empty_header_brace", "MATCH"),
    ("void_return_body_indent", "MATCH"),
    ("ptr_int_param_join", "MATCH"),
    ("int_char2_param_join", "MATCH"),
    ("dotdotdot_comma_join", "MATCH"),
    ("base_type_param_join", "MATCH"),
    ("local_var_decl_separator", "UNTESTED"),
    ("full_docfunction_object_graph", "UNTESTED"),
    ("markup_emitter_projection", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-printc-format-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/printc_format_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/printc_format_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/printc_format_1204.rs"
...
cpp_binary="$oracle_tmp/printc_format_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$host_cxx_bin" -std=c++11 -O2 -Wall -Wno-sign-compare -m64 \
    -I"$oracle_cpp" "$snapshot_cpp" \
...
if $ghidra_only; then
  ...
  echo "printc_format_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/printc_format_1204_rust"
...
if len(lines) != 6:
    raise SystemExit(f"expected six fixture case lines, found {len(lines)}")
expected_cases = [
    "void_empty",
    "void_return_body",
    "ptr_int_params",
...
actual_cases = [line.split("=", 1)[0] for line in lines]
if actual_cases != expected_cases:
    raise SystemExit(f"fixture case order mismatch: {actual_cases}")
for line in lines:
    if "=" not in line:
        raise SystemExit(f"malformed fixture case line: {line!r}")
PY
...
metadata = json.loads(
    (repo / "tests/oracle/printc_format_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_printc_format_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
for label, relative, key in (
    ("C++ fixture", "tests/oracle/printc_format_1204.cc", "cpp_fixture_sha256"),
    ("Rust fixture", "tests/oracle/printc_format_1204.rs", "rust_fixture_sha256"),
    ("printc implementation", "src/printc.rs", "printc_rs_sha256"),
    ("prettyprint implementation", "src/prettyprint.rs", "prettyprint_rs_sha256"),
    ("printc API document", "docs/api/printc.md", "printc_doc_sha256"),
    ("prettyprint API document", "docs/api/prettyprint.md", "prettyprint_doc_sha256"),
):
    require(label, sha((repo / relative).read_bytes()), comparand[key])

overlay_files = {"src/printc.rs", "src/prettyprint.rs"}
...
hasher.update(b"rugra-printc-format-base-overlay-v1\0")
...
echo "printc_format_1204: MATCH covered_projection=6/6 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

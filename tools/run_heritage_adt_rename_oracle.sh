...
runner="$repo_root/tools/run_heritage_adt_rename_oracle.sh"
...
metadata="$repo_root/tests/oracle/heritage_adt_rename_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/heritage_adt_rename_1204.cc"
rust_fixture="$repo_root/tests/oracle/heritage_adt_rename_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-heritage-adt-rename-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-heritage-adt-rename-1204.?????? ]]; then
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, heritage_raw,
    heritage_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/heritage_adt_rename_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/heritage_adt_rename_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/heritage_adt_rename_1204.rs",
...
def reject_pending(value, path="metadata"):
    ...
    elif isinstance(value, str) and value.startswith("REPLACE"):
...
crate_hasher.update(b"rugra-heritage-adt-rename-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/heritage_adt_rename_1204.cc"),
    pathlib.Path("tests/oracle/heritage_adt_rename_1204.rs"),
    pathlib.Path("tests/oracle/heritage_adt_rename_1204.metadata.json"),
    pathlib.Path("docs/api/heritage.md"),
    pathlib.Path("tools/run_heritage_adt_rename_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "HERITAGE-ADT-RENAME-0001")
...
require(
    ...
    "sha256 of rugra-heritage-adt-rename-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("diamond_phi_full_projection", "MATCH"),
    ("phi_block_begin_reverse_creation_order", "MATCH"),
    ("phi_input_identity_reverse_predecessor_slot", "MATCH"),
    ("phi_cycle_old_marker_skip_full_projection", "MATCH"),
    ("merge_order_pq_not_block_index", "MATCH"),
    ("collect_new_old_classification", "MATCH"),
    ("refinement_branch", "MATCH"),
    ("refine_concat_element_anchor_order", "MATCH"),
    ("refine_write_split_element_anchor_order", "MATCH"),
    ("guard_input_concat_path", "UNTESTED"),
    ("remove_revisited_positions", "MATCH"),
    ("refine_read_nonfree", "UNTESTED"),
    ("cross_space_collect_window", "MISMATCH"),
    ("production_action_switch", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-heritage-adt-rename-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/heritage_adt_rename_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/heritage_adt_rename_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/heritage_adt_rename_1204.rs"
...
# build.rs resolves the locked SLEIGH sources at
# <crate>/ghidra/Ghidra/Features/Decompiler/src/decompile/cpp; expose the
# freshly exported oracle subtree there (link into the private temporary
# directory only).
...
cpp_binary="$oracle_tmp/heritage_adt_rename_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if [[ "$ghidra_status" -ne 0 || -s "$oracle_tmp/ghidra.stderr" ]]; then
  echo "Ghidra fixture failed, hung, or emitted runtime diagnostics" >&2
...
if $ghidra_only; then
  ...
  echo "heritage_adt_rename_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/heritage_adt_rename_1204_rust"
...
if [[ "$rugra_status" -ne 0 || -s "$oracle_tmp/rugra.stderr" ]]; then
  echo "Rugra fixture failed, hung, or emitted runtime diagnostics" >&2
...
if lines[0] != (
    "schema=1|fixture=HERITAGE-ADT-RENAME-0001|"
...
for field in ("pass=1,2,3",):
    for line in lines[1:]:
        if field not in line:
            raise SystemExit(f"fixture line missing {field}: {line}")
# merge-order witness: the two-join case must place the deeper merge
# block's phi FIRST (PriorityQueue/augment order), which a block-index
# ordering would reverse.
if "|seq=6,3|" not in lines[3]:
    raise SystemExit("adt_two_join_order merge-order witness missing or wrong")
# insertion-order witness (mechanism-C M1/M2/M3): the refine case's
# per-op block order must show the concatPieces chain in CREATION order
# before the reader and each refineWrite SUBPIECE group in creation
# order after its write — a fixed numeric-index insertion would reverse
# both groups, and the per-op input descriptors (constant VALUES) make
# the within-group permutation visible.
if not lines[4].startswith("case=adt_refine_order"):
    raise SystemExit("adt_refine_order line misplaced")
if ",phi.PIECE(" not in lines[4] or ",phi.SUBPIECE(" not in lines[4]:
    raise SystemExit("adt_refine_order marker ops missing from block order")
# within-group creation order: the SUBPIECE offset constants must read
# 0 before 2 — a reversed (numeric-index) insertion reads C4:2 first.
s0 = lines[4].find("phi.SUBPIECE(U4:W+INT_ADD,C4:0)")
s2 = lines[4].find("phi.SUBPIECE(U4:W+INT_ADD,C4:2)")
if s0 == -1 or s2 == -1 or s0 > s2:
    raise SystemExit("adt_refine_order SUBPIECE group order witness wrong")
# mechanism-C M5(b) revisit-position witness: element-anchored reinsertion
# after removeRevisitedMarkers' opUninsert — dead-target mid-block
# [.., a2, S, f2], dead-target tail [a3, S], and the MULTIEQUAL group
# [m2, S, x]. A numeric pre-removal index shifts by one ([a2, f2, S] /
# [m2, x, S]) and panics at the block tail.
if "a2.INT_MULT" not in lines[5] or "f2.INT_OR" not in lines[5]:
    raise SystemExit("adt_revisit_positions block markers missing")
if lines[5].find("a2.INT_MULT") > lines[5].find("i.SUBPIECE"):
    raise SystemExit("adt_revisit_positions: SUBPIECE must follow a2 (dead-target mid-block anchor)")
if lines[5].find("i.SUBPIECE") > lines[5].find("f2.INT_OR"):
    raise SystemExit("adt_revisit_positions: SUBPIECE must precede f2 (element anchor, not append)")
if lines[5].find("m2.MULTIEQUAL") > lines[5].find("m.SUBPIECE") or lines[5].find("m.SUBPIECE") > lines[5].find("x.INT_MULT"):
    raise SystemExit("adt_revisit_positions: MULTIEQUAL-group anchor must give [m2, S, x]")
PY
...
metadata = json.loads(
    (repo / "tests/oracle/heritage_adt_rename_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_heritage_adt_rename_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
for label, relative, key in (
    ("C++ fixture", "tests/oracle/heritage_adt_rename_1204.cc", "cpp_fixture_sha256"),
    ("Rust fixture", "tests/oracle/heritage_adt_rename_1204.rs", "rust_fixture_sha256"),
...
hasher.update(b"rugra-heritage-adt-rename-base-overlay-v1\0")
...
echo "heritage_adt_rename_1204: MATCH covered_projection=5/5 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

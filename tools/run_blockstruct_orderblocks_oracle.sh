#!/usr/bin/env bash
# BLOCKSTRUCT-ORDERBLOCKS-0001 bilateral runner.
#
# Builds the locked Ghidra 12.0.4 oracle fixture (BlockGraph::orderBlocks /
# FlowBlock::compareFinalOrder, block.hh:430 / block.cc:709) against the
# oracle cpp tree, builds the Rugra comparand against the crate rlib, runs
# both, and diffs the shared post-sort projection.
set -euo pipefail
...
cpp_fixture="$repo_root/tests/oracle/blockstruct_orderblocks_1204.cc"
rust_fixture="$repo_root/tests/oracle/blockstruct_orderblocks_1204.rs"
metadata="$repo_root/tests/oracle/blockstruct_orderblocks_1204.metadata.json"
...
oracle_tmp=$(mktemp -d /tmp/rugra-blockstruct-orderblocks-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-blockstruct-orderblocks-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
g++ -std=c++11 -O2 -I"$cpp_root" -I"$bfd_root/usr/include" \
  ...
  -o "$oracle_tmp/blockstruct_orderblocks_1204"
...
rustc --edition=2021 -O -L "dependency=$fixture_target/debug/deps" \
  ...
  -o "$oracle_tmp/blockstruct_orderblocks_rugra"

"$oracle_tmp/blockstruct_orderblocks_1204" >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
"$oracle_tmp/blockstruct_orderblocks_rugra" >"$oracle_tmp/rugra.stdout" 2>"$oracle_tmp/rugra.stderr"

if diff -u "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout" \
    >"$oracle_tmp/bilateral.diff"; then
  cat "$oracle_tmp/ghidra.stdout"
  printf 'blockstruct_orderblocks_1204: MATCH\n'
else
  cat "$oracle_tmp/bilateral.diff"
  printf 'blockstruct_orderblocks_1204: MISMATCH (%s diff lines)\n' \

...
# RETURNFOLD-GAPB-CONDCONST-0001 locked Ghidra 12.0.4/Rugra bilateral runner.
# The Rust side is built from the frozen GAP-B production commit (c2a4a652,
# propagateConstant CPUI_RETURN copyBeforeRet arm) via a partial git archive,
# never from the live crate; the fixture trio is copied from the live tree
...
runner="$repo_root/tools/run_returnfold_gapb_oracle.sh"
...
cache_root="$cache_parent/rugra-returnfold-gapb-1204"
# Task RETURNFOLD-GAPB-CONDCONST-0001 dedicated Cargo dirs: every Cargo
...
cargo_target=/home/wirs/.cache/a67-gapb-target
cargo_tmp=/home/wirs/.cache/a67-gapb-tmp
...
rugra_base_commit=c2a4a652566c361ff97a34e222ecff7f336588e2
rugra_base_tree=0a331f5f17dfa149811067cbd5ec81171f92b9c3
rugra_base_src_tree=3b8cfb862c0e5283c0c70bc33d0a97b699a586ff
...
rugra_expected_records=13
rugra_expected_bytes=1895
rugra_expected_stdout_sha256=112c47dae77b59227da160b6e2f44933e21d71b1b768873aa362384d69eef73c
...
metadata_live="$repo_root/tests/oracle/returnfold_gapb_1204.metadata.json"
cpp_fixture_live="$repo_root/tests/oracle/returnfold_gapb_1204.cc"
rust_fixture_live="$repo_root/tests/oracle/returnfold_gapb_1204.rs"

# GAP-B snapshot model: the frozen production commit already carries the
# propagateConstant CPUI_RETURN arm, so the overlay table is empty.
overlay_paths=()
archive_paths=(
  Cargo.toml
  Cargo.lock
...
/usr/bin/cp "$cpp_fixture_live" "$snapshot_root/tests/oracle/returnfold_gapb_1204.cc"
/usr/bin/cp "$rust_fixture_live" "$snapshot_root/tests/oracle/returnfold_gapb_1204.rs"
/usr/bin/cp "$metadata_live" "$snapshot_root/tests/oracle/returnfold_gapb_1204.metadata.json"
...
require("fixture id", metadata["fixture_id"], "RETURNFOLD-GAPB-CONDCONST-0001")
...
require(
    ...
    "immutable partial GAP-B crate snapshot: git archive of commit c2a4a652 carrying the propagateConstant CPUI_RETURN copyBeforeRet arm; no source overlays",
)
require(
    "archive paths",
...
require("apply return residual status", coverage["apply_return_value"]["status"], "UNTESTED")
require("phi arm status", coverage["phi_arm"]["status"], "UNTESTED")
require("implied bool arm status", coverage["implied_bool_arm"]["status"], "UNTESTED")
require("print fold status", coverage["print_fold"]["status"], "UNTESTED")
for name, entry in coverage.items():
...
if [[ "$validate_only" -eq 1 ]]; then
  echo "returnfold_gapb_1204 metadata/source lock validation passed"
  exit 0
fi
...
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C TMPDIR="$cache_root/tmp" \
  ...
  -I"$oracle_cpp" \
  "$snapshot_root/tests/oracle/returnfold_gapb_1204.cc" \
...
if ! /usr/bin/env -i PATH="$clean_path" HOME="$user_home" LC_ALL=C \
  ...
  "$snapshot_root/tests/oracle/returnfold_gapb_1204.rs" \
...
print(
    "returnfold_gapb_1204: oracle_status=ORACLE_CAPTURED "
    f"rugra_status=EXECUTED bilateral_status={status} "
    f"overall_status={metadata['overall_status']} "
    "scope=condconst CPUI_RETURN copyBeforeRet arm"
)
PY

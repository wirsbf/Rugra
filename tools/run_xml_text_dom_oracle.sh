...
# XML text ingestion oracle runner (MARSHAL-XML-TEXT-0001).
# Builds the locked Ghidra 12.0.4 libdecomp oracle and an isolated
# base-plus-overlay Rust snapshot, runs both xml_text_dom_1204 fixtures on
# identical inputs, and diffs the canonical DOM/error projections.
...
runner="$repo_root/tools/run_xml_text_dom_oracle.sh"
...
rugra_base_tree=288a5dc242465923b647a445721c9d4ba5a538ee
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/xml_text_dom_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/xml_text_dom_1204.cc"
rust_fixture="$repo_root/tests/oracle/xml_text_dom_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-xml-text-dom-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-xml-text-dom-1204.??????) /usr/bin/rm -rf -- "$oracle_tmp" ;;
...
/usr/bin/cp -- "$cpp_fixture" "$snapshot_root/tests/oracle/xml_text_dom_1204.cc"
/usr/bin/cp -- "$rust_fixture" "$snapshot_root/tests/oracle/xml_text_dom_1204.rs"
/usr/bin/cp -- "$metadata" "$snapshot_root/tests/oracle/xml_text_dom_1204.metadata.json"
/usr/bin/cp -- "$runner_fd_path" "$snapshot_root/tools/run_xml_text_dom_oracle.sh"
...
metadata_path = snapshot / "tests/oracle/xml_text_dom_1204.metadata.json"
metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
reject_pending(metadata)
require("metadata schema", metadata["schema_version"], 1)
require("fixture id", metadata["fixture_id"], "MARSHAL-XML-TEXT-0001")
...
require("architecture", metadata["architecture"], "process-global xml text ingestion")
require("compiler spec", metadata["compiler_spec"], "N/A")
require(
    "dom observation status",
    metadata["coverage"]["dom_projection_status"], "MATCH")
require(
    "storage observation status",
    metadata["coverage"]["document_storage_status"], "MATCH")
require(
    "error observation status",
    metadata["coverage"]["error_message_status"], "MATCH")
require(
    "residual statuses",
...
require(
    ...
    [
        ...
        "tests/oracle/xml_text_dom_1204.cc",
        "tests/oracle/xml_text_dom_1204.rs",
        "tests/oracle/xml_text_dom_1204.metadata.json",
        "tools/run_xml_text_dom_oracle.sh",
    ],
)

paths = {
    "cpp_fixture_sha256": snapshot / "tests/oracle/xml_text_dom_1204.cc",
    "rust_fixture_sha256": snapshot / "tests/oracle/xml_text_dom_1204.rs",
    "marshal_rs_sha256": snapshot / "src/marshal.rs",
    "marshal_doc_sha256": snapshot / "docs/api/marshal.md",
    "runner_sha256": snapshot / "tools/run_xml_text_dom_oracle.sh",
...
cpp_fixture="$snapshot_root/tests/oracle/xml_text_dom_1204.cc"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_cxx_bin" \
  ...
  -o "$oracle_tmp/xml_text_dom_1204_cpp" \
...
rust_fixture="$snapshot_root/tests/oracle/xml_text_dom_1204.rs"
if ! /usr/bin/env -i HOME="$user_home" RUSTUP_HOME="$user_home/.rustup" \
  ...
  "$rust_fixture" -o "$oracle_tmp/xml_text_dom_1204_rust" \
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/xml_text_dom_1204_cpp" >"$oracle_tmp/ghidra.stdout"
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/xml_text_dom_1204_rust" >"$oracle_tmp/rugra.stdout"
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  /usr/bin/diff -u "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout"

/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S \
  - "$snapshot_root/tests/oracle/xml_text_dom_1204.metadata.json" \
...
print("dom=MATCH storage=MATCH errors=MATCH overall=MATCH")
PY

...
rugra_source_commit=32a136de1327e863799f75f1c0c833f51cf1b60f
rugra_source_tree=829e64782d43b8b192533877d1ad7cc2176c016b
rugra_source_src_tree=1fb9e4fa6c609fc1f54a3fc38ed3a39379768963
rugra_source_jumptable_blob=deb1b0009ea7f648a66f8c40e13edfd9de4ef8e1
...
metadata="$repo_root/tests/oracle/jt_guards_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/jt_guards_1204.cc"
...
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-jt-guards-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
cp "$cpp_fixture" "$snapshot_root/tests/oracle/jt_guards_1204.cc"
cp "$rust_fixture" "$snapshot_root/tests/oracle/jt_guards_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/jt_guards_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_jt_guards_oracle.sh"
...
require("fixture", metadata["fixture_id"], "JT-GUARDS-1204")
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/jt_guards_1204.cc" \
...
python3 -I -S - "$snapshot_root/tests/oracle/jt_guards_1204.metadata.json" \
...
if [record.split("|", 1)[0] for record in records] != [
    "sc1_chain_range", "sc2_unrolled", "sc3_neg_const",
    "sc4_other_switch", "sc5_pathout", "sc6_quasi_vm",
]:
...
printf 'jt_guards_1204: covered=6/6 projection_status=%s overall_status=%s residuals=%s\n' \
  ...
  "$(python3 -c "import json;print(','.join(r['todo_id'] for r in json.load(open('$metadata'))['residuals']))")"

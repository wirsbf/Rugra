...
rust_fixture="$repo_root/tests/oracle/stackstall_count_1204.rs"
...
require("fixture", metadata["fixture_id"], "PIPE-STACKSTALL-COUNT-0001")
...
require("coverage fixed point projection", coverage["stackstall_fixed_point_projection"]["status"], "MATCH")
...
cp "$rust_fixture" "$snapshot_root/tests/oracle/stackstall_count_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/stackstall_count_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_stackstall_count_oracle.sh"
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare -m64 \
  -I"$oracle_cpp" "$snapshot_root/tests/oracle/stackstall_count_1204.cc" \
  ...
  -o "$oracle_tmp/stackstall_count_1204_cpp"
...
set +e
"$oracle_tmp/stackstall_count_1204_cpp" \
...
if len(lines) != 34:
    raise SystemExit(f"expected 34 fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=PIPE-STACKSTALL-COUNT-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
required = (
    "run=1|pass=1|child=multicse|status=1|count=1|lcount=0|tests=1|apply=1",
    "run=1|pass=1|child=shadowvar|status=1|count=1|lcount=0|tests=1|apply=1",
    "run=1|pass=1|child=stackptrflow|status=1|count=1|lcount=0|tests=1|apply=1",
    "run=1|converged_passes=3|final_count=4|tests=1|apply=2",
    "run=2|pass=1|child=stackptrflow|status=1|count=0|lcount=0|tests=4|apply=1",
    "run=2|converged_passes=1|final_count=0|tests=2|apply=2",
)
for line in required:
    if line not in lines:
        raise SystemExit(f"missing required observation: {line}")
for marker in ("m2=COPY", "ld=COPY", "m4=BUILD/16777218,dead=1"):
...
printf 'stackstall_count_1204: stackstall_fixed_point_projection=MATCH overall=MATCH passes=3+1 stdout_sha256=%s\n' \

...
rugra_source_commit=5dc86bacaf959cf9921ad9b170494bb652c4a12f
rugra_source_tree=e721a81955ad102d6b34d01a9e4df31cc2589b1f
rugra_source_src_tree=a6d4ca9fe7d4688101153815233aa7827dffe7b4
rugra_source_transform_blob=42dea35c14354256fc7df864eb25aceac7201cb9
...
metadata="$repo_root/tests/oracle/transform_multiequal_insert_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/transform_multiequal_insert_1204.cc"
rust_fixture="$repo_root/tests/oracle/transform_multiequal_insert_1204.rs"
rust_overlay="$repo_root/src/transform.rs"
runner="$repo_root/tools/run_transform_multiequal_insert_oracle.sh"
bfd_include=/tmp/rugra-ghidra-bfd-2.38/usr/include
bfd_header="$bfd_include/bfd.h"
bfd_library=/usr/lib/x86_64-linux-gnu/libbfd-2.38-system.so

oracle_tmp=$(mktemp -d /tmp/rugra-transform-multiequal-insert-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-transform-multiequal-insert-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
cp "$rust_overlay" "$snapshot_root/src/transform.rs"
cp "$cpp_fixture" "$snapshot_root/tests/oracle/transform_multiequal_insert_1204.cc"
cp "$rust_fixture" "$snapshot_root/tests/oracle/transform_multiequal_insert_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/transform_multiequal_insert_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_transform_multiequal_insert_oracle.sh"
...
python3 -I -S - "$repo_root" "$snapshot_root" "$metadata" "$cpp_fixture" \
  ...
  "$rugra_source_src_tree" "$rugra_source_transform_blob" \
...
(
    ...
    source_transform_blob, source_cargo_toml_blob, source_cargo_lock_blob,
...
require("fixture", metadata["fixture_id"], "TRANSFORM-MULTIEQUAL-INSERT-0001")
...
for key, path in (
    ...
    ("transform_overlay_sha256", pathlib.Path(overlay_raw)),
):
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/transform_multiequal_insert_1204.cc" \
  ...
  "$bfd_library" -lz -o "$oracle_tmp/transform_multiequal_insert_1204_cpp"

CARGO_TARGET_DIR="$oracle_tmp/cargo-target" \
  cargo build --offline --locked --quiet --manifest-path "$snapshot_root/Cargo.toml" --lib
rustc --edition=2021 "$snapshot_root/tests/oracle/transform_multiequal_insert_1204.rs" \
  --extern rugra="$oracle_tmp/cargo-target/debug/librugra.rlib" \
  -L "dependency=$oracle_tmp/cargo-target/debug/deps" \
  -o "$oracle_tmp/transform_multiequal_insert_1204_rust"

set +e
"$oracle_tmp/transform_multiequal_insert_1204_cpp" \
  "$snapshot_root/sleigh_specs" "$snapshot_root/examples/curl" \
  >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
ghidra_status=$?
"$oracle_tmp/transform_multiequal_insert_1204_rust" \
...
python3 -I -S - "$snapshot_root/tests/oracle/transform_multiequal_insert_1204.metadata.json" \

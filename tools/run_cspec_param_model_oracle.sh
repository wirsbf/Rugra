...
cpp_fixture="$repo_root/tests/oracle/cspec_param_model_1204.cc"
rust_fixture="$repo_root/tests/oracle/cspec_param_model_1204.rs"
metadata="$repo_root/tests/oracle/cspec_param_model_1204.metadata.json"
runner="$repo_root/tools/run_cspec_param_model_oracle.sh"
...
for required in "$cpp_fixture" "$rust_fixture" "$metadata" "$runner" \
  "$fspec_rs" "$arch_rs" "$bfd_include/bfd.h" "$bfd_library" "$oracle_archive"; do
...
python3 -I - "$metadata" "$cpp_fixture" "$rust_fixture" "$runner" \
  "$fspec_rs" "$arch_rs" "$oracle_commit" <<'PY'
import hashlib
import json
import pathlib
import sys
...
keys = [
    ...
    "fspec_rs_sha256", "arch_rs_sha256",
]
...
oracle_tmp=$(mktemp -d /tmp/rugra-cspec-param-model-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-cspec-param-model-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
python3 -I - "$metadata" "$repo_root" "$rugra_base_commit" "$spec_root" "$binary" \
  ...
  "$rugra_overlay/src/fspec.rs" "$rugra_overlay/src/arch.rs" <<'PY'
import hashlib
import json
import pathlib
import subprocess
...
overlay_paths = [pathlib.Path(value) for value in sys.argv[6:11]]
...
if ! g++ -std=c++11 -O0 -Wall -Wno-sign-compare -m64 \
  ...
  "$bfd_library" -lz -o "$oracle_tmp/cspec_param_model_1204_cpp" \
...
"$oracle_tmp/cspec_param_model_1204_cpp" "$spec_root" "$binary" \
...
if document.get("schema") != 1 or document.get("fixture") != "CSPEC-PARAMMODEL-0001":
    raise SystemExit("invalid fixture envelope")
if document.get("source", {}).get("root") != "compiler_spec":
    raise SystemExit("fixture did not observe compiler_spec DOM")
for section in ("default_input", "msabi_input"):
    observed = document.get(section, {})
    if observed.get("entry_count") != len(observed.get("entries", [])):
        raise SystemExit(f"{section} entry-count mismatch")
if document.get("model", {}).get("default_map_identity") != 1:
    raise SystemExit("default model does not alias its architecture map entry")
identity = document.get("model", {}).get("stable_identity", {})
expected_identity = {
    "decode_return_map": 1,
    "decode_return_default": 1,
    "old_printed_after_select": 1,
    "decoded_printed_when_default": 0,
    "decoded_printed_after_restore": 1,
    "old_printed_after_restore": 0,
    "restored_default_identity": 1,
}
if identity != expected_identity:
    raise SystemExit(f"stable model identity observation mismatch: {identity}")
...
if $ghidra_only; then
  ...
  printf 'cspec_param_model_1204: GHIDRA_LOCKED_OUTPUT_OK overall=MISMATCH\n'
  exit 0
fi
...
rustc --edition=2021 -O -L "dependency=$cargo_target/debug/deps" \
  ...
  "$rust_fixture" -o "$oracle_tmp/cspec_param_model_1204_rust"

"$oracle_tmp/cspec_param_model_1204_rust" \
...
cat "$oracle_tmp/ghidra.stdout"
printf 'cspec_param_model_1204: DECLARED_OBSERVATIONS_MATCH overall=MISMATCH\n'

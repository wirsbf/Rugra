...
# RETURNFOLD-GAPA-PROTOTYPES-0001 locked Ghidra 12.0.4/Rugra bilateral runner.
# The Rust side is built from the frozen GAP-A production commit (181aa4c8,
# ActionPrototypeTypes output-locked direct-attach branch) via a partial git
# archive, never from the live crate; the fixture trio is copied from the live
...
runner="$repo_root/tools/run_returnfold_gapa_oracle.sh"
...
cache_root="$cache_parent/rugra-returnfold-gapa-1204"
# Task RETURNFOLD-GAPA-PROTOTYPES-0001 dedicated Cargo dirs: every Cargo
...
cargo_target=/home/wirs/.cache/returnfold-gapa-target
cargo_tmp=/home/wirs/.cache/returnfold-gapa-tmp
...
rugra_base_commit=181aa4c8de2868a898fd53be9cf6225627d744ae
rugra_base_tree=a5cf5a0297a6e0c553104b15a8f03068d1459923
rugra_base_src_tree=448359cd1cadb1e57006324c8b3c69a10a2a425c
...
rugra_expected_records=14
rugra_expected_bytes=1233
rugra_expected_stdout_sha256=81560905f90021f284c291246b93aabade041db15e0e2f123a7aec529ae93c6b
...
metadata_live="$repo_root/tests/oracle/returnfold_gapa_1204.metadata.json"
cpp_fixture_live="$repo_root/tests/oracle/returnfold_gapa_1204.cc"
rust_fixture_live="$repo_root/tests/oracle/returnfold_gapa_1204.rs"

# GAP-A snapshot model: the frozen production commit already carries the
# ActionPrototypeTypes output-locked attach arm, so the overlay table is empty.
overlay_paths=()
archive_paths=(
  Cargo.toml
  Cargo.lock
...
/usr/bin/cp "$cpp_fixture_live" "$snapshot_root/tests/oracle/returnfold_gapa_1204.cc"
/usr/bin/cp "$rust_fixture_live" "$snapshot_root/tests/oracle/returnfold_gapa_1204.rs"
/usr/bin/cp "$metadata_live" "$snapshot_root/tests/oracle/returnfold_gapa_1204.metadata.json"
...
require("fixture id", metadata["fixture_id"], "RETURNFOLD-GAPA-PROTOTYPES-0001")
...
require(
    ...
    "immutable partial GAP-A crate snapshot: git archive of commit 181aa4c8 carrying the ActionPrototypeTypes output-locked attach arm; no source overlays",
)
require(
    "archive paths",
...
require("model glue status", coverage["model_glue"]["status"], "UNTESTED")
require("multi output status", coverage["multi_output"]["status"], "UNTESTED")
require("e2e fold status", coverage["e2e_fold"]["status"], "UNTESTED")
for name, entry in coverage.items():
...
if [[ "$validate_only" -eq 1 ]]; then
  echo "returnfold_gapa_1204 metadata/source lock validation passed"
  exit 0
fi
...
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C TMPDIR="$cache_root/tmp" \
  ...
  -I"$oracle_cpp" \
  "$snapshot_root/tests/oracle/returnfold_gapa_1204.cc" \
...
if ! /usr/bin/env -i PATH="$clean_path" HOME="$user_home" LC_ALL=C \
  ...
  "$snapshot_root/tests/oracle/returnfold_gapa_1204.rs" \
...
print(
    "returnfold_gapa_1204: oracle_status=ORACLE_CAPTURED "
    f"rugra_status=EXECUTED bilateral_status={status} "
    f"overall_status={metadata['overall_status']} "
    "scope=prototypetypes output-locked direct-attach arm"
)
PY

...
runner="$repo_root/tools/run_comment_sorter_iterators_oracle.sh"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-comment-sorter-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-comment-sorter-1204.??????) /usr/bin/rm -rf -- "$oracle_tmp" ;;
...
metadata="$oracle_tmp/candidate/comment_sorter_iterators_1204.metadata.json"
...
for case_name in (
    "header_walks", "block_walk_interleaving", "prev_op_tail_placement",
    "migrated_backup_op", "empty_block_drain", "no_type_filter_at_setup",
    "excised_without_display_unplaced", "dead_op_throw", "opless_placement",
    "pos_counter_lifecycle",
):
...
/usr/bin/env -i HOME="$resolved_user_home" RUSTUP_HOME="$resolved_user_home/.rustup" \
  ...
  "$rust_fixture" -o "$oracle_tmp/comment_sorter_rust"

/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/comment_sorter_cpp" >"$oracle_tmp/ghidra.stdout" \
  2>"$oracle_tmp/ghidra.stderr"
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/comment_sorter_rust" >"$oracle_tmp/rugra.stdout" \
...
/usr/bin/printf 'comment_sorter_iterators_1204: covered_projection=%s/10 projection_status=MATCH overall_status=MISMATCH\n' \
  "$(/usr/bin/wc -l < "$oracle_tmp/ghidra.stdout")"
/usr/bin/printf 'residuals=COMMENT-SORTER-ITERATORS-0001(block-cover-projection/db-comment-clone/printc-glue-snapshot)\n'

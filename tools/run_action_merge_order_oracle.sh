...
runner="$repo_root/tools/run_action_merge_order_oracle.sh"
...
rust_fixture="$repo_root/tests/oracle/action_merge_order_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$action_rs" "$coreaction_rs" "$action_doc" "$coreaction_doc" \
  "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-action-merge-order-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-action-merge-order-1204.?????? ]]; then
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, action_raw, coreaction_raw,
    action_doc_raw, coreaction_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/action_merge_order_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/action_merge_order_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/action_merge_order_1204.rs",
    pathlib.Path(action_raw): repo / "src/action.rs",
    pathlib.Path(coreaction_raw): repo / "src/coreaction.rs",
    pathlib.Path(action_doc_raw): repo / "docs/api/action.md",
...
crate_hasher.update(b"rugra-action-merge-order-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/action_merge_order_1204.cc"),
    pathlib.Path("tests/oracle/action_merge_order_1204.rs"),
...
require("fixture id", metadata["fixture_id"], "PIPE-MERGETYPE-ORDER-0001")
require(
    "stable function id",
...
require(
    ...
    "sha256 of rugra-action-merge-order-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
payload = {
    ...
    "input_manifest": metadata["input_manifest"]["cases"],
}
...
for required_key, required_prefix in (
    ("post_cleanup_order", "MATCH"),
    ("single_late_mergetype", "MATCH"),
...
registry_hasher.update(b"rugra-action-merge-order-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/action_merge_order_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/action_merge_order_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/action_merge_order_1204.rs"
...
cpp_binary="$oracle_tmp/action_merge_order_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
rust_binary="$oracle_tmp/action_merge_order_1204_rust"
...
if actual != expected_sha:
    raise SystemExit(f"Ghidra stdout mismatch: expected={expected_sha} actual={actual}")
for label, status in (
...
if len(lines) != 29:
    raise SystemExit(f"expected 29 fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=PIPE-MERGETYPE-ORDER-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_seq = ",".join([
    "prefercomplement", "structuretransform", "normalizebranches", "assignhigh",
    "mergerequired", "markexplicit", "markimplied", "mergemultientry",
    "mergecopy", "dominantcopy", "dynamicsymbols", "markindirectonly",
    "mergeadjacent", "mergetype", "hideshadow", "copymarker",
    "outputprototype", "inputprototype", "mapglobals", "dynamicsymbols",
    "namevars", "setcasts", "finalstructure", "prototypewarnings", "stop",
])
if not lines[1].startswith("seq=") or lines[1][4:] != expected_seq:
    raise SystemExit("post-cleanup sequence mismatch against oracle tree")
if not lines[2].startswith("pre|") or not lines[28].startswith("post|"):
    raise SystemExit("IR projection lines missing")
act_names = [line.split("|", 1)[0].removeprefix("act=") for line in lines[3:28]]
if act_names != expected_seq.split(","):
...
metadata = json.loads(
    (repo / "tests/oracle/action_merge_order_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_action_merge_order_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
for label, relative, key in (
    ("C++ fixture", "tests/oracle/action_merge_order_1204.cc", "cpp_fixture_sha256"),
...
hasher.update(b"rugra-action-merge-order-base-overlay-v1\0")

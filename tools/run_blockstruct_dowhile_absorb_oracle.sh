#!/usr/bin/env bash
# BLOCKACTION-DOWHILE-ABSORB-0001 bilateral runner.
#
...
g++ -std=c++11 -O2 -I"$cpp_root" -I"$bfd_root/usr/include" \
  ...
  -o "$oracle_tmp/blockstruct_dowhile_absorb_1204"
...
if diff -u "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout" \
  ...
  printf 'blockstruct_dowhile_absorb_1204: MISMATCH (%s diff lines) — see metadata residual_diffs\n' \

...
cpp_fixture="$repo_root/tests/oracle/printc_switch_emit_1204.cc"
rust_fixture="$repo_root/tests/oracle/printc_switch_emit_1204.rs"
metadata="$repo_root/tests/oracle/printc_switch_emit_1204.metadata.json"
...
oracle_files=(printc.cc printc.hh block.cc block.hh jumptable.cc jumptable.hh op.cc op.hh typeop.cc typeop.hh prettyprint.cc prettyprint.hh)
...
if metadata["observation"]["overall_status"] != "MATCH":
    raise SystemExit("switch emit fixture must stay MATCH on both layers")
...
comparands = {
    ...
    "ghidra_jumptable_cc": cpp_root / "jumptable.cc",
    "ghidra_jumptable_hh": cpp_root / "jumptable.hh",
    "ghidra_op_cc": cpp_root / "op.cc",
    "ghidra_op_hh": cpp_root / "op.hh",
    "ghidra_typeop_cc": cpp_root / "typeop.cc",
    "ghidra_typeop_hh": cpp_root / "typeop.hh",
}
...
oracle_tmp=$(mktemp -d /tmp/rugra-printc-switch-emit-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-printc-switch-emit-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
g++ -std=c++11 -O2 -I"$cpp_root" \
  ...
  -o "$oracle_tmp/printc_switch_emit_1204"
...
rustc --edition=2021 -O -L "dependency=$fixture_target/debug/deps" \
  ...
  -o "$oracle_tmp/printc_switch_emit_rugra"

"$oracle_tmp/printc_switch_emit_1204" >"$oracle_tmp/ghidra.statement"
"$oracle_tmp/printc_switch_emit_rugra" >"$oracle_tmp/rugra.statement"
diff -u "$oracle_tmp/ghidra.statement" "$oracle_tmp/rugra.statement"

"$oracle_tmp/printc_switch_emit_1204" --raw >"$oracle_tmp/ghidra.raw"
"$oracle_tmp/printc_switch_emit_rugra" --raw >"$oracle_tmp/rugra.raw"
if ! cmp -s "$oracle_tmp/ghidra.raw" "$oracle_tmp/rugra.raw"; then
  echo "raw PrintC switch output mismatch; both layers must MATCH" >&2
...
printf 'printc_switch_emit_1204: MATCH statement-selection; MATCH raw bytes; overall=MATCH\n'

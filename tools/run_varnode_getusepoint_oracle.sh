...
runner="$repo_root/tools/run_varnode_getusepoint_oracle.sh"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-varnode-getusepoint-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-varnode-getusepoint-1204.??????) /usr/bin/rm -rf -- "$oracle_tmp" ;;
...
for candidate in \
  tests/oracle/varnode_getusepoint_1204.metadata.json \
  tests/oracle/varnode_getusepoint_1204.cc \
  tests/oracle/varnode_getusepoint_1204.rs \
  src/varnode.rs Cargo.toml Cargo.lock build.rs; do
...
metadata="$oracle_tmp/candidate/varnode_getusepoint_1204.metadata.json"
cpp_fixture="$oracle_tmp/candidate/varnode_getusepoint_1204.cc"
rust_fixture="$oracle_tmp/candidate/varnode_getusepoint_1204.rs"
varnode_source="$oracle_tmp/candidate/varnode.rs"
...
/usr/bin/cp -- "$varnode_source" "$oracle_tmp/rugra/src/varnode.rs"
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S - \
  "$metadata" "$cpp_fixture" "$rust_fixture" "$varnode_source" \
...
require("fixture id", metadata["fixture_id"], "VARNODE-GETUSEPOINT-0001")
...
for case_name in ("written_def_leg", "input_param_sentinel",
                  "free_vn_sentinel", "zero_base_wrap"):
...
paths = {
    ...
    "varnode_rs_sha256": pathlib.Path(source_name),
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_cxx_bin" \
  ...
  -o "$oracle_tmp/varnode_getusepoint_cpp"
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C.UTF-8 \
  ...
  "$rust_fixture" -o "$oracle_tmp/varnode_getusepoint_rust"

/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/varnode_getusepoint_cpp" >"$oracle_tmp/ghidra.stdout" \
  2>"$oracle_tmp/ghidra.stderr"
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/varnode_getusepoint_rust" >"$oracle_tmp/rugra.stdout" \
...
print(f"covered_projection={len(ghidra_out.decode().splitlines())}/4 lines byte-identical")
PYVERDICT

/usr/bin/cat "$oracle_tmp/ghidra.stdout"
/usr/bin/printf 'varnode_getusepoint_1204: covered_projection=4/4 projection_status=MATCH overall_status=MATCH\n'

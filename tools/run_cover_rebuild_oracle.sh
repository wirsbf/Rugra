...
runner="$repo_root/tools/run_cover_rebuild_oracle.sh"
...
rust_fixture="$repo_root/tests/oracle/cover_rebuild_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$cover_rs" "$varnode_rs" "$merge_rs" "$cover_doc" "$varnode_doc" \
  "$merge_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-cover-rebuild-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-cover-rebuild-1204.?????? ]]; then
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, cover_raw, varnode_raw, merge_raw,
    cover_doc_raw, varnode_doc_raw, merge_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/cover_rebuild_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/cover_rebuild_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/cover_rebuild_1204.rs",
...
crate_hasher.update(b"rugra-cover-rebuild-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/cover_rebuild_1204.cc"),
    pathlib.Path("tests/oracle/cover_rebuild_1204.rs"),
    pathlib.Path("tests/oracle/cover_rebuild_1204.metadata.json"),
    pathlib.Path("docs/api/cover.md"),
    pathlib.Path("docs/api/varnode.md"),
    pathlib.Path("docs/api/merge.md"),
    pathlib.Path("tools/run_cover_rebuild_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "COVER-REBUILD-SELFLOCK-0001")
require(
    "stable function id",
...
observed_hashes = {
    ...
    "varnode_doc_sha256": sha(special[special_paths[4].as_posix()]),
    "merge_doc_sha256": sha(special[special_paths[5].as_posix()]),
...
require(
    ...
    "sha256 of rugra-cover-rebuild-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("multiequal_slot2_self_reference_double_slot", "MATCH"),
    ("implied_output_worklist_extension", "MATCH"),
...
registry_hasher.update(b"rugra-cover-rebuild-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/cover_rebuild_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/cover_rebuild_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/cover_rebuild_1204.rs"
...
cpp_binary="$oracle_tmp/cover_rebuild_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "cover_rebuild_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/cover_rebuild_1204_rust"
...
metadata = json.loads(
    (repo / "tests/oracle/cover_rebuild_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_cover_rebuild_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/cover_rebuild_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/cover_rebuild_1204.rs").read_bytes()),
...
overlay_files = {"src/cover.rs", "src/varnode.rs", "src/merge.rs"}
...
hasher.update(b"rugra-cover-rebuild-base-overlay-v1\0")

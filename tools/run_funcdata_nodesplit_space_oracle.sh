...
cpp_fixture="$repo_root/tests/oracle/funcdata_nodesplit_space_1204.cc"
rust_fixture="$repo_root/tests/oracle/funcdata_nodesplit_space_1204.rs"
metadata="$repo_root/tests/oracle/funcdata_nodesplit_space_1204.metadata.json"
...
g++ -std=c++11 -O1 -I"$cpp_root" "$cpp_fixture" \
  ...
  "$cpp_root/libdecomp.a" -lz -o "$oracle_tmp/funcdata_nodesplit_space_1204"

CARGO_TARGET_DIR="$oracle_tmp/cargo-target" \
  cargo build --manifest-path "$repo_root/Cargo.toml" --locked --offline --quiet \
  --example funcdata_nodesplit_space_1204_oracle
...
echo "funcdata_nodesplit_space_1204: MATCH"

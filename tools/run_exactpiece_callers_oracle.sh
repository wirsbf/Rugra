...
#   rule*     RulePieceStructure::applyOp leaf   ruleaction.cc:7665
set -euo pipefail
...
runner="$repo_root/tools/run_exactpiece_callers_oracle.sh"
...
metadata="$repo_root/tests/oracle/exactpiece_callers_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/exactpiece_callers_1204.cc"
rust_fixture="$repo_root/tests/oracle/exactpiece_callers_1204.rs"
...
live = {
    ...
    "variable_rs_sha256": repo / "src/variable.rs",
    "database_rs_sha256": repo / "src/database.rs",
    "funcdata_rs_sha256": repo / "src/funcdata.rs",
    "ruleaction_rs_sha256": repo / "src/ruleaction.rs",
    "coreaction_rs_sha256": repo / "src/coreaction.rs",
    "variable_doc_sha256": repo / "docs/api/variable.md",
    "database_doc_sha256": repo / "docs/api/database.md",
    "funcdata_doc_sha256": repo / "docs/api/funcdata.md",
    "ruleaction_doc_sha256": repo / "docs/api/ruleaction.md",
    "coreaction_doc_sha256": repo / "docs/api/coreaction.md",
...
expected_critical = {
    ...
    "src/ruleaction.rs", "src/coreaction.rs",
...
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C TMPDIR="$compiler_tmp" "$host_cxx" \
  ...
  -Wl,--no-whole-archive -lz -o "$oracle_tmp/exactpiece_cpp" \
...
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/exactpiece_cpp" >"$oracle_tmp/ghidra.stdout" \
...
if [[ "$diff_status" -ne 0 ]]; then
  echo "expected byte-identical exact-piece caller observations, diff exit=$diff_status" >&2
...
/usr/bin/printf 'exactpiece_callers_1204: MATCH records=20 entry=8 finalize=5 sync=5 rule=2 oracle=%s candidate=%s\n' \

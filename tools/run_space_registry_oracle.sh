...
runner="$repo_root/tools/run_space_registry_oracle.sh"
...
rugra_base_commit=c6f5c48a8bbeef672bf9e9110389e822b3300db5
...
rust_fixture="$repo_root/tests/oracle/space_registry_1204.rs"
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/space_registry_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/space_registry_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/space_registry_1204.rs",
...
special_paths = [
    pathlib.Path("tests/oracle/space_registry_1204.cc"),
    pathlib.Path("tests/oracle/space_registry_1204.rs"),
    pathlib.Path("tests/oracle/space_registry_1204.metadata.json"),
    pathlib.Path("docs/api/space.md"),
    pathlib.Path("docs/api/translate.md"),
    pathlib.Path("tools/run_space_registry_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "SPACE-REGISTRY-1204")
require(
    "stable function id",
    metadata["stable_function_id"],
    "GH12-F-0de8bbaa1979f78d01a9",
)
...
for required_key, required_prefix in (
    ("canonical_insert_order", "MATCH"),
    ("rejection_paths", "MATCH"),
    ("hole_fill_iteration", "MATCH"),
    ("shortcut_collision", "MATCH"),
    ("shortcut_z_reuse", "MATCH"),
    ("projection_wordsize_endian_flags", "MATCH"),
    ("spacebase_pointer_bridge", "MATCH"),
    ("copy_spaces_refcount", "MATCH"),
    ("space_decode_read_printraw", "UNTESTED"),
    ("resolver_and_join_halves", "UNTESTED"),
...
snapshot_metadata="$snapshot_root/tests/oracle/space_registry_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/space_registry_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/space_registry_1204.rs"
...
cpp_binary="$oracle_tmp/space_registry_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "space_registry_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/space_registry_1204_rust"
...
if len(lines) != 89:
    raise SystemExit(f"expected eighty-nine fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=SPACE-REGISTRY-1204|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "canonical_insert_order", "rejection_paths", "hole_fill_iteration",
    "shortcut_collision", "shortcut_z_reuse",
    "projection_wordsize_endian_flags", "spacebase_pointer_bridge",
    "copy_spaces_refcount",
]
...
require(
    "live runner",
    sha((repo / "tools/run_space_registry_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/space_registry_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/space_registry_1204.rs").read_bytes()),
...
echo "space_registry_1204: MATCH covered_projection=8/8 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

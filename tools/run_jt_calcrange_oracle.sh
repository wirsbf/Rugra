...
rugra_source_commit=f6fd4ea9bcac9309433a98afb1e7a98776b6edf0
rugra_source_tree=fc5743cdbf7913faf7df486efc6d187351492eda
rugra_source_src_tree=2d192788d1fa667255f4fad8d12f377d5b50fa88
rugra_source_jumptable_blob=b921fd5371eca15af0d2950e3494a1037bc60931
...
metadata="$repo_root/tests/oracle/jt_calcrange_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/jt_calcrange_1204.cc"
rust_fixture="$repo_root/tests/oracle/jt_calcrange_1204.rs"
rust_overlay="$repo_root/src/jumptable.rs"
runner="$repo_root/tools/run_jt_calcrange_oracle.sh"
...
oracle_tmp=$(mktemp -d /tmp/rugra-jt-calcrange-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-jt-calcrange-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
cp "$cpp_fixture" "$snapshot_root/tests/oracle/jt_calcrange_1204.cc"
cp "$rust_fixture" "$snapshot_root/tests/oracle/jt_calcrange_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/jt_calcrange_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_jt_calcrange_oracle.sh"
...
(
    ...
    source_jumptable_blob, source_cargo_toml_blob, source_cargo_lock_blob,
...
require("fixture", metadata["fixture_id"], "JT-CALCRANGE-1204")
...
for label, actual, expected in (
    ...
    ("source jumptable blob", source["base_jumptable_blob"], source_jumptable_blob),
...
expected_matches = {
    key for key, record in coverage.items() if record["status"] == "MATCH"
}
expected_mismatches = {
    key for key, record in coverage.items() if record["status"] != "MATCH"
}
require("coverage status set", set(coverage), expected_matches | expected_mismatches)
require(
    "projection/coverage consistency",
    metadata["projection_status"],
    "MATCH" if not expected_mismatches else "MISMATCH",
)
require(
...
known_diffs = metadata.get("known_diffs")
if not isinstance(known_diffs, list):
    raise SystemExit("known_diffs must be a list")

residuals = metadata["residuals"]
if not isinstance(residuals, list):
    raise SystemExit("residuals must be a list")
if residuals and not expected_mismatches:
    raise SystemExit("residuals registered for an all-MATCH fixture")
if not residuals and expected_mismatches:
    raise SystemExit("non-empty residuals required when coverage is not all MATCH")
residual_ids = {residual["todo_id"] for residual in residuals}
require("coverage/residual union", coverage_residual_ids, residual_ids)
for residual in residuals:
    if residual["status"] not in valid_statuses:
        raise SystemExit(f"residual {residual['todo_id']} status is invalid")
    if not residual.get("detail"):
        raise SystemExit(f"residual {residual['todo_id']} has no detail")
    if not residual.get("branches"):
        raise SystemExit(f"residual {residual['todo_id']} has no branches")
    for branch in residual["branches"]:
        require(
            f"residual {residual['todo_id']} branch {branch['id']} status",
            branch["status"],
            "MISMATCH",
        )
        if not branch["detail"]:
            raise SystemExit(f"residual branch {branch['id']} has no detail")

for entry in known_diffs:
    require(
        "known_diffs entry fields",
        set(entry),
        {"expect_line_prefix", "residual_todo_id"},
    )
    if entry["residual_todo_id"] not in residual_ids:
        raise SystemExit(
            f"known_diffs references unregistered residual {entry['residual_todo_id']}"
        )
require(
    "known_diffs prefixes match mismatched coverage lines",
    {entry["expect_line_prefix"] for entry in known_diffs},
    {f"{key}|" for key in expected_mismatches},
)
PY
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/jt_calcrange_1204.cc" \
...
python3 -I -S - "$snapshot_root/tests/oracle/jt_calcrange_1204.metadata.json" \
...
if [record.split("|", 1)[0] for record in records] != [
    "sc1_range_guard", "sc2_constant_input", "sc3_markmodel_skip",
]:
...
if len(explained) != len(known):
    raise SystemExit("a known_diff prefix did not appear in the outputs")
PY

cat "$oracle_tmp/ghidra.stdout"
printf 'jt_calcrange_1204: covered=3/3 projection_status=%s overall_status=%s residuals=%s\n' \
  "$(python3 -c "import json;print(json.load(open('$metadata'))['projection_status'])")" \
  "$(python3 -c "import json;print(json.load(open('$metadata'))['overall_status'])")" \
  "$(python3 -c "import json;print(','.join(r['todo_id'] for r in json.load(open('$metadata'))['residuals']) or 'none')")"

...
runner="$repo_root/tools/run_heritage_tryoutput_oracle.sh"
...
oracle_tmp=$(/usr/bin/mktemp -d "$temp_root/rugra-heritage-tryoutput-1204.XXXXXX")
cleanup() {
  if [[ "$oracle_tmp" != "$temp_root"/rugra-heritage-tryoutput-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/heritage_tryoutput_1204.metadata.json",
...
special_paths = [
    pathlib.Path("tests/oracle/heritage_tryoutput_1204.cc"),
    pathlib.Path("tests/oracle/heritage_tryoutput_1204.rs"),
    pathlib.Path("tests/oracle/heritage_tryoutput_1204.metadata.json"),
    pathlib.Path("docs/api/heritage.md"),
    pathlib.Path("tools/run_heritage_tryoutput_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "HERITAGE-TRYOUTPUT-STACKGUARD-CONTAINS")
...
for required_key, required_prefix in (
    ("output_contains_branch", "MATCH"),
    ("cc1420_truncate_constant", "MATCH"),
    ("missing_outvn_creation", "MATCH"),
...
registry_hasher.update(b"rugra-heritage-tryoutput-registry-lock-v1\0")
...
if $validate_only; then
  echo "heritage_tryoutput_1204 metadata/source lock validation passed"
  exit 0
fi

snapshot_metadata="$snapshot_root/tests/oracle/heritage_tryoutput_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/heritage_tryoutput_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/heritage_tryoutput_1204.rs"
...
cpp_binary="$oracle_tmp/heritage_tryoutput_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" TMPDIR="$temp_root" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "heritage_tryoutput_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/heritage_tryoutput_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=HERITAGE-TRYOUTPUT-STACKGUARD-CONTAINS|"
...
geom_headers = [line for line in lines if line.startswith("to geom=")]
if len(geom_headers) != 6:
    raise SystemExit(f"expected six geometry headers, found {len(geom_headers)}")
for line in geom_headers:
    if "res=1" not in line:
        raise SystemExit(f"every direct drive must return the cc:1430 true: {line}")
    if "occ=" not in line or "retc=1010" not in line:
        raise SystemExit(f"geometry header missing staged classification/translation: {line}")
sp_lines = [line for line in lines if line.startswith("  sp geom=")]
if len(sp_lines) != 6:
    raise SystemExit(f"expected six cc1420_constant rows, found {len(sp_lines)}")
if "  sp geom=0 le=0 be=4" not in lines or "  sp geom=2 le=4 be=0" not in lines:
    raise SystemExit("cc1420_constant must witness both endian routings")
no_op = [line for line in lines if line.startswith("to geom=5")]
if no_op and "write0" in "\n".join(
...
metadata = json.loads(
    (repo / "tests/oracle/heritage_tryoutput_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_heritage_tryoutput_oracle.sh").read_bytes()),

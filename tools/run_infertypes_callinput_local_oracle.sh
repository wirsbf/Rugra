...
# INFERTYPES-CALLINPUT-LOCAL-0001 (TYPEOP-LOCALTYPE-DISPATCH-0001 D2) locked
# Ghidra 12.0.4/Rugra bilateral runner. The Rust side is built from the
# frozen D2 production commit (a95562f, buildLocaltypes CALL/CALLIND input
# seeding via TypeOp local dispatch) via a partial git archive, never from
# the live crate; the fixture trio is copied from the live tree and pinned
# by sha256.
...
runner="$repo_root/tools/run_infertypes_callinput_local_oracle.sh"
...
cache_root="$cache_parent/rugra-infertypes-callinput-1204"
# Task INFERTYPES-CALLINPUT-D2-0001 dedicated Cargo dirs: every Cargo
...
cargo_target=/home/wirs/.cache/a21-d2-target
cargo_tmp=/home/wirs/.cache/a21-d2-tmp
...
rugra_base_commit=a95562f1df660e2a729c626a1e7867d6d090dbb8
rugra_base_tree=f2ea0a35bcd0b42c28720e9083e1b3376adc9b7d
rugra_base_src_tree=f3784cd761de3bca48d1e330362e0941704fa79c
...
rugra_expected_records=5
rugra_expected_bytes=1115
rugra_expected_stdout_sha256=90b68819b4eaca9ff9fb9781b8c417e2bb39ca6da1e8c6ce7f2fe7106ab11f42
...
metadata_live="$repo_root/tests/oracle/infertypes_callinput_local_1204.metadata.json"
cpp_fixture_live="$repo_root/tests/oracle/infertypes_callinput_local_1204.cc"
rust_fixture_live="$repo_root/tests/oracle/infertypes_callinput_local_1204.rs"

# D2 snapshot model: the frozen production commit already carries the
# buildLocaltypes input-dispatch port, so the overlay table is empty.
overlay_paths=()
archive_paths=(
  Cargo.toml
  Cargo.lock
...
/usr/bin/cp "$cpp_fixture_live" "$snapshot_root/tests/oracle/infertypes_callinput_local_1204.cc"
/usr/bin/cp "$rust_fixture_live" "$snapshot_root/tests/oracle/infertypes_callinput_local_1204.rs"
/usr/bin/cp "$metadata_live" "$snapshot_root/tests/oracle/infertypes_callinput_local_1204.metadata.json"
...
require("fixture id", metadata["fixture_id"], "INFERTYPES-CALLINPUT-LOCAL-0001")
...
require(
    ...
    "immutable partial D2 crate snapshot: git archive of the frozen production commit a95562f with the buildLocaltypes CALL/CALLIND input dispatch; no source overlays",
)
require(
    "archive paths",
...
require("case7 stop-up status", coverage["stop_up_three_layer_chain"]["status"], "UNTESTED")
for name, entry in coverage.items():
...
if [[ "$validate_only" -eq 1 ]]; then
  echo "infertypes_callinput_local_1204 metadata/source lock validation passed"
  exit 0
fi
...
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C TMPDIR="$cache_root/tmp" \
  ...
  -I"$oracle_cpp" \
  "$snapshot_root/tests/oracle/infertypes_callinput_local_1204.cc" \
  "$oracle_cpp/libdecomp.cc" "$oracle_cpp/sleigh_arch.cc" \
  "$oracle_cpp/inject_sleigh.cc" "$standard_archive" \
  -lz -o "$run_root/infertypes_callinput_cpp" \
...
if ! /usr/bin/env -i PATH="$clean_path" HOME="$user_home" LC_ALL=C \
  ...
  "$snapshot_root/tests/oracle/infertypes_callinput_local_1204.rs" \
  --extern rugra="$cargo_target/debug/librugra.rlib" \
  -L dependency="$cargo_target/debug/deps" \
  -o "$run_root/infertypes_callinput_rust" \
...
for run in 1 2; do
  /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
    "$run_root/infertypes_callinput_cpp" \
    >"$run_root/ghidra.$run.stdout" 2>"$run_root/ghidra.$run.stderr"
  /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
    "$run_root/infertypes_callinput_rust" \
...
print(
    "infertypes_callinput_local_1204: oracle_status=ORACLE_CAPTURED "
    f"rugra_status=EXECUTED bilateral_status={status} "
    f"overall_status={metadata['overall_status']} "
    "scope=CALL-input seeding path"
)
PY

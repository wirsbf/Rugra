...
runner="$repo_root/tools/run_marshal_packed_join_oracle.sh"
...
rugra_base_commit=e9ba48afbe575c8b923c0babc2e1d965fa23181c
rugra_base_tree=7333dc83c71b1d22c32f908d49d46a4ec2fcec72
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/marshal_packed_join_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/marshal_packed_join_1204.cc"
rust_fixture="$repo_root/tests/oracle/marshal_packed_join_1204.rs"
marshal_rs="$repo_root/src/marshal.rs"
space_rs="$repo_root/src/space.rs"
address_rs="$repo_root/src/address.rs"
marshal_doc="$repo_root/docs/api/marshal.md"
space_doc="$repo_root/docs/api/space.md"
address_doc="$repo_root/docs/api/address.md"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$marshal_rs" "$space_rs" "$address_rs" "$marshal_doc" "$space_doc" \
  "$address_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-marshal-pk-join.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-marshal-pk-join.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C GIT_CONFIG_NOSYSTEM=1 \
  ...
  "$marshal_rs" "$space_rs" "$address_rs" \
  "$marshal_doc" "$space_doc" "$address_doc" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, marshal_raw, space_raw, address_raw,
    marshal_doc_raw, space_doc_raw, address_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/marshal_packed_join_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/marshal_packed_join_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/marshal_packed_join_1204.rs",
    pathlib.Path(marshal_raw): repo / "src/marshal.rs",
    pathlib.Path(space_raw): repo / "src/space.rs",
    pathlib.Path(address_raw): repo / "src/address.rs",
    pathlib.Path(marshal_doc_raw): repo / "docs/api/marshal.md",
    pathlib.Path(space_doc_raw): repo / "docs/api/space.md",
    pathlib.Path(address_doc_raw): repo / "docs/api/address.md",
}
...
overlay_files = {
    pathlib.Path("src/marshal.rs"),
    pathlib.Path("src/space.rs"),
    pathlib.Path("src/address.rs"),
}
crate_files = [
...
crate_hasher.update(b"rugra-marshal-pk-join-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/marshal_packed_join_1204.cc"),
    pathlib.Path("tests/oracle/marshal_packed_join_1204.rs"),
    pathlib.Path("tests/oracle/marshal_packed_join_1204.metadata.json"),
    pathlib.Path("docs/api/marshal.md"),
    pathlib.Path("docs/api/space.md"),
    pathlib.Path("docs/api/address.md"),
    pathlib.Path("tools/run_marshal_packed_join_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "MARSHAL-PACKED-JOIN-1204")
require(
    "stable function id",
    metadata["stable_function_id"],
    "GH12-M-7c31e0a94b2f5d884206",
)
...
observed_hashes = {
    ...
    "marshal_rs_sha256": sha(crate_bytes["src/marshal.rs"]),
    "space_rs_sha256": sha(crate_bytes["src/space.rs"]),
    "address_rs_sha256": sha(crate_bytes["src/address.rs"]),
    "marshal_doc_sha256": sha(special[special_paths[3].as_posix()]),
    "space_doc_sha256": sha(special[special_paths[4].as_posix()]),
    "address_doc_sha256": sha(special[special_paths[5].as_posix()]),
...
require(
    ...
    "sha256 of rugra-marshal-pk-join-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("packed_special_space_bytes", "MATCH"),
    ("packed_read_space", "MATCH"),
    ("packed_join_roundtrip", "MATCH"),
    ("xml_join_roundtrip", "MATCH"),
    ("join_codec_edges", "MATCH"),
    ("piece_register_form", "UNTESTED"),
    ("piece_unknown_space_name", "UNTESTED"),
    ("fspec_valid_entry_packed", "UNTESTED"),
    ("iop_print_raw", "UNTESTED"),
    ("consumer_migration", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-marshal-pk-join-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/marshal_packed_join_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/marshal_packed_join_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/marshal_packed_join_1204.rs"
...
cpp_binary="$oracle_tmp/marshal_packed_join_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "marshal_packed_join_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/marshal_packed_join_1204_rust"
...
expected_cases = [
    "packed_special_space_bytes", "packed_read_space", "packed_join_roundtrip",
    "xml_join_roundtrip", "join_codec_edges",
]
...
metadata = json.loads(
    (repo / "tests/oracle/marshal_packed_join_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_marshal_packed_join_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/marshal_packed_join_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/marshal_packed_join_1204.rs").read_bytes()),
    comparand["rust_fixture_sha256"],
)
for label, relative, key in (
    ("marshal implementation", "src/marshal.rs", "marshal_rs_sha256"),
    ("space implementation", "src/space.rs", "space_rs_sha256"),
    ("address implementation", "src/address.rs", "address_rs_sha256"),
    ("marshal API document", "docs/api/marshal.md", "marshal_doc_sha256"),
    ("space API document", "docs/api/space.md", "space_doc_sha256"),
    ("address API document", "docs/api/address.md", "address_doc_sha256"),
):
    require(label, sha((repo / relative).read_bytes()), comparand[key])

overlay_files = {"src/marshal.rs", "src/space.rs", "src/address.rs"}
...
hasher.update(b"rugra-marshal-pk-join-base-overlay-v1\0")
...
echo "marshal_packed_join_1204: MATCH covered_projection=5/5 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

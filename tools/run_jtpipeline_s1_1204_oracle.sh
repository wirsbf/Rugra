#!/usr/bin/env bash
# JUMPTABLE-PIPELINE-0001 segment 1 — locked Ghidra 12.0.4 differential gate.
# Bilateral fixture for the jumptable.rs domain: model selection chain
# (override → Assisted → Basic → Basic2), recover_model → find_normalized
# call shape, EmulateFunction loader bridge and typed error channels.
# Modeled on tools/run_jt_guards_oracle.sh with task-owned staging root.
set -euo pipefail
...
if [[ ! -d "$staging_root" || -L "$staging_root" ]] || \
   [[ "$(stat -c '%U:%a' "$staging_root")" != "$(id -un):700" ]]; then
...
for required in "$metadata" "$cpp_fixture" "$rust_fixture" "$rust_overlay" \
  "$runner" "$bfd_include/bfd.h" "$bfd_library"; do
...
# jumptable observations depend on; overlay it the same way.
rust_rangeutil_overlay="$repo_root/src/rangeutil.rs"
cp "$rust_rangeutil_overlay" "$snapshot_root/src/rangeutil.rs"
cp "$cpp_fixture" "$snapshot_root/tests/oracle/jtpipeline_s1_1204.cc"
...
(
    ...
    rangeutil_raw, runner_sha, binary_size, oracle_commit, oracle_tag, cpp_tree,
...
require("fixture", metadata["fixture_id"], "JT-PIPELINE-S1-1204")
...
for key, path in (
    ...
    ("rangeutil_overlay_sha256", pathlib.Path(rangeutil_raw)),
):
...
if not isinstance(residuals, list) or not residuals:
    raise SystemExit("residuals must be a non-empty list")
...
expected_mismatches = {
    key for key, record in coverage.items() if record["status"] == "MISMATCH"
}
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/jtpipeline_s1_1204.cc" \
  ...
  "$bfd_library" -lz -o "$oracle_tmp/jtpipeline_s1_1204_cpp"
...
set +e
LD_LIBRARY_PATH="$bfd_root/usr/lib/x86_64-linux-gnu" \
  "$oracle_tmp/jtpipeline_s1_1204_cpp" \
...
if [record.split("|", 1)[0] for record in records] != [
    "sel_basic", "sel_override", "sel_basic2_default", "sel_allfail",
    "emulfn_load_ok", "emulfn_load_dataunavail", "emulfn_channels",
    "pipeline_env_note",
]:
...
printf 'jtpipeline_s1_1204: covered=8/8 projection_status=%s overall_status=%s residuals=%s\n' \
  ...
  "$(python3 -c "import json;print(len(json.load(open('$metadata'))['residuals']))")"

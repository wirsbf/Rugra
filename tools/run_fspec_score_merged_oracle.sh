...
# FSPEC-SCORE-MERGED-1204 oracle runner (pin-base schema2): rebuilds the
...
runner="$repo_root/tools/run_fspec_score_merged_oracle.sh"
...
rugra_base_commit=b8b26e25d21e80b7fea60e6207467c3bda95851f
rugra_base_tree=1c2fd8f7b2abf2bb0cb1c6146d6b2877bae3ed2b
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/fspec_score_merged_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/fspec_score_merged_1204.cc"
rust_fixture="$repo_root/tests/oracle/fspec_score_merged_1204.rs"
...
host_cargo_bin=/home/ls/.cargo/bin/cargo
host_rustc_bin=/home/ls/.cargo/bin/rustc
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-fspec-score-merged-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-fspec-score-merged-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/fspec_score_merged_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/fspec_score_merged_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/fspec_score_merged_1204.rs",
...
crate_hasher.update(b"rugra-fspec-score-merged-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/fspec_score_merged_1204.cc"),
    pathlib.Path("tests/oracle/fspec_score_merged_1204.rs"),
    pathlib.Path("tests/oracle/fspec_score_merged_1204.metadata.json"),
    pathlib.Path("docs/api/fspec.md"),
    pathlib.Path("tools/run_fspec_score_merged_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "FSPEC-SCORE-MERGED-1204")
...
require(
    ...
    "sha256 of rugra-fspec-score-merged-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("score_penalty_walk", "MATCH"),
    ("param_list_merged_fold_in", "MATCH"),
    ("proto_model_merged_fold_in", "MATCH"),
    ("select_model", "MATCH"),
    ("join_resolver_window", "UNTESTED"),
    ("cspec_loaded_models", "UNTESTED"),
    ("store_family", "UNTESTED"),
    ("callsite_family", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-fspec-score-merged-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/fspec_score_merged_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/fspec_score_merged_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/fspec_score_merged_1204.rs"
...
cpp_binary="$oracle_tmp/fspec_score_merged_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "fspec_score_merged_1204: GHIDRA_LOCKED_OUTPUT_OK covered=MATCH overall=UNTESTED stdout_sha256=$actual_stdout_sha"
  exit 0
fi

fixture_target="$user_home/.cache/rugra-oracle-targets/fspec-score-merged"
...
rust_binary="$oracle_tmp/fspec_score_merged_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=FSPEC-SCORE-MERGED-1204|"
...
echo "fspec_score_merged_1204: OK covered=MATCH overall=UNTESTED ghidra=$actual_stdout_sha"

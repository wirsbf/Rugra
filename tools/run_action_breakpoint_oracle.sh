...
# ACTION-BREAKPOINT-RESUME-0001 pin-base schema2 oracle runner.  Builds the
# locked Ghidra 12.0.4 fixture (real ActionDatabase::universalAction ->
# resetDefaults -> getCurrent tree plus scripted probe trees driven through
# the real setBreakPoint/getSubAction/getSubRule/perform/printState) and the
# Rugra comparand (real set_default_actions tree plus the same scripted
# probes over the ported breakpoint machinery) from a pinned base commit
# overlaid with the working-tree src/action.rs, then requires byte-identical
# observations.
...
rugra_base_commit=55edaed3540dd7aa7565a1bad5679d9a37dd75f2
rugra_base_tree=a929beecfc650658a3d4401554f59a1920b97624
rugra_base_src_tree=281125cfb0baf116c840fc74e52415415bd771dc
rugra_base_action_blob=35e806e0e7abbc2760bc6344a7d262de5b038464
...
metadata="$repo_root/tests/oracle/action_breakpoint_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/action_breakpoint_1204.cc"
rust_fixture="$repo_root/tests/oracle/action_breakpoint_1204.rs"
action_overlay="$repo_root/src/action.rs"
runner="$repo_root/tools/run_action_breakpoint_oracle.sh"

oracle_tmp=$(mktemp -d /tmp/rugra-action-bkpt-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-action-bkpt-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
(
    repo_raw, metadata_raw, cpp_raw, rust_raw, action_raw,
...
require("fixture", metadata["fixture_id"], "ACTION-BREAKPOINT-RESUME-0001")
...
cp "$cpp_fixture" "$snapshot_root/tests/oracle/action_breakpoint_1204.cc"
cp "$rust_fixture" "$snapshot_root/tests/oracle/action_breakpoint_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/action_breakpoint_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_action_breakpoint_oracle.sh"
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare -m64 \
  -I"$oracle_cpp" "$snapshot_root/tests/oracle/action_breakpoint_1204.cc" \
  ...
  -o "$oracle_tmp/action_breakpoint_1204_cpp"
...
rustc --edition=2021 -O \
  ...
  "$snapshot_root/tests/oracle/action_breakpoint_1204.rs" \
  -o "$oracle_tmp/action_breakpoint_1204_rust"

set +e
"$oracle_tmp/action_breakpoint_1204_cpp" \
  >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
ghidra_status=$?
"$oracle_tmp/action_breakpoint_1204_rust" \
...
if len(lines) != 52:
    raise SystemExit(f"expected 52 fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=ACTION-BREAKPOINT-RESUME-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
# Decisive semantic anchors.
anchors = {
    "addr|spec=universal:fullloop:mainloop:unreachable|kind=action|match=NONE",
    "addr|spec=universal:fullloop:deadcode|kind=action|match=NONE",
    "addr|spec=universal:dynamicsymbols|kind=action|match=NONE",
    "addr|spec=universal:cleanup:multnegone|kind=rule|name=multnegone|pool=cleanup",
    "addr|spec=universal:fullloop:mainloop:stackstall:oppool1:earlyremoval|kind=rule|name=earlyremoval|pool=oppool1",
    "real|fullloop_perform1=-1|state=fullloop:mainloop start|mainloop_status=2",
    "real|ss_perform1=-1|state1=stackstall start",
    "c1_call1|ret=-1|status=2|count=0|lcount=0|tests=0|applies=0|breakpoint=1|apply_calls=0",
    "c3_call2|ret=5|status=1|count=5|lcount=0|tests=1|applies=1|breakpoint=4|apply_calls=1",
    "c4_call4|ret=-1|status=32|count=7|state_index=3|trace=",
    "c5_call2|ret=3|status=1|count=3|state_index=2|trace=c5_last",
    "c6|single=7|resumed=7|single_count=7|resumed_count=7|single_applies=1|resumed_applies=1|trace=same|equal=1",
    "d_break1|ret=-1|rule_tests=1|rule_applies=1|applied_at=0xb000",
    "d_break2|ret=-1|rule_tests=2|rule_applies=2|applied_at=0xb000>0xb002",
}
missing = anchors - set(lines)
if missing:
    raise SystemExit(f"decisive anchor lines missing: {sorted(missing)}")
if not any(
    line.startswith("d_final|") and line.endswith("|resume_equals_single=1")
    for line in lines
):
    raise SystemExit("d_final resume-equals-single anchor missing")
...
printf 'action_breakpoint_1204: breakpoint_resume=MATCH overall=MATCH lines=52 stdout_sha256=%s\n' \

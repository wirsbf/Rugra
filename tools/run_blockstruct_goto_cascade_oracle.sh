#!/usr/bin/env bash
# BLOCKSTRUCT-GOTOCASCADE-CONDSTMT-0001 bilateral runner.
#
# Builds the locked Ghidra 12.0.4 oracle fixture (CollapseStructure::
# collapseAll on synthetic FlowBlock graphs) against the oracle cpp tree,
# builds the Rugra comparand against the crate rlib, runs both, and diffs
# the shared projection. Residual diffs are the registered MISMATCH set
# (see tests/oracle/blockstruct_goto_cascade_1204.metadata.json).
set -euo pipefail
...
cpp_fixture="$repo_root/tests/oracle/blockstruct_goto_cascade_1204.cc"
rust_fixture="$repo_root/tests/oracle/blockstruct_goto_cascade_1204.rs"
metadata="$repo_root/tests/oracle/blockstruct_goto_cascade_1204.metadata.json"
...
oracle_tmp=$(mktemp -d /tmp/rugra-blockstruct-goto-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-blockstruct-goto-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
g++ -std=c++11 -O2 -I"$cpp_root" -I"$bfd_root/usr/include" \
  ...
  -o "$oracle_tmp/blockstruct_goto_cascade_1204"

# Rust comparand: crate rlib + rustc-linked fixture (printc_blockgraph
# runner pattern).
...
rustc --edition=2021 -O -L "dependency=$fixture_target/debug/deps" \
  ...
  -o "$oracle_tmp/blockstruct_goto_cascade_rugra"

"$oracle_tmp/blockstruct_goto_cascade_1204" >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
"$oracle_tmp/blockstruct_goto_cascade_rugra" >"$oracle_tmp/rugra.stdout" 2>"$oracle_tmp/rugra.stderr"

if diff -u "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout" \
    >"$oracle_tmp/bilateral.diff"; then
  cat "$oracle_tmp/ghidra.stdout"
  printf 'blockstruct_goto_cascade_1204: MATCH\n'
else
  cat "$oracle_tmp/bilateral.diff"
  printf 'blockstruct_goto_cascade_1204: MISMATCH (%s diff lines) — registered to BLOCKSTRUCT-IDENTIFY-BOUNDARY-0001 (see metadata)\n' \

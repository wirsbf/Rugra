...
metadata="$repo_root/tests/oracle/address_space_handle_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/address_space_handle_1204.cc"
rust_fixture="$repo_root/tests/oracle/address_space_handle_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$address_rs" "$space_rs" "$address_doc" "$space_doc" \
  "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-address-handle-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-address-handle-1204.?????? ]]; then
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, address_raw, space_raw,
    address_doc_raw, space_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/address_space_handle_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/address_space_handle_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/address_space_handle_1204.rs",
...
crate_hasher.update(b"rugra-address-handle-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/address_space_handle_1204.cc"),
    pathlib.Path("tests/oracle/address_space_handle_1204.rs"),
    pathlib.Path("tests/oracle/address_space_handle_1204.metadata.json"),
...
require("fixture id", metadata["fixture_id"], "ADDRESS-SPACE-HANDLE-1204")
require(
    "stable function id",
...
require(
    ...
    "sha256 of rugra-address-handle-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("invalid_vs_ram0", "MATCH"),
    ("cross_space_ordering", "MATCH"),
    ("wrap_boundaries", "MATCH"),
    ("justified_big_endian", "MATCH"),
    ("range_space_isolation", "MATCH"),
    ("rangelist_queries", "MATCH"),
    ("range_properties", "MATCH"),
    ("address_read_encode_decode", "UNTESTED"),
    ("renormalize_join_records", "UNTESTED"),
    ("register_name_ranges", "UNTESTED"),
    ("seqnum_space_migration", "UNTESTED"),
...
registry_hasher.update(b"rugra-address-handle-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/address_space_handle_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/address_space_handle_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/address_space_handle_1204.rs"
...
cpp_binary="$oracle_tmp/address_space_handle_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "address_space_handle_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/address_space_handle_1204_rust"
...
if len(lines) != 75:
    raise SystemExit(f"expected seventy-five fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=ADDRESS-SPACE-HANDLE-1204|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "invalid_vs_ram0", "cross_space_ordering", "wrap_boundaries",
    "justified_big_endian", "range_space_isolation", "rangelist_queries",
    "range_properties",
]
...
metadata = json.loads(
    (repo / "tests/oracle/address_space_handle_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "C++ fixture",
    sha((repo / "tests/oracle/address_space_handle_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/address_space_handle_1204.rs").read_bytes()),
...
overlay_files = {"src/address.rs", "src/space.rs"}
...
hasher.update(b"rugra-address-handle-base-overlay-v1\0")
...
echo "address_space_handle_1204: MATCH covered_projection=7/7 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

...
# FSPEC-FINDENTRY-1204 oracle runner (pin-base schema2): rebuilds the
...
# metadata. Covered projection: ParamListStandard::findEntry resolver
# find-window gating (extent-out queries return NULL even with
# just=false; minSize inside the window), join entries reached through
# per-piece registration (populateResolver), the cross-space per-piece
# address.cc:133 space guard of the join walk, and the
# find/characterize shared-window contrast
# (FSPEC-FINDENTRY-GATE-0005 + FSPEC-RESOLVER-JOIN-WINDOW-0004).
...
runner="$repo_root/tools/run_fspec_findentry_oracle.sh"
...
metadata="$repo_root/tests/oracle/fspec_findentry_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/fspec_findentry_1204.cc"
rust_fixture="$repo_root/tests/oracle/fspec_findentry_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-fspec-findentry-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-fspec-findentry-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/fspec_findentry_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/fspec_findentry_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/fspec_findentry_1204.rs",
...
crate_hasher.update(b"rugra-fspec-findentry-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/fspec_findentry_1204.cc"),
    pathlib.Path("tests/oracle/fspec_findentry_1204.rs"),
    pathlib.Path("tests/oracle/fspec_findentry_1204.metadata.json"),
    pathlib.Path("docs/api/fspec.md"),
    pathlib.Path("tools/run_fspec_findentry_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "FSPEC-FINDENTRY-1204")
...
require(
    ...
    "sha256 of rugra-fspec-findentry-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("find_window_gate", "MATCH"),
    ("join_piece_access", "MATCH"),
    ("cross_space_join", "MATCH"),
    ("find_vs_characterize", "MATCH"),
    ("be_space_rows", "UNTESTED"),
    ("aligned_entry_foreign_space", "UNTESTED"),
    ("spaceless_wrapper_callers", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-fspec-findentry-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/fspec_findentry_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/fspec_findentry_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/fspec_findentry_1204.rs"
...
cpp_binary="$oracle_tmp/fspec_findentry_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "fspec_findentry_1204: GHIDRA_LOCKED_OUTPUT_OK covered=MATCH overall=UNTESTED stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/fspec_findentry_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=FSPEC-FINDENTRY-1204|"
...
echo "fspec_findentry_1204: OK covered=MATCH overall=UNTESTED ghidra=$actual_stdout_sha"

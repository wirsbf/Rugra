...
runner="$repo_root/tools/run_heritage_callguard2_oracle.sh"
...
require("fixture id", metadata["fixture_id"], "HERITAGE-CALLGUARD2-0001")
...
for required_key, required_prefix in (
    ...
    ("production_entry_getstr_form_per_object", "MATCH"),
    ("canonical_entry_identical", "MATCH"),
    ("guard_def_use_wiring", "MATCH"),
    ("production_second_pass_inert", "MATCH"),
...
registry_hasher.update(b"rugra-heritage-callguard2-registry-lock-v1\0")
...
if $ghidra_only; then
  ...
  echo "heritage_callguard2_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
expected_cases = [
    "model_state",
    "production_two_calls_ten_ranges",
    "canonical_two_calls_ten_ranges",
    "guard_def_use",
    "production_second_pass_inert",
    "stack_translation",
]
...
for line in lines:
    if line.startswith("case=production_two_calls_ten_ranges") and "count=20" not in line:
        raise SystemExit(f"production GetStr-form case must witness twenty guards: {line}")
    if line.startswith("case=canonical_two_calls_ten_ranges") and "identical=1" not in line:
        raise SystemExit(f"canonical entry must be identical to the production entry: {line}")
    if line.startswith("case=guard_def_use") and "count=10" not in line:
        raise SystemExit(f"def-use case must witness ten guards: {line}")
    if (
        line.startswith("case=production_second_pass_inert")
        and ("pass2new=0" not in line or "stable=1" not in line)
    ):
        raise SystemExit(f"second production pass must be inert and stable: {line}")
    if line.startswith("case=stack_translation") and "pass1count=0" not in line:
        raise SystemExit(f"stack delay must gate the first pass: {line}")
PY
...
metadata = json.loads(
    (repo / "tests/oracle/heritage_callguard2_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_heritage_callguard2_oracle.sh").read_bytes()),
...
echo "heritage_callguard2_1204: MATCH covered_projection=7/7 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

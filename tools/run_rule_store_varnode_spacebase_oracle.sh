...
metadata="$repo_root/tests/oracle/rule_store_varnode_spacebase_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/rule_store_varnode_spacebase_1204.cc"
rust_fixture="$repo_root/tests/oracle/rule_store_varnode_spacebase_1204.rs"
runner="$repo_root/tools/run_rule_store_varnode_spacebase_oracle.sh"
bfd_include=/tmp/rugra-ghidra-bfd-2.38/usr/include
bfd_header="$bfd_include/bfd.h"
bfd_library=/usr/lib/x86_64-linux-gnu/libbfd-2.38-system.so

oracle_tmp=$(mktemp -d /tmp/rugra-rule-store-varnode-spacebase-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-rule-store-varnode-spacebase-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
expected_paths = {
    metadata_path: repo_root / "tests/oracle/rule_store_varnode_spacebase_1204.metadata.json",
    cpp_fixture_path: repo_root / "tests/oracle/rule_store_varnode_spacebase_1204.cc",
    rust_fixture_path: repo_root / "tests/oracle/rule_store_varnode_spacebase_1204.rs",
    runner_path: repo_root / "tools/run_rule_store_varnode_spacebase_oracle.sh",
}
...
crate_hasher.update(b"rugra-rule-store-varnode-spacebase-lib-snapshot-v1\0")
crate_bytes = {}
...
special_paths = [
    pathlib.Path("tests/oracle/rule_store_varnode_spacebase_1204.cc"),
    pathlib.Path("tests/oracle/rule_store_varnode_spacebase_1204.rs"),
    pathlib.Path("tests/oracle/rule_store_varnode_spacebase_1204.metadata.json"),
    pathlib.Path("tools/run_rule_store_varnode_spacebase_oracle.sh"),
]
special = {path.as_posix(): snapshot_file(path) for path in special_paths}
require(
    "runner snapshot/live copy",
    sha256(special["tools/run_rule_store_varnode_spacebase_oracle.sh"]),
    runner_sha,
)
...
metadata = json.loads(special["tests/oracle/rule_store_varnode_spacebase_1204.metadata.json"])
require("metadata schema", metadata["schema_version"], 1)
require("fixture id", metadata["fixture_id"], "PRINTC-INPUTREG-DEADSTORE-0001")
...
observed = {
    "cpp_fixture_sha256": sha256(special["tests/oracle/rule_store_varnode_spacebase_1204.cc"]),
    "rust_fixture_sha256": sha256(special["tests/oracle/rule_store_varnode_spacebase_1204.rs"]),
...
require(
    ...
    "sha256 of rugra-rule-store-varnode-spacebase-lib-snapshot-v1 plus sorted length-prefixed relative paths and contents",
)
...
require(
    ...
    "MATCH: spacebase-registry observations (stack contain link + spacebase record, 8 rule-call cases incl. hit/miss families) are byte-identical under the locked oracle; ARCH-0001 enum-space stand-in remains a known MISMATCH at the full-state level and CALLSPECS-0001/SCOPELOCAL-0001 tails remain UNTESTED",
)
for key in (
    "registry_stack_contain_and_record",
    "store_sbinput_off0",
    "store_sbinput_add",
    "store_sbinput_swap",
    "store_contain_mismatch",
    "store_written_base",
    "store_copy_wrapped",
    "load_sbinput_add",
    "store_non_spacebase",
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/rule_store_varnode_spacebase_1204.cc" \
  ...
  -o "$oracle_tmp/rule_store_varnode_spacebase_1204_cpp"

CARGO_TARGET_DIR="$oracle_tmp/cargo-target" \
  cargo build --offline --locked --quiet --manifest-path "$snapshot_root/Cargo.toml" --lib
rustc --edition=2021 "$snapshot_root/tests/oracle/rule_store_varnode_spacebase_1204.rs" \
  --extern rugra="$oracle_tmp/cargo-target/debug/librugra.rlib" \
  -L "dependency=$oracle_tmp/cargo-target/debug/deps" \
  -o "$oracle_tmp/rule_store_varnode_spacebase_1204_rust"

"$oracle_tmp/rule_store_varnode_spacebase_1204_cpp" \
  "$snapshot_root/sleigh_specs" "$snapshot_root/examples/curl" \
  >"$oracle_tmp/ghidra.stdout"
"$oracle_tmp/rule_store_varnode_spacebase_1204_rust" >"$oracle_tmp/rugra.stdout"
diff -u "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout"

python3 -I -S - "$snapshot_root/tests/oracle/rule_store_varnode_spacebase_1204.metadata.json" \
...
printf 'rule_store_varnode_spacebase_1204: MATCH cases=9 records=18 dependencies=ARCH-0001 untested=CALLSPECS-0001,SCOPELOCAL-0001'

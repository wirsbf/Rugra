...
runner="$repo_root/tools/run_cover_twopiece_oracle.sh"
...
rugra_base_commit=a301036407de8f1b2291abf600f0ac72335a1912
rugra_base_tree=b8ea38f30882068bb2e902db2a5cc836b28c6891
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/cover_twopiece_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/cover_twopiece_1204.cc"
rust_fixture="$repo_root/tests/oracle/cover_twopiece_1204.rs"
cover_rs="$repo_root/src/cover.rs"
merge_rs="$repo_root/src/merge.rs"
cover_doc="$repo_root/docs/api/cover.md"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-cover-twopiece-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-cover-twopiece-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C GIT_CONFIG_NOSYSTEM=1 \
  ...
  "$cover_rs" "$merge_rs" "$cover_doc" \
  "$merge_doc" "$runner_fd_path" "$runner_snapshot_sha" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, cover_raw, merge_raw,
    cover_doc_raw, merge_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/cover_twopiece_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/cover_twopiece_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/cover_twopiece_1204.rs",
    pathlib.Path(cover_raw): repo / "src/cover.rs",
    pathlib.Path(merge_raw): repo / "src/merge.rs",
    pathlib.Path(cover_doc_raw): repo / "docs/api/cover.md",
...
crate_hasher.update(b"rugra-cover-twopiece-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/cover_twopiece_1204.cc"),
    pathlib.Path("tests/oracle/cover_twopiece_1204.rs"),
    pathlib.Path("tests/oracle/cover_twopiece_1204.metadata.json"),
    pathlib.Path("docs/api/cover.md"),
    pathlib.Path("docs/api/merge.md"),
    pathlib.Path("tools/run_cover_twopiece_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "COVER-TWOPIECE-RESIDUAL-0001")
require(
    "stable function id",
...
observed_hashes = {
    ...
    "cover_rs_sha256": sha(crate_bytes["src/cover.rs"]),
    "merge_rs_sha256": sha(crate_bytes["src/merge.rs"]),
    "cover_doc_sha256": sha(special[special_paths[3].as_posix()]),
    "merge_doc_sha256": sha(special[special_paths[4].as_posix()]),
...
require(
    ...
    "sha256 of rugra-cover-twopiece-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("construct_setbegin_setend_sentinel_side_effect", "MATCH"),
    ("twopiece_construct_empty_and_contain", "MATCH"),
    ("boundary_pointer_identity_discriminators", "MATCH"),
    ("merge_disjoint_wrap", "MATCH"),
    ("merge_internal4_wrap", "MATCH"),
    ("merge_setall_and_equal_start", "MATCH"),
    ("intersect_four_quadrants", "MATCH"),
    ("merge_empty_identity_copy", "MATCH"),
    ("rebuild_wrap_join_multiequal_reader", "MATCH"),
    ("rebuild_wrap_then_contained_reader", "MATCH"),
    ("rebuild_multiequal_tip_old_stop_discrimination", "MATCH"),
    ("rebuild_cover_merge_onepiece_twopiece_union", "MATCH"),
...
registry_hasher.update(b"rugra-cover-twopiece-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/cover_twopiece_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/cover_twopiece_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/cover_twopiece_1204.rs"
...
cpp_binary="$oracle_tmp/cover_twopiece_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "cover_twopiece_1204: GHIDRA_LOCKED_OUTPUT_OK overall=MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/cover_twopiece_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=COVER-TWOPIECE-RESIDUAL-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "a1_construct", "a2_twopiece_construct", "a3_boundary_sentinels",
    "a4_merge_disjoint_wrap", "a5_merge_internal4_wrap", "a6_merge_setall",
    "a7_intersect_quadrants", "a8_merge_empty_copy",
    "b1_wrap_join_multiequal_reader", "b2_wrap_then_contained_reader",
    "b3_multiequal_tip_precise", "b4_merge_wrap_internal",
]
...
metadata = json.loads(
    (repo / "tests/oracle/cover_twopiece_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_cover_twopiece_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/cover_twopiece_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/cover_twopiece_1204.rs").read_bytes()),
    comparand["rust_fixture_sha256"],
)
for label, relative in (
    ("cover implementation", "src/cover.rs"),
    ("merge implementation", "src/merge.rs"),
    ("cover API document", "docs/api/cover.md"),
    ("merge API document", "docs/api/merge.md"),
):
...
overlay_files = {"src/cover.rs", "src/merge.rs"}
...
hasher.update(b"rugra-cover-twopiece-base-overlay-v1\0")
...
echo "cover_twopiece_1204: MATCH covered_projection=13/13 overall=MATCH stdout_sha256=$actual_stdout_sha"

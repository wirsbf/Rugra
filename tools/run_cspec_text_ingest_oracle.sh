...
# CSPEC-TEXT-INGEST-0001 oracle runner: compiler-spec TEXT ingestion
# parity.  Builds the locked Ghidra 12.0.4 oracle (BfdArchitecture real
# chain: spec-dir scan -> Architecture::init -> parseCompilerConfig) and an
# isolated base-plus-overlay Rust snapshot (marshal DocumentStorage text
# parse -> Architecture::parse_compiler_config), runs both
# cspec_text_ingest_1204 fixtures on identical production x86-64-gcc.cspec
# bytes, and diffs the published-state projections byte for byte.
...
runner="$repo_root/tools/run_cspec_text_ingest_oracle.sh"
...
rugra_base_commit=1f65b9d76c2e7e5214257c98d82791c2150a8989
rugra_base_tree=e33b9f272fa6513bb417f39653d1ccd7bd9b464f
ghidra_root="$repo_root/ghidra"
bfd_include=/tmp/rugra-ghidra-bfd-2.38/usr/include
bfd_library=/usr/lib/x86_64-linux-gnu/libbfd-2.38-system.so
metadata="$repo_root/tests/oracle/cspec_text_ingest_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/cspec_text_ingest_1204.cc"
rust_fixture="$repo_root/tests/oracle/cspec_text_ingest_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-cspec-text-ingest-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-cspec-text-ingest-1204.??????) /usr/bin/rm -rf -- "$oracle_tmp" ;;
...
# Owned-file overlays (the four CSPEC slices + fixtures + docs).
overlay_paths=(
  "src/arch.rs"
  "src/fspec.rs"
  "src/marshal.rs"
  ...
  "docs/api/fspec.md"
  "docs/api/marshal.md"
  "docs/api/pcodeinject.md"
  "docs/api/pcodeparse.md"
  "docs/api/userop.md"
  "tests/oracle/cspec_text_ingest_1204.cc"
  "tests/oracle/cspec_text_ingest_1204.rs"
  "tests/oracle/cspec_text_ingest_1204.metadata.json"
  "tools/run_cspec_text_ingest_oracle.sh"
)
...
metadata_path = snapshot / "tests/oracle/cspec_text_ingest_1204.metadata.json"
metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
reject_pending(metadata)
require("metadata schema", metadata["schema_version"], 1)
require("fixture id", metadata["fixture_id"], "CSPEC-TEXT-INGEST-0001")
...
require(
    "ingest observation status",
    metadata["coverage"]["text_ingest_status"], "MATCH")
require(
    "coverage record counts",
    metadata["coverage"]["record_counts"], {
        "SPACE": 9, "GLOBAL": 3, "RET": 1, "STACK": 1, "MODELRET": 5,
        "THISCALL": 1, "FIXUP": 16, "TPL": 16,
        "CALLOTHER_COUNT_BEFORE": 1, "CALLOTHER_COMPILEFAIL": 1,
        "CALLOTHER_COUNT_AFTER": 1, "CALLOTHER_RESIDUE_ID": 1,
        "CALLOTHER_UNKNOWN": 1, "CALLOTHER_COUNT_AFTER2": 1,
        "CALLOTHER_RESIDUE_ID2": 1, "CALLOTHER_SEGMENT": 1,
        "CALLOTHER_COUNT_AFTER3": 1, "CALLOTHER_SEGMENT_TYPE": 1,
        "VOLATILE1": 1, "VOLATILE_NAMES": 1, "VOLATILE2": 1,
        "CALLFIXUP_PARAMNAME": 1, "CALLFIXUP_PARAMNAME_ID": 1,
        "RA_EMPTY_CHAIN": 1, "NOHIGHPTR_HEX": 1, "RA_DOUBLE_CHAIN": 1,
        "SCHEMA": 1, "DONE": 1,
    })
require(
    "residual statuses",
...
paths = {
    "cpp_fixture_sha256": snapshot / "tests/oracle/cspec_text_ingest_1204.cc",
    "rust_fixture_sha256": snapshot / "tests/oracle/cspec_text_ingest_1204.rs",
    "arch_rs_sha256": snapshot / "src/arch.rs",
    "fspec_rs_sha256": snapshot / "src/fspec.rs",
    "marshal_rs_sha256": snapshot / "src/marshal.rs",
    "pcodeinject_rs_sha256": snapshot / "src/pcodeinject.rs",
    "pcodeparse_rs_sha256": snapshot / "src/pcodeparse.rs",
    "userop_rs_sha256": snapshot / "src/userop.rs",
    "arch_doc_sha256": snapshot / "docs/api/arch.md",
    "fspec_doc_sha256": snapshot / "docs/api/fspec.md",
    "marshal_doc_sha256": snapshot / "docs/api/marshal.md",
    "pcodeinject_doc_sha256": snapshot / "docs/api/pcodeinject.md",
    "pcodeparse_doc_sha256": snapshot / "docs/api/pcodeparse.md",
    "userop_doc_sha256": snapshot / "docs/api/userop.md",
    "runner_sha256": snapshot / "tools/run_cspec_text_ingest_oracle.sh",
...
cpp_fixture="$snapshot_root/tests/oracle/cspec_text_ingest_1204.cc"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_cxx_bin" \
  ...
  "$bfd_library" -lz \
  -o "$oracle_tmp/cspec_text_ingest_1204_cpp" \
...
rust_fixture="$snapshot_root/tests/oracle/cspec_text_ingest_1204.rs"
if ! /usr/bin/env -i HOME="$user_home" RUSTUP_HOME="$user_home/.rustup" \
  ...
  "$rust_fixture" -o "$oracle_tmp/cspec_text_ingest_1204_rust" \
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/cspec_text_ingest_1204_cpp" "$spec_root" "$binary" \
  >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/cspec_text_ingest_1204_rust" \
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S \
  - "$snapshot_root/tests/oracle/cspec_text_ingest_1204.metadata.json" \
...
print("text_ingest=MATCH overall=PARTIAL_MATCH (residuals registered in metadata)")
PY

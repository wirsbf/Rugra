...
cpp_fixture="$repo_root/tests/oracle/printc_terminal_1204.cc"
rust_fixture="$repo_root/tests/oracle/printc_terminal_1204.rs"
metadata="$repo_root/tests/oracle/printc_terminal_1204.metadata.json"
...
oracle_files=(printc.cc printc.hh block.cc block.hh op.cc op.hh typeop.cc typeop.hh)
...
python3 -I -S - "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$repo_root/src/printc.rs" "$cpp_root" "$oracle_commit" "$oracle_tag" <<'PY'
import hashlib
import json
import pathlib
import subprocess
...
if metadata["architecture"] != "language-independent PrintC P-code block":
...
if metadata["observation"]["covered_status"] != "MATCH":
    raise SystemExit("terminal statement-selection status must be MATCH")
if metadata["observation"]["overall_status"] != "MISMATCH":
    raise SystemExit("fixture must retain MISMATCH overall status")
...
g++ -std=c++11 -O2 -I"$cpp_root" \
  ...
  -o "$oracle_tmp/printc_terminal_1204"
...
printf 'printc_terminal_1204: MATCH statement-selection; raw=MISMATCH; overall=MISMATCH\n'

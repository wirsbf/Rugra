...
runner="$repo_root/tools/run_address_compat_order_oracle.sh"
...
rugra_base_commit=197b34cb6bde4959e93a8606eaa1f41f57814b82
rugra_base_tree=f3e7c3c3db80178cf71f3e719f9508d16e10a638
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/address_compat_order_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/address_compat_order_1204.cc"
rust_fixture="$repo_root/tests/oracle/address_compat_order_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-address-compat-order-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-address-compat-order-1204.?????? ]]; then
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, address_raw,
    address_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/address_compat_order_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/address_compat_order_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/address_compat_order_1204.rs",
...
crate_hasher.update(b"rugra-address-compat-order-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/address_compat_order_1204.cc"),
    pathlib.Path("tests/oracle/address_compat_order_1204.rs"),
    pathlib.Path("tests/oracle/address_compat_order_1204.metadata.json"),
    pathlib.Path("docs/api/address.md"),
    pathlib.Path("tools/run_address_compat_order_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "ADDRESS-COMPAT-ORDER-1204")
require(
    "stable function id",
...
require(
    ...
    "sha256 of rugra-address-compat-order-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("null_base_vs_real", "MATCH"),
    ("cross_space_walk", "MATCH"),
    ("same_space_offset_order", "MATCH"),
    ("wrap_arithmetic", "MATCH"),
    ("overlap_gates", "MATCH"),
    ("seqnum_order", "MATCH"),
    ("none_none_offset_compat", "UNTESTED"),
...
registry_hasher.update(b"rugra-address-compat-order-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/address_compat_order_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/address_compat_order_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/address_compat_order_1204.rs"
...
cpp_binary="$oracle_tmp/address_compat_order_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "address_compat_order_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/address_compat_order_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=ADDRESS-COMPAT-ORDER-1204|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "null_base_vs_real", "cross_space_walk", "same_space_offset_order",
    "wrap_arithmetic", "overlap_gates", "seqnum_order",
]
...
metadata = json.loads(
    (repo / "tests/oracle/address_compat_order_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_address_compat_order_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/address_compat_order_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/address_compat_order_1204.rs").read_bytes()),
...
hasher.update(b"rugra-address-compat-order-base-overlay-v1\0")
...
echo "address_compat_order_1204: MATCH covered_projection=6/6 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

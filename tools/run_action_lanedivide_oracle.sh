...
rugra_source_commit=98c3a9b12a2f40c4053c09b6c81354a9fffe19c4
rugra_source_tree=9ddf46fe84538621e132e1131cc756f92263409e
rugra_source_src_tree=b3301e02b00c034f483d56c25db7e27c91fe91aa
rugra_source_coreaction_blob=80c4462300c62c5d7bd2cc4746a716af305b9930
...
metadata="$repo_root/tests/oracle/action_lanedivide_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/action_lanedivide_1204.cc"
rust_fixture="$repo_root/tests/oracle/action_lanedivide_1204.rs"
coreaction_overlay="$repo_root/src/coreaction.rs"
runner="$repo_root/tools/run_action_lanedivide_oracle.sh"
bfd_include=/tmp/rugra-ghidra-bfd-2.38/usr/include
bfd_header="$bfd_include/bfd.h"
bfd_library=/usr/lib/x86_64-linux-gnu/libbfd-2.38-system.so

oracle_tmp=$(mktemp -d /tmp/rugra-action-lanedivide-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-action-lanedivide-1204.??????) rm -rf -- "$oracle_tmp" ;;
    *) echo "refusing unsafe cleanup target: $oracle_tmp" >&2 ;;
  esac
}
trap cleanup EXIT HUP INT TERM

for required in "$metadata" "$cpp_fixture" "$rust_fixture" "$coreaction_overlay" \
...
cp "$cpp_fixture" "$snapshot_root/tests/oracle/action_lanedivide_1204.cc"
cp "$rust_fixture" "$snapshot_root/tests/oracle/action_lanedivide_1204.rs"
cp "$metadata" "$snapshot_root/tests/oracle/action_lanedivide_1204.metadata.json"
cp "$runner" "$snapshot_root/tools/run_action_lanedivide_oracle.sh"
...
python3 -I -S - "$repo_root" "$snapshot_root" "$metadata" "$cpp_fixture" \
  "$rust_fixture" "$coreaction_overlay" \
...
(
    repo_raw, snapshot_raw, metadata_raw, cpp_raw, rust_raw, coreaction_raw,
...
require("fixture", metadata["fixture_id"], "ACTION-LANEDIVIDE-0001")
...
for label, actual, expected in (
    ...
    ("source coreaction blob", source["base_coreaction_blob"], source_coreaction_blob),
...
for key, relative in (
    ("coreaction_overlay_sha256", "src/coreaction.rs"),
):
...
expected_matches = {
    "collect_lane_sizes_sources_and_filter",
    "process_success_piece_split",
    "process_failure_default_mode_reject",
    "apply_ir_and_count",
    "rule_onceperfunc_second_pass_skip",
    "laned_map_clear_timing",
}
expected_residuals = {
    "mode1_downcast_subpiece_terminator",
    "multi_varnode_bounds_restart",
    "multi_storage_map_iteration",
    "mode2_pointer_size4_normalization",
}
...
for key in expected_residuals - {"mode2_pointer_size4_normalization"}:
    require(f"coverage.{key}.status", coverage[key]["status"], "UNTESTED")
require(
    "coverage mode2 pointer-4 status",
    coverage["mode2_pointer_size4_normalization"]["status"],
    "NO_ORACLE",
)
...
require(
    ...
    {"ACTION-LANEDIVIDE-RESIDUAL-0001"},
)
residual = metadata["residual_union"]
require("residual TODO", residual["todo_id"], "ACTION-LANEDIVIDE-RESIDUAL-0001")
...
for branch in residual["branches"]:
    expected_branch_status = (
        "NO_ORACLE" if branch["id"] == "mode2_pointer_size4_normalization" else "UNTESTED"
    )
...
nice -n 10 g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/action_lanedivide_1204.cc" \
  ...
  "$bfd_library" -lz -o "$oracle_tmp/action_lanedivide_1204_cpp"
...
rustc --edition=2021 "$snapshot_root/tests/oracle/action_lanedivide_1204.rs" \
  --extern rugra="$oracle_tmp/cargo-target/debug/librugra.rlib" \
  -L "dependency=$oracle_tmp/cargo-target/debug/deps" \
  -o "$oracle_tmp/action_lanedivide_1204_rust"

set +e
"$oracle_tmp/action_lanedivide_1204_cpp" \
  "$snapshot_root/sleigh_specs" "$snapshot_root/examples/curl" \
  >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
ghidra_status=$?
"$oracle_tmp/action_lanedivide_1204_rust" \
...
python3 -I -S - "$snapshot_root/tests/oracle/action_lanedivide_1204.metadata.json" \
...
printf 'action_lanedivide_1204: covered_projection=6/6 projection_status=MATCH overall_status=UNTESTED residual=ACTION-LANEDIVIDE-RESIDUAL-0001\n'

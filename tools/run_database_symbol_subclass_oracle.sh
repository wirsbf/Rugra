...
# Immutable runner for DATABASE-SYMBOL-SUBCLASS-FIXTURE-0001 (MIGW1-DATABASE-0005
# phase 2, second fixture).  The Ghidra oracle, Rugra base commit, leased
# database.rs, both
# fixture sources, spec assets, the pinned curl blob, and the BFD closure
# are all identity-checked before either comparand executes; the bilateral
# stdout must be byte-identical and match the registered hash.
...
metadata="$repo_root/tests/oracle/database_symbol_subclass_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/database_symbol_subclass_1204.cc"
rust_fixture="$repo_root/tests/oracle/database_symbol_subclass_1204.rs"
runner="$repo_root/tools/run_database_symbol_subclass_oracle.sh"
...
if metadata["fixture_id"] != "DATABASE-SYMBOL-SUBCLASS-FIXTURE-0001":
...
if metadata["observation"]["overall_status"] != "MATCH":
    raise SystemExit("symbol subclass fixture must be MATCH")
...
oracle_tmp=$(mktemp -d /tmp/rugra-database-symbol-subclass-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-database-symbol-subclass-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare -Wno-exceptions -m64 \
  ...
  "$bfd_library" -lz \
  -o "$oracle_tmp/database_symbol_subclass_ghidra"
...
ghidra_status=0
"$oracle_tmp/database_symbol_subclass_ghidra" \
...
# leased database.rs (which carries both MIGW1-DATABASE-0005 phases) and
...
cp "$rust_fixture" "$rugra_snapshot/src/bin/database_symbol_subclass_1204.rs"
...
CARGO_TARGET_DIR="$oracle_tmp/cargo-target" \
  ...
  --bin database_symbol_subclass_1204
rugra_status=0
"$oracle_tmp/cargo-target/debug/database_symbol_subclass_1204" \
...
printf 'database_symbol_subclass_1204: covered_oracle=MATCH rugra=MATCH overall=MATCH\n'

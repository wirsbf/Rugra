#!/usr/bin/env bash
# Immutable PRINTC-SYMBOL-DECL-0001 oracle runner (cover_rebuild mode).
#
...
# funcdata.rs/coreaction.rs/varmap.rs/merge.rs (FUNCDATA-LINKSYMBOL-TYPED-0001
# uncommitted WIP, consumed live) and printc.rs overlays, compiles both
# fixtures, runs them, and classifies every output record.  Every record must
# be byte-identical: the four cases cover the Symbol-driven declaration walk
# emitLocalVarDecls -> emitScopeVarDecls(no_category) -> emitVarDecl
# (printc.cc:2260/2518/2497) — typed register temporaries, the irregular
# input register (in_RCX), the dynamic-symbol list walk, and the
# $$undef-emitted / parameter-category-skipped map-branch filters.
set -euo pipefail
...
rugra_base_commit=ba706fb4a87a5f803f65d37484a2078f9eef1d5a
rugra_base_tree=ae70baacabc879181200252722e60efea59dd602
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/printc_symbol_decl_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/printc_symbol_decl_1204.cc"
rust_fixture="$repo_root/tests/oracle/printc_symbol_decl_1204.rs"
...
doc_printc="$repo_root/docs/api/printc.md"
bfd_include=/tmp/rugra-ghidra-bfd-2.38/usr/include
bfd_header="$bfd_include/bfd.h"
bfd_library=/usr/lib/x86_64-linux-gnu/libbfd-2.38-system.so

oracle_tmp=$(mktemp -d /tmp/rugra-printc-symbol-decl-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-printc-symbol-decl-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
for required in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$funcdata_rs" "$coreaction_rs" "$varmap_rs" "$merge_rs" "$printc_rs" \
  "$doc_printc" "$bfd_header" "$bfd_library"; do
...
python3 -I -S - "$repo_root" "$oracle_tmp" "$metadata" "$cpp_fixture" \
  ...
  "$printc_rs" "$doc_printc" "$runner_sha" "$rugra_base_commit" \
...
crate_hasher.update(b"rugra-printc-symbol-decl-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/printc_symbol_decl_1204.cc"),
    pathlib.Path("tests/oracle/printc_symbol_decl_1204.rs"),
    pathlib.Path("tests/oracle/printc_symbol_decl_1204.metadata.json"),
...
metadata = json.loads(special["tests/oracle/printc_symbol_decl_1204.metadata.json"])
require("metadata schema", metadata["schema_version"], 1)
require("fixture id", metadata["fixture_id"], "PRINTC-SYMBOL-DECL-0001")
...
observed = {
    "cpp_fixture_sha256": sha256(special["tests/oracle/printc_symbol_decl_1204.cc"]),
    "rust_fixture_sha256": sha256(special["tests/oracle/printc_symbol_decl_1204.rs"]),
    "runner_sha256": runner_sha,
    "doc_printc_sha256": sha256(special["docs/api/printc.md"]),
    "rust_crate_tree_sha256": crate_hasher.hexdigest(),
}
require(
    "crate hash scheme",
    comparand["rust_crate_tree_hash_scheme"],
    "sha256 of rugra-printc-symbol-decl-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$oracle_tmp/workspace/tests/oracle/printc_symbol_decl_1204.cc" \
  ...
  -o "$oracle_tmp/printc_symbol_decl_1204_cpp"
...
rustc --edition=2021 "$oracle_tmp/workspace/tests/oracle/printc_symbol_decl_1204.rs" \
  --extern rugra="$oracle_tmp/cargo-target/debug/librugra.rlib" \
  -L "dependency=$oracle_tmp/cargo-target/debug/deps" \
  -o "$oracle_tmp/printc_symbol_decl_1204_rust"

set +e
cd "$oracle_tmp/workspace"
"$oracle_tmp/printc_symbol_decl_1204_cpp" \
  sleigh_specs examples/curl \
  >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"
ghidra_status=$?
"$oracle_tmp/printc_symbol_decl_1204_rust" \
...
expected_records = [
    (case["id"], "decls") for case in metadata["input_manifest"]["cases"]
]

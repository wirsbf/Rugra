...
runner="$repo_root/tools/run_action_opcode_redispatch_oracle.sh"
...
rugra_source_commit=52b1ccbb9f1c50518e6535721a48f955a4c4831c
rugra_source_tree=85987c6ed633fd63fa3f138326d85758dcf385ce
rugra_source_src_tree=559b450cdc9dd0248e322d40251be4648bf11ad9
rugra_source_sleigh_shim_tree=c7729d9d1554dc62c486bcd7d58fdbf44bebb97d
rugra_source_coreaction_blob=7c4c07d7f6c5e2efa837e2ca145a053d7b9ba51c
...
metadata="$repo_root/tests/oracle/action_opcode_redispatch_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/action_opcode_redispatch_1204.cc"
rust_fixture="$repo_root/tests/oracle/action_opcode_redispatch_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-action-opcode-redispatch-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-action-opcode-redispatch-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/action_opcode_redispatch_1204.metadata.json",
    pathlib.Path(cpp_fixture_raw): repo / "tests/oracle/action_opcode_redispatch_1204.cc",
    pathlib.Path(rust_fixture_raw): repo / "tests/oracle/action_opcode_redispatch_1204.rs",
    pathlib.Path(runner_raw): repo / "tools/run_action_opcode_redispatch_oracle.sh",
}
...
base_hasher.update(b"rugra-action-opcode-redispatch-base-closure-v1\0")
...
special_paths = [
    "tests/oracle/action_opcode_redispatch_1204.cc",
    "tests/oracle/action_opcode_redispatch_1204.rs",
    "tests/oracle/action_opcode_redispatch_1204.metadata.json",
    "tools/run_action_opcode_redispatch_oracle.sh",
...
metadata = json.loads(special["tests/oracle/action_opcode_redispatch_1204.metadata.json"])
...
require(
    ...
    "sha256 of rugra-action-opcode-redispatch-base-closure-v1 plus sorted length-prefixed relative paths and pinned archive contents",
)
...
require(
    "Rugra overlay paths",
    [overlay["path"] for overlay in overlays],
    ["src/action.rs", "src/arch.rs"],
)
for overlay in overlays:
    require(
        f"Rugra overlay policy {overlay['path']}",
        overlay["policy"],
        "Hash this owned live source before replacing the same path in the pinned Rugra archive",
    )
    ...
    reject_pending(overlay["sha256"], f"rugra_source.overlays.{overlay['path']}.sha256")
    require(f"Rugra overlay {overlay['path']}", overlay["sha256"], sha(overlay_bytes))
...
observed = {
    "cpp_fixture_sha256": sha(special["tests/oracle/action_opcode_redispatch_1204.cc"]),
    "rust_fixture_sha256": sha(special["tests/oracle/action_opcode_redispatch_1204.rs"]),
...
require("runner snapshot/live", sha(special["tools/run_action_opcode_redispatch_oracle.sh"]), runner_sha)
...
for key, value in metadata["expected_observation_sha256"].items():
    reject_pending(value, f"expected_observation_sha256.{key}")
...
metadata="$snapshot_root/tests/oracle/action_opcode_redispatch_1204.metadata.json"
cpp_fixture="$snapshot_root/tests/oracle/action_opcode_redispatch_1204.cc"
rust_fixture="$snapshot_root/tests/oracle/action_opcode_redispatch_1204.rs"
...
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_cxx_bin" \
  ...
  "$bfd_library" -lz -o "$oracle_tmp/action_opcode_redispatch_1204_cpp" \
...
if ! /usr/bin/env -i HOME="$HOME" RUSTUP_HOME="$HOME/.rustup" \
  ...
  -o "$oracle_tmp/action_opcode_redispatch_1204_rust" \
...
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/action_opcode_redispatch_1204_cpp" "$spec_root" "$binary" \
  >"$oracle_tmp/ghidra.stdout" 2>"$oracle_tmp/ghidra.stderr"; then
  /usr/bin/cat "$oracle_tmp/ghidra.stderr" >&2
  exit 1
fi
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$oracle_tmp/action_opcode_redispatch_1204_rust" \
  >"$oracle_tmp/rugra.stdout" 2>"$oracle_tmp/rugra.stderr"; then
  /usr/bin/cat "$oracle_tmp/rugra.stderr" >&2
  exit 1
fi

stdout_diff_status=0
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  /usr/bin/diff -u --label ghidra-12.0.4 --label rugra \
  "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout" \
  >"$oracle_tmp/full.stdout.diff" || stdout_diff_status=$?
if [[ "$stdout_diff_status" -ne 1 ]]; then
  echo "expected registered full ActionPool carrier differences, diff status=$stdout_diff_status" >&2
  exit 1
fi
/usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  /usr/bin/diff -u --label ghidra-12.0.4-stderr --label rugra-stderr \
  "$oracle_tmp/ghidra.stderr" "$oracle_tmp/rugra.stderr"

/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S \
  - "$metadata" "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout" \
  "$oracle_tmp/ghidra.stderr" "$oracle_tmp/rugra.stderr" \
  "$oracle_tmp/full.stdout.diff" "$runner_fd_path" \
  "$snapshot_root/tests/oracle/action_opcode_redispatch_1204.cc" \
  "$snapshot_root/tests/oracle/action_opcode_redispatch_1204.rs" \
  "$snapshot_root/src/action.rs" "$snapshot_root/src/arch.rs" <<'PY'
import hashlib
import json
import pathlib
import sys
...
paths = {
    "ghidra_stdout": pathlib.Path(sys.argv[2]),
    "rugra_stdout": pathlib.Path(sys.argv[3]),
    "ghidra_stderr": pathlib.Path(sys.argv[4]),
    "rugra_stderr": pathlib.Path(sys.argv[5]),
    "full_stdout_diff": pathlib.Path(sys.argv[6]),
}
runner_bytes = pathlib.Path(sys.argv[7]).read_bytes()
cpp_bytes = pathlib.Path(sys.argv[8]).read_bytes()
rust_bytes = pathlib.Path(sys.argv[9]).read_bytes()
action_bytes = pathlib.Path(sys.argv[10]).read_bytes()
arch_bytes = pathlib.Path(sys.argv[11]).read_bytes()
...
for label, path in paths.items():
    ...
    expected = metadata["expected_observation_sha256"][label]
    if actual != expected:
        raise SystemExit(f"{label} hash mismatch: expected={expected} actual={actual}")

post_readback = {
    "runner_sha256": sha(runner_bytes),
    "cpp_fixture_sha256": sha(cpp_bytes),
    "rust_fixture_sha256": sha(rust_bytes),
}
for key, actual in post_readback.items():
    if metadata["comparand"][key] != actual:
        raise SystemExit(f"{key} post-readback mismatch: {actual}")
overlay_hashes = {item["path"]: item["sha256"] for item in metadata["rugra_source"]["overlays"]}
if sha(action_bytes) != overlay_hashes["src/action.rs"]:
    raise SystemExit("action.rs snapshot changed during gate")
if sha(arch_bytes) != overlay_hashes["src/arch.rs"]:
    raise SystemExit("arch.rs snapshot changed during gate")
...
ghidra = parse_stdout(paths["ghidra_stdout"])
rugra = parse_stdout(paths["rugra_stdout"])
expected_cases = {"positive_change", "zero_change"}
if set(ghidra) != expected_cases or set(rugra) != expected_cases:
    raise SystemExit("fixture case set mismatch")
for case_name in sorted(expected_cases):
    for key in ("trace", "rule_stats", "op"):
        if ghidra[case_name][key] != rugra[case_name][key]:
            raise SystemExit(f"redispatch projection mismatch: {case_name}.{key}")
    if ghidra[case_name]["trace"] != "change>new_head>new_tail":
        raise SystemExit(f"old opcode tail was not abandoned: {case_name}")

if ghidra["positive_change"]["apply_return"] != "0":
    raise SystemExit("Ghidra positive apply return drifted")
if rugra["positive_change"]["apply_return"] != "1":
    raise SystemExit("registered Rugra positive apply-return residual drifted")
if ghidra["positive_change"]["action_count"] != "1":
    raise SystemExit("Ghidra positive Action::count drifted")
if rugra["positive_change"]["action_count"] != "unavailable":
    raise SystemExit("registered Rugra Action::count visibility residual drifted")
if ghidra["positive_change"]["action_status"] != "1":
    raise SystemExit("Ghidra positive Action::status drifted")
if rugra["positive_change"]["action_status"] != "unavailable":
    raise SystemExit("registered Rugra Action::status visibility residual drifted")
for side in (ghidra, rugra):
    if side["zero_change"]["apply_return"] != "0":
        raise SystemExit("zero-result apply return drifted")

expected_diagnostic = b"ERROR: Rule change changed op without returning result of 1!\n"
if paths["ghidra_stderr"].read_bytes() != expected_diagnostic:
    raise SystemExit("Ghidra raw diagnostic drifted")
if paths["rugra_stderr"].read_bytes() != expected_diagnostic:
    raise SystemExit("Rugra raw diagnostic differs from locked oracle")

observation = metadata["observation"]
for key in (
    "positive_redispatch_status",
    "zero_result_redispatch_status",
    "zero_result_raw_diagnostic_status",
    "target_op_state_status",
):
    if observation[key] != "MATCH":
        raise SystemExit(f"projection status drifted: {key}")
if observation["overall_status"] != "MISMATCH":
    raise SystemExit("complete ActionPool must remain MISMATCH")
PY

/usr/bin/cat "$oracle_tmp/ghidra.stdout"
/usr/bin/cat "$oracle_tmp/ghidra.stderr"
/usr/bin/cat "$oracle_tmp/full.stdout.diff"
/usr/bin/printf 'action_opcode_redispatch_1204: target_projection=MATCH raw_diagnostic=MATCH overall=MISMATCH\n'

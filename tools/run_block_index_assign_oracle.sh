...
runner="$repo_root/tools/run_block_index_assign_oracle.sh"
...
metadata="$repo_root/tests/oracle/block_index_assign_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/block_index_assign_1204.cc"
rust_fixture="$repo_root/tests/oracle/block_index_assign_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-block-index-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-block-index-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/block_index_assign_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/block_index_assign_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/block_index_assign_1204.rs",
...
crate_hasher.update(b"rugra-block-index-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/block_index_assign_1204.cc"),
    pathlib.Path("tests/oracle/block_index_assign_1204.rs"),
    pathlib.Path("tests/oracle/block_index_assign_1204.metadata.json"),
    pathlib.Path("docs/api/block.md"),
    pathlib.Path("tools/run_block_index_assign_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "BLOCK-INDEX-ASSIGN-0001")
require(
    "stable function id",
...
require(
    ...
    "sha256 of rugra-block-index-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("single_entry_tree_forward_cross_classification", "MATCH"),
    ("multi_entry_rootlist_head_tail_swap", "MATCH"),
    ("no_root_assume_first_block", "MATCH"),
    ("unreachable_two_pass_extraroots", "MATCH"),
    ("irreducible_prelabel_wipe", "MATCH"),
    ("index_distinct_in_range", "MATCH"),
    ("stale_root_removal_branch", "UNTESTED"),
    ("irreducible_skip_branch", "UNTESTED"),
...
registry_hasher.update(b"rugra-block-index-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/block_index_assign_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/block_index_assign_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/block_index_assign_1204.rs"
...
cpp_binary="$oracle_tmp/block_index_assign_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "block_index_assign_1204: GHIDRA_LOCKED_OUTPUT_OK overall=MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/block_index_assign_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=BLOCK-INDEX-ASSIGN-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "empty_graph_early_return", "single_block", "single_entry_dag_corrupt_reset",
    "multi_entry_rootlist_swap", "no_root_assume_first_loop",
    "unreachable_extraroots_twopass", "stale_root_machinery",
    "irreducible_prelabel_wiped",
]
...
metadata = json.loads(
    (repo / "tests/oracle/block_index_assign_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_block_index_assign_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/block_index_assign_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/block_index_assign_1204.rs").read_bytes()),
...
hasher.update(b"rugra-block-index-base-overlay-v1\0")
...
echo "block_index_assign_1204: MATCH covered_projection=8/8 overall=MATCH stdout_sha256=$actual_stdout_sha"

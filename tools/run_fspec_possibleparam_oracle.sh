...
# FSPEC-POSSIBLEPARAM-1204 oracle runner (pin-base schema2): rebuilds
# the locked Ghidra 12.0.4 decompiler and the pinned Rugra base plus
# the candidate src/fspec.rs overlay in an isolated snapshot, runs both
# fixtures byte-for-byte, and verifies every comparand hash in the
# metadata. Covered projection: ParamListStandardOut::possibleParam
# iterates the raw entry list with no caller-level space filter and no
# minSize gate (join entries reachable through the per-piece join walk,
# >= 0 acceptance, both space-guard routes), and
# ParamEntry::assumedExtension's cc:377 justifiedContain call is
# space-aware (foreign-space numerically-justified query -> CPUI_COPY),
# with the cc:376 join guard, the sz gates, the list minSize skip,
# both container shapes, and the cc:389-393 flag precedence
# (FSPEC-POSSIBLEPARAM-JOIN-0006, A46 residual 3).
...
runner="$repo_root/tools/run_fspec_possibleparam_oracle.sh"
...
rugra_base_commit=b8c70e804fe33dbb9137bced7ce7b164646d2c03
rugra_base_tree=a7176a65cb2f9dc39eb1979cc8a4db7a93443032
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/fspec_possibleparam_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/fspec_possibleparam_1204.cc"
rust_fixture="$repo_root/tests/oracle/fspec_possibleparam_1204.rs"
...
if [[ "$actual_base_commit" != "$rugra_base_commit" || \
  ...
  echo "pinned Rugra base identity mismatch: commit=$actual_base_commit tree=$actual_base_tree" >&2
  exit 1
fi
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-fspec-possibleparam-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-fspec-possibleparam-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/fspec_possibleparam_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/fspec_possibleparam_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/fspec_possibleparam_1204.rs",
...
crate_hasher.update(b"rugra-fspec-possibleparam-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/fspec_possibleparam_1204.cc"),
    pathlib.Path("tests/oracle/fspec_possibleparam_1204.rs"),
    pathlib.Path("tests/oracle/fspec_possibleparam_1204.metadata.json"),
    pathlib.Path("docs/api/fspec.md"),
    pathlib.Path("tools/run_fspec_possibleparam_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "FSPEC-POSSIBLEPARAM-1204")
...
require(
    ...
    "sha256 of rugra-fspec-possibleparam-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("plain_space_guards", "MATCH"),
    ("join_reachability", "MATCH"),
    ("cross_space_join", "MATCH"),
    ("assumed_extension_gates", "MATCH"),
    ("extension_flag_precedence", "MATCH"),
    ("be_space_rows", "UNTESTED"),
    ("remaining_spaceless_callers", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-fspec-possibleparam-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/fspec_possibleparam_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/fspec_possibleparam_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/fspec_possibleparam_1204.rs"
...
cpp_binary="$oracle_tmp/fspec_possibleparam_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "fspec_possibleparam_1204: GHIDRA_LOCKED_OUTPUT_OK covered=MATCH overall=UNTESTED stdout_sha256=$actual_stdout_sha"
  exit 0
fi

fixture_target=/home/wirs/.cache/a85-fspin-target/fspec-possibleparam
...
rust_binary="$oracle_tmp/fspec_possibleparam_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=FSPEC-POSSIBLEPARAM-1204|"
...
echo "fspec_possibleparam_1204: OK covered=MATCH overall=UNTESTED ghidra=$actual_stdout_sha"

...
# unjustifiedContainer/assumedExtension, sortTrials operator< ladder,
# splitHi/splitLo address+flags, splitTrial slot rebuild.
...
runner="$repo_root/tools/run_fspec_phase0_oracle.sh"
...
rust_fixture="$repo_root/tests/oracle/fspec_phase0_1204.rs"
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/fspec_phase0_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/fspec_phase0_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/fspec_phase0_1204.rs",
...
special_paths = [
    pathlib.Path("tests/oracle/fspec_phase0_1204.cc"),
    pathlib.Path("tests/oracle/fspec_phase0_1204.rs"),
    pathlib.Path("tests/oracle/fspec_phase0_1204.metadata.json"),
    pathlib.Path("docs/api/fspec.md"),
    pathlib.Path("tools/run_fspec_phase0_oracle.sh"),
]
special = {}
...
for required_key, required_prefix in (
    ("cross_space_find_entry", "MATCH"),
    ("unjustified_container", "MATCH"),
    ("assumed_extension", "MATCH"),
    ("sort_trials", "MATCH"),
    ("split_hi_lo", "MATCH"),
    ("split_trial", "MATCH"),
...
snapshot_cpp="$snapshot_root/tests/oracle/fspec_phase0_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/fspec_phase0_1204.rs"
...
cpp_binary="$oracle_tmp/fspec_phase0_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if lines[0] != (
    "schema=1|fixture=FSPEC-PHASE0-1204|"
...
require(
    "live runner",
    sha((repo / "tools/run_fspec_phase0_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/fspec_phase0_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/fspec_phase0_1204.rs").read_bytes()),

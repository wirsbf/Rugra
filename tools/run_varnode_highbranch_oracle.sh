...
# Authoritative oracle runner for VARNODE-COPYSYMBOL-HIGHBRANCH-0001:
# the high!=0 bookkeeping half of Varnode::copySymbol (varnode.cc:500-504)
# driven through Varnode::copySymbolIfValid (varnode.cc:510-522),
# Varnode::copySymbol directly, and the PcodeOp::collapseConstantSymbol
# (op.cc:503-540) markedInput integration.
#
# Comparand model: pinned Rugra base commit plus the live overlay files of
# this lease's write-set (src/varnode.rs, src/op.rs), so concurrent WIP in
# other agents' domains never leaks into this comparand.
...
runner="$repo_root/tools/run_varnode_highbranch_oracle.sh"
...
rugra_base_commit=a42a9b76f55328809222400e23345b088fd8ae1a
rugra_base_tree=0e68d7ae9ca2eaf97d9c9f50b4d2e1a805ecc965
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/varnode_highbranch_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/varnode_highbranch_1204.cc"
rust_fixture="$repo_root/tests/oracle/varnode_highbranch_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$varnode_rs" "$op_rs" "$varnode_doc" "$op_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-varnode-highbranch-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-varnode-highbranch-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S - \
  ...
  "$metadata" "$cpp_fixture" "$rust_fixture" "$varnode_rs" "$op_rs" \
  "$varnode_doc" "$op_doc" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, varnode_raw, op_raw,
    varnode_doc_raw, op_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/varnode_highbranch_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/varnode_highbranch_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/varnode_highbranch_1204.rs",
    pathlib.Path(varnode_raw): repo / "src/varnode.rs",
    pathlib.Path(op_raw): repo / "src/op.rs",
    pathlib.Path(varnode_doc_raw): repo / "docs/api/varnode.md",
    pathlib.Path(op_doc_raw): repo / "docs/api/op.md",
}
...
crate_hasher.update(b"rugra-varnode-highbranch-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/varnode_highbranch_1204.cc"),
    pathlib.Path("tests/oracle/varnode_highbranch_1204.rs"),
    pathlib.Path("tests/oracle/varnode_highbranch_1204.metadata.json"),
    pathlib.Path("docs/api/varnode.md"),
    pathlib.Path("docs/api/op.md"),
    pathlib.Path("tools/run_varnode_highbranch_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "VARNODE-COPYSYMBOL-HIGHBRANCH-0001")
require(
    "stable function id",
...
observed_hashes = {
    ...
    "op_rs_sha256": sha(crate_bytes["src/op.rs"]),
    "varnode_doc_sha256": sha(special[special_paths[3].as_posix()]),
    "op_doc_sha256": sha(special[special_paths[4].as_posix()]),
...
require(
    ...
    "sha256 of rugra-varnode-highbranch-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("copysymbol_high_close_propagates", "MATCH"),
    ("copysymbol_high_not_close_clean", "MATCH"),
    ("copysymbol_high_null_mapentry_dirty_only", "MATCH"),
    ("copysymbol_high_dst_no_high_guard", "MATCH"),
    ("collapse_marked_input_high_setsymbol", "MATCH"),
    ("funcdata_dedup_callsite_manual_high", "NO_ORACLE"),
    ("ruleaction_addunsigned_callsite_high_gap", "MISMATCH"),
):
...
require(
    "residual todo ids",
    set(residual_ids),
    {"RULEACTION-ADDUNSIGNED-COPYSYMBOL-HIGH-0001", "VARIABLE-GETTYPE-LAZY-UPDATETYPE-0001"},
)
mismatch_entries = [
...
require("MISMATCH entries exist", len(mismatch_entries) >= 1, True)
...
registry_hasher.update(b"rugra-varnode-highbranch-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/varnode_highbranch_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/varnode_highbranch_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/varnode_highbranch_1204.rs"
...
cpp_binary="$oracle_tmp/varnode_highbranch_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "varnode_highbranch_1204: GHIDRA_LOCKED_OUTPUT_OK covered_projection=MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/varnode_highbranch_1204_rust"
...
if len(lines) != 5:
    raise SystemExit(f"expected five fixture lines, found {len(lines)}")
...
metadata = json.loads(
    (repo / "tests/oracle/varnode_highbranch_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_varnode_highbranch_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
for relative, key in (
    ("tests/oracle/varnode_highbranch_1204.cc", "cpp_fixture_sha256"),
    ("tests/oracle/varnode_highbranch_1204.rs", "rust_fixture_sha256"),
    ("src/varnode.rs", "varnode_rs_sha256"),
    ("src/op.rs", "op_rs_sha256"),
    ("docs/api/varnode.md", "varnode_doc_sha256"),
    ("docs/api/op.md", "op_doc_sha256"),
):
    require(relative, sha((repo / relative).read_bytes()), comparand[key])

overlay_files = {"src/varnode.rs", "src/op.rs"}
...
hasher.update(b"rugra-varnode-highbranch-base-overlay-v1\0")
...
echo "varnode_highbranch_1204: covered_projection=5/5 MATCH residual=RULEACTION-ADDUNSIGNED-COPYSYMBOL-HIGH-0001,VARIABLE-GETTYPE-LAZY-UPDATETYPE-0001 overall=UNTESTED stdout_sha256=$actual_stdout_sha"

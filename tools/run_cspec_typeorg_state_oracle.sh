...
# CSPEC-TYPEORG-STATE-0001 oracle runner: TypeFactory data-organization
# state parity.  Compiles the locked Ghidra 12.0.4 oracle fixture (real
# BfdArchitecture chain: spec-dir scan -> Architecture::init ->
# parseCompilerConfig -> types->decodeDataOrganization + setupSizes) and an
# isolated base-plus-overlay Rust snapshot (fixture-local Element DOM ->
# TreeDecoder -> TypeFactory::decode_data_organization +
# setup_sizes(locked arch inputs)), runs both cspec_typeorg_state_1204
# fixtures on identical locked x86-64-gcc.cspec bytes plus synthetic
# data_organization documents, and diffs the state projections byte for
# byte.
...
cpp_fixture="$repo_root/tests/oracle/cspec_typeorg_state_1204.cc"
rust_fixture="$repo_root/tests/oracle/cspec_typeorg_state_1204.rs"
metadata="$repo_root/tests/oracle/cspec_typeorg_state_1204.metadata.json"
runner="$repo_root/tools/run_cspec_typeorg_state_oracle.sh"
typefactory_rs="$repo_root/src/type_system/typefactory.rs"
typefactory_doc="$repo_root/docs/api/type_system/typefactory.md"
...
for required in "$cpp_fixture" "$rust_fixture" "$metadata" "$runner" \
  "$typefactory_rs" "$typefactory_doc" "$bfd_include/bfd.h" "$bfd_library" "$oracle_archive"; do
...
python3 -I - "$metadata" "$cpp_fixture" "$rust_fixture" "$runner" \
  "$typefactory_rs" "$typefactory_doc" "$oracle_commit" <<'PY'
import hashlib
import json
import pathlib
import sys
...
keys = [
    ...
    "typefactory_rs_sha256", "typefactory_doc_sha256",
]
...
oracle_tmp=$(mktemp -d /tmp/rugra-cspec-typeorg-state-1204.XXXXXX)
cleanup() {
  ...
  case "$oracle_tmp" in
    /tmp/rugra-cspec-typeorg-state-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
cp "$typefactory_rs" "$rugra_overlay/src/type_system/typefactory.rs"
...
python3 -I - "$metadata" "$repo_root" "$rugra_base_commit" "$spec_root" "$binary" \
  ...
  "$rugra_overlay/src/type_system/typefactory.rs" <<'PY'
import hashlib
import json
import pathlib
import subprocess
...
if ! g++ -std=c++11 -O0 -Wall -Wno-sign-compare -m64 \
  ...
  "$bfd_library" -lz -o "$oracle_tmp/cspec_typeorg_state_1204_cpp" \
...
"$oracle_tmp/cspec_typeorg_state_1204_cpp" "$spec_root" "$binary" \
...
if document.get("schema") != 1 or document.get("fixture") != "CSPEC-TYPEORG-STATE-0001":
    raise SystemExit("invalid fixture envelope")
if document.get("done") != 1:
    raise SystemExit("fixture did not complete")
if document.get("arch_inputs") != {
    "default_size": 8,
    "stack_spacebase_size": 8,
    "default_data_space_addr_size": 8,
    "far_pointer": 0,
}:
    raise SystemExit(f"locked arch inputs mismatch: {document.get('arch_inputs')}")
production = document.get("production", {})
if [production.get("int"), production.get("long"), production.get("char"),
    production.get("wchar"), production.get("pointer"),
    production.get("alt_pointer")] != [4, 8, 1, 4, 8, 0]:
    raise SystemExit(f"production sizes mismatch: {production}")
align = production.get("align", [])
if len(align) == 18 and align[0] != -1:
    raise SystemExit(f"production align[0] must stay -1: {align[0]}")
cases = {case["name"]: case for case in document.get("cases", [])}
if len(cases) != 8:
    raise SystemExit(f"expected 8 cases, found {sorted(cases)}")
if cases["c2_empty_map"].get("align_error") != \
        "TypeFactory alignment map not initialized":
    raise SystemExit("empty-map oracle error text mismatch")
if cases["c7_negative_setup"].get("setup", {}).get("int") != -2:
    raise SystemExit("negative size did not survive the oracle decode")
if cases["c8_sam_followed_by_child"].get("decoded", {}).get("char") != 3:
    raise SystemExit("oracle dropped the child after <size_alignment_map>")
...
if $ghidra_only; then
  ...
  printf 'cspec_typeorg_state_1204: GHIDRA_LOCKED_OUTPUT_OK\n'
  exit 0
fi
...
rustc --edition=2021 -O -L "dependency=$cargo_target/debug/deps" \
  ...
  "$rust_fixture" -o "$oracle_tmp/cspec_typeorg_state_1204_rust"

"$oracle_tmp/cspec_typeorg_state_1204_rust" \
  "$spec_root/x86-64-gcc.cspec" \
  >"$oracle_tmp/rugra.stdout"
diff -u "$oracle_tmp/ghidra.stdout" "$oracle_tmp/rugra.stdout"
cat "$oracle_tmp/ghidra.stdout"
printf 'cspec_typeorg_state_1204: DECLARED_OBSERVATIONS_MATCH\n'

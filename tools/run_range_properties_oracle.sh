...
runner="$repo_root/tools/run_range_properties_oracle.sh"
...
rugra_base_commit=07efbff6af58fbea91b27a99fb3ecfbe39690e19
rugra_base_tree=c3ea7960436d72c9fce3631f9b36c3e49b1b4265
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/range_properties_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/range_properties_1204.cc"
rust_fixture="$repo_root/tests/oracle/range_properties_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$address_rs" "$address_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-range-properties-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-range-properties-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C GIT_CONFIG_NOSYSTEM=1 \
  ...
  "$address_rs" "$address_doc" "$runner_fd_path" "$runner_snapshot_sha" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, address_raw, doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/range_properties_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/range_properties_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/range_properties_1204.rs",
    pathlib.Path(address_raw): repo / "src/address.rs",
    pathlib.Path(doc_raw): repo / "docs/api/address.md",
}
...
crate_hasher.update(b"rugra-range-properties-base-overlay-v1\0")
crate_hasher.update(rugra_base_commit.encode())
crate_bytes = {}
for relative in crate_files:
    data = live_file(relative) if relative == pathlib.Path("src/address.rs") else base_file(relative)
...
special_paths = [
    pathlib.Path("tests/oracle/range_properties_1204.cc"),
    pathlib.Path("tests/oracle/range_properties_1204.rs"),
    pathlib.Path("tests/oracle/range_properties_1204.metadata.json"),
    pathlib.Path("docs/api/address.md"),
    pathlib.Path("tools/run_range_properties_oracle.sh"),
]
special = {}
...
observed_hashes = {
    ...
    "address_doc_sha256": sha(special[special_paths[3].as_posix()]),
...
require(
    ...
    "sha256 of rugra-range-properties-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
require(
    "abstract Decoder unknown coverage",
    coverage["abstract_decoder_supplied_unknown_id_159"],
    "MATCH: the fixture's abstract Decoder returns raw locked unknown attribute id 159, and decode continues to later attributes",
)
require(
    "TreeDecoder unknown coverage",
    coverage["tree_decoder_unregistered_unknown_attribute"],
    "MATCH: the real TreeDecoder/XmlDecode path maps the unregistered attribute name xmlunknown-attr to the locked unknown attribute id 159 and traversal continues to later attributes, observed directly by this fixture",
)
...
registry_hasher.update(b"rugra-range-properties-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/range_properties_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/range_properties_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/range_properties_1204.rs"
...
cpp_binary="$oracle_tmp/range_properties_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
  "$host_cxx_bin" -std=c++11 -O0 -Wall -Wextra -Werror \
    -Wno-unused-parameter -Wno-sign-compare -fno-strict-aliasing \
    -I"$oracle_cpp" "$snapshot_cpp" "$oracle_cpp/libdecomp.a" -lz \
...
if [[ -s "$oracle_tmp/cxx.stdout" || -s "$oracle_tmp/cxx.stderr" ]]; then
  echo "C++ fixture compilation emitted diagnostics" >&2
  /usr/bin/cat "$oracle_tmp/cxx.stdout" >&2
  /usr/bin/cat "$oracle_tmp/cxx.stderr" >&2
  exit 1
fi
...
if $ghidra_only; then
  ...
  echo "range_properties_1204: GHIDRA_LOCKED_OUTPUT_OK overall=MISMATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/range_properties_1204_rust"
...
if len(lines) != 12:
    raise SystemExit(f"expected twelve fixture lines, found {len(lines)}")
if lines[0] != (
    "schema=1|fixture=CSPEC-RANGEPROPS-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = [
    "defaults", "ordered_unknown", "name_then_space", "range_name",
    "duplicates", "persistent_first", "persistent_second",
    "invalid_after_success", "child_unvisited", "u64_boundary",
...
metadata = json.loads(
    (repo / "tests/oracle/range_properties_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_range_properties_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/range_properties_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/range_properties_1204.rs").read_bytes()),
    comparand["rust_fixture_sha256"],
)
require(
    "address implementation",
    sha((repo / "src/address.rs").read_bytes()),
    comparand["address_rs_sha256"],
)
require(
    "address API document",
    sha((repo / "docs/api/address.md").read_bytes()),
    comparand["address_doc_sha256"],
)
...
echo "range_properties_1204: MATCH covered_projection=11/11 overall=MISMATCH stdout_sha256=$actual_stdout_sha"

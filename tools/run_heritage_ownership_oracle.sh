...
runner="$repo_root/tools/run_heritage_ownership_oracle.sh"
...
rugra_base_commit=e034f80ba06d9acff0952ee50be4be39c3f8580a
rugra_base_tree=107446db17295e9d774d96f0d3ee79721da8993e
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/heritage_ownership_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/heritage_ownership_1204.cc"
rust_fixture="$repo_root/tests/oracle/heritage_ownership_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$heritage_rs" "$funcdata_rs" "$heritage_doc" "$funcdata_doc" \
  "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-heritage-ownership-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-heritage-ownership-1204.?????? ]]; then
...
(
    ...
    heritage_doc_raw, funcdata_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/heritage_ownership_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/heritage_ownership_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/heritage_ownership_1204.rs",
...
crate_hasher.update(b"rugra-heritage-ownership-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/heritage_ownership_1204.cc"),
    pathlib.Path("tests/oracle/heritage_ownership_1204.rs"),
    pathlib.Path("tests/oracle/heritage_ownership_1204.metadata.json"),
    pathlib.Path("docs/api/heritage.md"),
    pathlib.Path("docs/api/funcdata.md"),
    pathlib.Path("tools/run_heritage_ownership_oracle.sh"),
]
special = {}
...
require(
    ...
    "sha256 of rugra-heritage-ownership-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("ownership_bridge_single_pass_per_call", "MATCH"),
    ("no_lock_reentry", "MATCH"),
    ("empty_entry_ir_inert", "MATCH"),
    ("register_free_promote_full_projection", "MATCH"),
    ("later_pass_inertness", "MATCH"),
    ("adt_rebuild_sentinel", "MATCH"),
    ("persistent_globaldisjoint", "MATCH"),
    ("old_recipe_difference_record", "RECORDED"),
    ("phi_cycle_full_projection", "MISMATCH"),
    ("production_action_switch", "UNTESTED"),
    ("guardcalls_discovery_loadguards", "UNTESTED"),
):
...
registry_hasher.update(b"rugra-heritage-ownership-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/heritage_ownership_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/heritage_ownership_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/heritage_ownership_1204.rs"
...
cpp_binary="$oracle_tmp/heritage_ownership_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "heritage_ownership_1204: GHIDRA_LOCKED_OUTPUT_OK overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/heritage_ownership_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=HERITAGE-OWNERSHIP-0001|"
    "oracle=e40ed13014025f82488b1f8f7bca566894ac376b"
):
    raise SystemExit("fixture envelope mismatch")
expected_cases = ["empty_entry", "register_free_promote", "phi_cycle_selfref"]
...
metadata = json.loads(
    (repo / "tests/oracle/heritage_ownership_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_heritage_ownership_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
for label, relative, key in (
    ("C++ fixture", "tests/oracle/heritage_ownership_1204.cc", "cpp_fixture_sha256"),
    ("Rust fixture", "tests/oracle/heritage_ownership_1204.rs", "rust_fixture_sha256"),
...
overlay_files = {"src/heritage.rs", "src/funcdata.rs"}
...
hasher.update(b"rugra-heritage-ownership-base-overlay-v1\0")
...
echo "heritage_ownership_1204: MATCH covered_projection=3/3 overall=PARTIAL_MATCH stdout_sha256=$actual_stdout_sha"

...
metadata="$repo_root/tests/oracle/ptrarith_addtree_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/ptrarith_addtree_1204.cc"
rust_fixture="$repo_root/tests/oracle/ptrarith_addtree_1204.rs"
runner="$repo_root/tools/run_ptrarith_addtree_oracle.sh"
...
oracle_tmp=$(mktemp -d /tmp/rugra-ptrarith-addtree-1204.XXXXXX)
cleanup() {
  case "$oracle_tmp" in
    /tmp/rugra-ptrarith-addtree-1204.??????) rm -rf -- "$oracle_tmp" ;;
...
python3 -I -S - "$repo_root" "$snapshot_root" "$metadata" "$cpp_fixture" \
  ...
  "$rugra_input_blob" "$bfd_include/bfd.h" "$bfd_library" <<'PY'
import hashlib
import json
import pathlib
import shutil
...
expected_paths = {
    metadata_path: repo_root / "tests/oracle/ptrarith_addtree_1204.metadata.json",
    cpp_fixture_path: repo_root / "tests/oracle/ptrarith_addtree_1204.cc",
    rust_fixture_path: repo_root / "tests/oracle/ptrarith_addtree_1204.rs",
    runner_path: repo_root / "tools/run_ptrarith_addtree_oracle.sh",
}
for actual, expected in expected_paths.items():
    if actual != expected.resolve():
        raise SystemExit(f"unexpected runner input path: {actual} != {expected.resolve()}")
...
crate_hasher.update(b"rugra-ptrarith-addtree-lib-snapshot-v1\0")
crate_bytes = {}
...
special_paths = [
    pathlib.Path("tests/oracle/ptrarith_addtree_1204.cc"),
    pathlib.Path("tests/oracle/ptrarith_addtree_1204.rs"),
    pathlib.Path("tests/oracle/ptrarith_addtree_1204.metadata.json"),
    pathlib.Path("tools/run_ptrarith_addtree_oracle.sh"),
]
special = {path.as_posix(): snapshot_file(path) for path in special_paths}
require(
    "runner snapshot/live copy",
    sha256(special["tools/run_ptrarith_addtree_oracle.sh"]),
    runner_sha,
)
...
metadata = json.loads(special["tests/oracle/ptrarith_addtree_1204.metadata.json"])
require("metadata schema", metadata["schema_version"], 1)
require("fixture id", metadata["fixture_id"], "RULE-PTRARITH-ADDTREE-0001")
...
observed = {
    "cpp_fixture_sha256": sha256(special["tests/oracle/ptrarith_addtree_1204.cc"]),
    "rust_fixture_sha256": sha256(special["tests/oracle/ptrarith_addtree_1204.rs"]),
...
require(
    ...
    "sha256 of rugra-ptrarith-addtree-lib-snapshot-v1 plus sorted length-prefixed relative paths and contents",
)
...
g++ -std=c++11 -O2 -Wall -Wno-sign-compare \
  ...
  "$snapshot_root/tests/oracle/ptrarith_addtree_1204.cc" \
...
printf 'ptrarith_addtree_1204: MATCH target_structural_cases=5 records=10 dependencies=OPBANK-0001,ARCH-0001\n'

...
# Authoritative oracle runner for VARNODE-COPYSYMBOL-FIELDS-0001:
# Funcdata::opSetInput constant single-reader dedup branch calling
# Varnode::copySymbol (funcdata_op.cc:104-125, varnode.cc:493-505).
#
# Comparand model: pinned Rugra base commit plus a single live overlay file
# (src/funcdata.rs, the task's write-set), so concurrent WIP in other agents'
# domains never leaks into this comparand.
...
runner="$repo_root/tools/run_varnode_copy_symbol_oracle.sh"
...
cpp_fixture="$repo_root/tests/oracle/varnode_copy_symbol_1204.cc"
rust_fixture="$repo_root/tests/oracle/varnode_copy_symbol_1204.rs"
...
for required_file in "$metadata" "$cpp_fixture" "$rust_fixture" \
  "$funcdata_rs" "$funcdata_doc" "$runner"; do
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-varnode-copysymbol-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-varnode-copysymbol-1204.?????? ]]; then
...
/usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_python_bin" -I -S - \
  ...
  "$metadata" "$cpp_fixture" "$rust_fixture" "$funcdata_rs" "$funcdata_doc" \
...
(
    ...
    metadata_raw, cpp_raw, rust_raw, funcdata_raw, funcdata_doc_raw,
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/varnode_copy_symbol_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/varnode_copy_symbol_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/varnode_copy_symbol_1204.rs",
...
crate_hasher.update(b"rugra-varnode-copysymbol-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/varnode_copy_symbol_1204.cc"),
    pathlib.Path("tests/oracle/varnode_copy_symbol_1204.rs"),
    pathlib.Path("tests/oracle/varnode_copy_symbol_1204.metadata.json"),
    pathlib.Path("docs/api/funcdata.md"),
    pathlib.Path("tools/run_varnode_copy_symbol_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "VARNODE-COPYSYMBOL-FIELDS-0001")
require(
    "stable function id",
...
observed_hashes = {
    ...
    "funcdata_doc_sha256": sha(special[special_paths[3].as_posix()]),
...
require(
    ...
    "sha256 of rugra-varnode-copysymbol-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("opsetinput_dedup_locks_and_type", "MATCH"),
    ("opsetinput_dedup_no_locks_baseline", "MATCH"),
    ("opsetinput_dedup_mapentry_lock_bits_only", "MATCH"),
    ("opsetinput_identity_early_return", "MATCH"),
    ("opsetinput_spacebase_exemption", "MATCH"),
    ("copysymbol_high_type_dirty_and_setsymbol", "NO_ORACLE"),
    ("funcdata_newconstant_assignhigh", "NO_ORACLE"),
):
...
registry_hasher.update(b"rugra-varnode-copysymbol-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/varnode_copy_symbol_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/varnode_copy_symbol_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/varnode_copy_symbol_1204.rs"
...
cpp_binary="$oracle_tmp/varnode_copy_symbol_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "varnode_copy_symbol_1204: GHIDRA_LOCKED_OUTPUT_OK overall=MATCH stdout_sha256=$actual_stdout_sha"
  exit 0
fi
...
rust_binary="$oracle_tmp/varnode_copy_symbol_1204_rust"
...
expected_lines = [
    "locks_type:op1_is_src=1,op2_is_src=0,cvn_flags=16777986,cvn_tl=1,cvn_nl=1,type_same=1,type_size=4,type_meta=int,cvn_mapped=0,cvn_mapentry=0,off=1234,sz=4,src_flags=16777986,src_descend=1,cvn_descend=1,src_tl=1,src_nl=1",
    "no_locks:op1_is_src=1,op2_is_src=0,cvn_flags=16777218,cvn_tl=0,cvn_nl=0,type_same=1,type_size=4,type_meta=unknown,cvn_mapped=0,cvn_mapentry=0,off=5a5a,sz=4,src_flags=16777218,src_descend=1,cvn_descend=1,src_tl=0,src_nl=0",
    "mapentry_symbol:op1_is_src=1,op2_is_src=0,cvn_flags=16777986,cvn_tl=1,cvn_nl=1,type_same=1,type_size=4,type_meta=int,cvn_mapped=0,cvn_mapentry=1,off=1234,sz=4,src_flags=18875138,src_descend=1,cvn_descend=1,src_tl=1,src_nl=1",
    "mapentry_copy:sym_name=eq0,entry_offset=0,entry_same=1",
    "identity_return:op1_is_src=1,src_descend=1,src_flags=16777218",
    "spacebase_exempt:op1_is_src=1,op2_is_src=1,src_descend=2,src_flags=16908290",
]
...
metadata = json.loads(
    (repo / "tests/oracle/varnode_copy_symbol_1204.metadata.json").read_text(encoding="utf-8")
)
...
require(
    "live runner",
    sha((repo / "tools/run_varnode_copy_symbol_oracle.sh").read_bytes()),
    comparand["runner_sha256"],
)
require(
    "C++ fixture",
    sha((repo / "tests/oracle/varnode_copy_symbol_1204.cc").read_bytes()),
    comparand["cpp_fixture_sha256"],
)
require(
    "Rust fixture",
    sha((repo / "tests/oracle/varnode_copy_symbol_1204.rs").read_bytes()),
    comparand["rust_fixture_sha256"],
)
require(
    "funcdata implementation",
    sha((repo / "src/funcdata.rs").read_bytes()),
    comparand["funcdata_rs_sha256"],
)
require(
    "funcdata API document",
    sha((repo / "docs/api/funcdata.md").read_bytes()),
    comparand["funcdata_doc_sha256"],
)

overlay_files = {"src/funcdata.rs"}
...
hasher.update(b"rugra-varnode-copysymbol-base-overlay-v1\0")
...
echo "varnode_copy_symbol_1204: MATCH locks_type=1 no_locks=1 mapentry_symbol=1 mapentry_copy=1 identity_return=1 spacebase_exempt=1 overall=MATCH stdout_sha256=$actual_stdout_sha"

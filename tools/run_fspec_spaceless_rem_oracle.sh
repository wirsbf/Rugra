...
# FSPEC-SPACELESS-REM-1204 oracle runner (pin-base schema2): rebuilds
...
# metadata. Covered projection: the last two spaceless
# justifiedContain callers — ParamListStandard::unjustifiedContainer
# (fspec.cc:1411-1424: minSize gate before the walk, foreign-space
# rejection inside the walk, join PIECE containers, just==0 early
# false) and ParamListStandardOut::fillinMapFallback's two trial
# queries (fspec.cc:1656/cc:1702: space-aware per-piece matching, join
# output entries reachable, best-loop overwrite, offmatch contiguity,
# minSize coverage gate, type/cover preference, firstOnly skip)
# (FSPEC-SPACELESS-REMAINDER, A46 residual 4).
...
runner="$repo_root/tools/run_fspec_spaceless_rem_oracle.sh"
...
rugra_base_commit=fce01555544a1ccb19f82ac31a2d7593b9f0640b
rugra_base_tree=37bc660cdefa47c5844d596d00caad8b8edd810a
ghidra_root="$repo_root/ghidra"
metadata="$repo_root/tests/oracle/fspec_spaceless_rem_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/fspec_spaceless_rem_1204.cc"
rust_fixture="$repo_root/tests/oracle/fspec_spaceless_rem_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-fspec-spaceless-rem-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-fspec-spaceless-rem-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo / "tests/oracle/fspec_spaceless_rem_1204.metadata.json",
    pathlib.Path(cpp_raw): repo / "tests/oracle/fspec_spaceless_rem_1204.cc",
    pathlib.Path(rust_raw): repo / "tests/oracle/fspec_spaceless_rem_1204.rs",
...
crate_hasher.update(b"rugra-fspec-spaceless-rem-base-overlay-v1\0")
...
special_paths = [
    pathlib.Path("tests/oracle/fspec_spaceless_rem_1204.cc"),
    pathlib.Path("tests/oracle/fspec_spaceless_rem_1204.rs"),
    pathlib.Path("tests/oracle/fspec_spaceless_rem_1204.metadata.json"),
    pathlib.Path("docs/api/fspec.md"),
    pathlib.Path("tools/run_fspec_spaceless_rem_oracle.sh"),
]
special = {}
...
require("fixture id", metadata["fixture_id"], "FSPEC-SPACELESS-REM-1204")
...
require(
    ...
    "sha256 of rugra-fspec-spaceless-rem-base-overlay-v1 plus base commit and sorted length-prefixed paths and contents",
)
...
for required_key, required_prefix in (
    ("uc_plain_space_guards", "MATCH"),
    ("uc_join_reachability", "MATCH"),
    ("fb_plain_best_cover", "MATCH"),
    ("fb_join_reachable", "MATCH"),
    ("fb_cross_space_join", "MATCH"),
    ("fb_first_only", "MATCH"),
...
registry_hasher.update(b"rugra-fspec-spaceless-rem-registry-lock-v1\0")
...
snapshot_metadata="$snapshot_root/tests/oracle/fspec_spaceless_rem_1204.metadata.json"
snapshot_cpp="$snapshot_root/tests/oracle/fspec_spaceless_rem_1204.cc"
snapshot_rust="$snapshot_root/tests/oracle/fspec_spaceless_rem_1204.rs"
...
cpp_binary="$oracle_tmp/fspec_spaceless_rem_1204_cpp"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C \
...
if $ghidra_only; then
  ...
  echo "fspec_spaceless_rem_1204: GHIDRA_LOCKED_OUTPUT_OK covered=MATCH overall=UNTESTED stdout_sha256=$actual_stdout_sha"
  exit 0
fi

fixture_target=/tmp/rugra-target-fspec-writer/fspec-spaceless-rem
...
rust_binary="$oracle_tmp/fspec_spaceless_rem_1204_rust"
...
if lines[0] != (
    "schema=1|fixture=FSPEC-SPACELESS-REM-1204|"
...
echo "fspec_spaceless_rem_1204: OK covered=MATCH overall=UNTESTED ghidra=$actual_stdout_sha"

...
runner="$repo_root/tools/run_varnode_add_descend_oracle.sh"
...
metadata="$repo_root/tests/oracle/varnode_add_descend_1204.metadata.json"
cpp_fixture="$repo_root/tests/oracle/varnode_add_descend_1204.cc"
rust_fixture="$repo_root/tests/oracle/varnode_add_descend_1204.rs"
...
oracle_tmp=$(/usr/bin/mktemp -d /tmp/rugra-varnode-adddescend-1204.XXXXXX)
cleanup() {
  if [[ "$oracle_tmp" != /tmp/rugra-varnode-adddescend-1204.?????? ]]; then
...
expected_paths = {
    pathlib.Path(metadata_raw): repo_root / "tests/oracle/varnode_add_descend_1204.metadata.json",
    pathlib.Path(cpp_fixture_raw): repo_root / "tests/oracle/varnode_add_descend_1204.cc",
    pathlib.Path(rust_fixture_raw): repo_root / "tests/oracle/varnode_add_descend_1204.rs",
    pathlib.Path(runner_raw): repo_root / "tools/run_varnode_add_descend_oracle.sh",
}
...
crate_hasher.update(b"rugra-varnode-adddescend-lib-snapshot-v1\0")
crate_bytes = {}
...
special_files = [
    pathlib.Path("tests/oracle/varnode_add_descend_1204.cc"),
    pathlib.Path("tests/oracle/varnode_add_descend_1204.rs"),
    pathlib.Path("tests/oracle/varnode_add_descend_1204.metadata.json"),
    pathlib.Path("tools/run_varnode_add_descend_oracle.sh"),
...
metadata = json.loads(
    special_bytes["tests/oracle/varnode_add_descend_1204.metadata.json"].decode("utf-8")
)
...
observed_hashes = {
    "cpp_fixture_sha256": sha256_bytes(special_bytes["tests/oracle/varnode_add_descend_1204.cc"]),
    "rust_fixture_sha256": sha256_bytes(special_bytes["tests/oracle/varnode_add_descend_1204.rs"]),
...
require_equal(
    ...
    sha256_bytes(special_bytes["tools/run_varnode_add_descend_oracle.sh"]),
    runner_snapshot_sha,
)
require_equal(
    "crate snapshot scheme", comparand["rust_crate_tree_hash_scheme"],
    "sha256 of rugra-varnode-adddescend-lib-snapshot-v1 plus sorted length-prefixed relative paths and contents",
)
...
require_equal(
    ...
    "MATCH: free-multi-descendant throw text, spacebase/written/input accumulation and push order byte-match the locked oracle",
)
...
metadata="$snapshot_root/tests/oracle/varnode_add_descend_1204.metadata.json"
cpp_fixture="$snapshot_root/tests/oracle/varnode_add_descend_1204.cc"
rust_fixture="$snapshot_root/tests/oracle/varnode_add_descend_1204.rs"
...
/usr/bin/cp -- "$cpp_fixture" "$cpp_fixture_dir/varnode_add_descend_1204.cc"
cpp_fixture="$cpp_fixture_dir/varnode_add_descend_1204.cc"
if ! /usr/bin/env -i PATH="$clean_path" LC_ALL=C "$host_cxx_bin" \
  ...
  -o "$oracle_tmp/varnode_add_descend_1204_cpp" \
...
if ! /usr/bin/env -i HOME="$user_home" RUSTUP_HOME="$user_home/.rustup" \
  ...
  "$rust_fixture" -o "$oracle_tmp/varnode_add_descend_1204_rust" \
...
if [[ "$diff_status" -ne 0 ]]; then
  echo "expected byte-identical addDescend observations, diff exit=$diff_status" >&2
...
if len(ghidra_lines) != 7 or len(rugra_lines) != 7:
    raise SystemExit(
        f"expected 7 observation lines, found Ghidra={len(ghidra_lines)} Rugra={len(rugra_lines)}"
    )
required_lines = [
    "free_first:count=1,flags=16777216",
    "free_second_throw:error=Free varnode has multiple descendants,count=1,flags=16777216",
    "spacebase_exempt:count=2,flags=16908288",
    "written_accumulate:count=2,flags=16777264",
    "input_accumulate:count=2,flags=16777256",
    "const_second_throw:error=Free varnode has multiple descendants,count=1,flags=16777218",
    "order:written=1,2,",
]
if ghidra_lines != required_lines:
    raise SystemExit(f"Ghidra observations are not the exact pinned lines: {ghidra_lines!r}")
if rugra_lines != required_lines:
    raise SystemExit(f"Rugra observations are not the exact pinned lines: {rugra_lines!r}")
if direct_diff != "":
    print(direct_diff, file=sys.stderr)
    raise SystemExit("direct diff is not empty")
...
/usr/bin/printf 'varnode_add_descend_1204: MATCH lines=7 throw_text=2 spacebase_exempt=1 written_accumulate=1 input_accumulate=1 order=1\n'

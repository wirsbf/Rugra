/*
 * Locked Ghidra 12.0.4 RuleMultiCollapse::applyOp abort-shape oracle for
 * RULE-MULTICOLLAPSE-ABORT-0001.
 *
 ...
 *
 * The const_pair_distinct_objects case runs applyOp in a forked child: the
 * locked e40ed130 oracle takes a null-pointer dereference at
 * ruleaction.cc:3306 (defcopyr is an unwritten constant), so the parent
 * records the observed child termination and continues with the byte-exact
 * comparison cases.
 */
...
// RULE-MULTICOLLAPSE-ABORT-0001: the functional-equality CSE branch of
// RuleMultiCollapse unconditionally dereferences defcopyr->getDef()
// (ruleaction.cc:3306).  A MULTIEQUAL whose branches are two DISTINCT
// same-valued constant objects selects the unwritten constant as defcopyr
// (ruleaction.cc:3254-3257), functionalEquality(defcopyr,copyr) returns
// true for const-const equal offsets (expression.cc:409-412), and the
// locked e40ed130 oracle then dereferences null inside applyOp.  This is
// reproduced for the record: the case runs in a forked child so the parent
// can print the crash observation and continue with the remaining cases.
PcodeOp *buildConstPairDistinct(Funcdata &fd,Fixture &fixture)
{
  fd.clear();
  BlockBasic *block = fixture.makeBlock("b0");
  Varnode *c0a = fixture.makeConstant("c0a",1,0);
  Varnode *c0b = fixture.makeConstant("c0b",1,0);
  PcodeOp *root = fixture.makeOp("root",CPUI_MULTIEQUAL,2,1);
  PcodeOp *use = fixture.makeOp("use",CPUI_COPY,1,1);
  fixture.setInput(root,c0a,0);
  fixture.setInput(root,c0b,1);
...
void runConstPairDistinct(Funcdata &fd)
{
  // before-stage state is printed by the child so both sides share it.
  // Flush first: the child inherits this stream buffer, and an unflushed
  // parent buffer would be duplicated by the child's own flush.
  std::cout.flush();
  pid_t child = fork();
  if (child == 0) {
    Fixture fixture(fd);
    PcodeOp *root = buildConstPairDistinct(fd,fixture);
    fixture.dump("const_pair_distinct_objects","before","na");
    std::cout.flush();
    RuleMultiCollapse rule("analysis");
    rule.applyOp(root,fd); // locked oracle: null deref (SIGSEGV)
    std::cout << "case=const_pair_distinct_objects|stage=after|result=COMPLETED_UNEXPECTEDLY\n";
    std::cout.flush();
    _exit(0);
  }
  int4 status = 0;
  ...
  if (WIFSIGNALED(status) && WTERMSIG(status) == SIGSEGV)
    ...
    observation = "exit" + std::to_string(WEXITSTATUS(status));
  else
    observation = "unknown";
  std::cout << "case=const_pair_distinct_objects|stage=after|result="
            << observation << "|note=oracle_null_def_deref_ruleaction.cc:3306\n";
  std::cout.flush();
  fd.clear();
}

// A constant branch plus an input branch: functionalEquality(const,input)
// is -1 and an unwritten input is not a MULTIEQUAL, so the rule must report
// no change on both sides.
void runConstNonconstMix(Funcdata &fd)
{
  fd.clear();
  Fixture fixture(fd);
  BlockBasic *block = fixture.makeBlock("b0");
  ...
  fixture.insertEnd(root,block);
  fixture.insertEnd(use,block);
  fixture.dump("const_nonconst_mix_no_change","before","na");
  RuleMultiCollapse rule("analysis");
  int4 result = rule.applyOp(root,fd);
  fixture.dump("const_nonconst_mix_no_change","after",std::to_string(result));
}

// Functional-equality collapse whose skiplist holds a nested MULTIEQUAL:
// processing skiplist entry zero destroys the root through the CSE
// substitute while entry one still has to re-read defcopyr->getDef()
// (ruleaction.cc:3306 is inside the skiplist loop).  The defining branch's
// defining op must stay alive across the mid-loop opDestroy calls.
void runFuncEqSkiplistDefPreserved(Funcdata &fd)
{
  fd.clear();
  Fixture fixture(fd);
  ...
  Varnode *thirdConstant = fixture.makeConstant("third_const",8,5);
  ...
  PcodeOp *thirdAdd = fixture.makeOp("third_add",CPUI_INT_ADD,2,8);
  ...
  fixture.setInput(thirdAdd,x,0);
  fixture.setInput(thirdAdd,thirdConstant,1);
  fixture.setInput(nested,leftAdd->getOut(),0);
  fixture.setInput(nested,thirdAdd->getOut(),1);
  ...
  fixture.insertEnd(thirdAdd,right);
  fixture.insertEnd(nested,merge);
  fixture.insertEnd(root,merge);
  fixture.insertEnd(cse,merge);
  fixture.insertEnd(nestedUse,merge);
  fixture.insertEnd(rootUse,merge);
  fixture.dump("func_eq_skiplist_def_preserved","before","na");
  RuleMultiCollapse rule("analysis");
  int4 result = rule.applyOp(root,fd);
  fixture.dump("func_eq_skiplist_def_preserved","after",std::to_string(result));
}
...
void run(const string &specDirectory,const string &binary)
{
  ...
  startDecompilerLibrary(specPaths);
  {
    ...
    runConstNonconstMix(*fd);
    runFuncEqSkiplistDefPreserved(*fd);
    runConstPairDistinct(*fd);
  }
...
{
  if (argc != 3) {
    std::cerr << "usage: rule_multi_collapse_abort_1204 SPEC_ROOT CURL_BINARY\n";
    return 2;
  }
  try {
    run(argv[1],argv[2]);

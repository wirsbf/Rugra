//! Rust counterpart of the locked `flow_tailcall_overtrace_1204` oracle
//! fixture (Ghidra 12.0.4, FLOW-TAILCALL-OVERTRACE-0001).
//!
//! Mirrors the C++ projection exactly: call specs (op/entry deltas plus the
//! resolved display name setFuncdata copies), warning comments (via a shared
//! in-memory CommentDatabaseInternal attached through `Funcdata::set_arch`,
//! the analogue of Ghidra's Architecture::commentdb), the block graph and
//! every op in bank order with its address delta. Address spelling inside
//! the PIC warning text is projected to a base-relative delta on both sides
//! (Rugra's legacy flow Address has no space tag; ADDRESS-0001).
//!
//! The symbol seed (the queryFunction equivalent) is Rugra's driver-seeded
//! `Funcdata::symbol_table`: the same front-end boundary the curl driver's
...
const CASES: [&str; 2] = ["tailover_sym_user", "tailover_offcut_user"];
...
fn spec_line(index: usize, fc: &FuncCallSpecs, base: u64) -> String {
    ...
    // Ghidra projects fc->getName(): the ctor-derived name until queryCall's
    // setFuncdata (fspec.cc:4957-4958) copies the resolved callee's display
    // name. Rugra's query_call runs the same set_funcdata copy, so surviving
    // specs print the same resolved names on both sides.
    format!(
        "spec={} op_delta={} entry={} name={}",
        index,
...
fn observe(
    ...
    // Ghidra's BfdArchitecture.readLoaderSymbols("::") populates the symbol
    // database that FlowInfo::queryCall's queryFunction (flow.cc:660) reads.
    // Rugra's front-end equivalent is the driver-seeded symbol table.
    ...
    // optree is the same BTreeSet with the same SeqNum ordering. The address
    // delta makes the visited pollution observable: ops addressed inside the
    // tail-jumped callee body appear with positive deltas beyond the
    // caller's own bytes.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 9 {
        return Err(
            "usage: flow_tailcall_overtrace_1204 TEXT TEXT_BASE SYM_USER SYM_CALLEE USER_ADDR USER_SIZE OFFCUT_ADDR OFFCUT_SIZE".into(),
        );
    }

    let image = fs::read(&args[1])?;
    let image_base = parse_u64(&args[2])?;
    let sym_user = parse_u64(&args[3])?;
    let sym_callee = parse_u64(&args[4])?;
    for (i, name) in CASES.iter().enumerate() {
        let address = parse_u64(&args[5 + 2 * i])?;
        let size = parse_u64(&args[6 + 2 * i])? as usize;
        // The loader-symbol seed for queryCall: both fixture functions are
        // real ELF symbols on the Ghidra side (readLoaderSymbols); the offcut
        // control's interior label is a symbol on neither side.
        let symbols: Vec<(u64, &str)> = vec![
            (sym_user, "tailover_sym_user"),
            (sym_callee, "tailover_sym_callee"),
        ];
        observe(name, &image, image_base, address, size, &symbols)?;
    }
    Ok(())
}

...
| 53 | `xml.cc` + `marshal.cc` | `marshal.rs` | 🔧 L2 | 结构骨架存在，但 12.0.4 固定进程级 ID/名称表被实例注册顺序取代（0 被误当 UNKNOWN）；Translate 共享 ID 也错，Packed open/close/skip/error/raw-byte 状态机未对齐 | `xml.cc`, `marshal.cc` |

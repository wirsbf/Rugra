...
| # | Ghidra 模块 | Rugra 模块 | 状态 | 差距说明 | Ghidra 源码参考 |
|---|---|---|---|---|---|
| 17 | `blockaction.cc` (2366行) | `blockaction.rs` (4560行) | 🔧 **L2（G4 核心完成，循环检测已修复）** | identifyInternal/selfIdentify ✅；ruleBlockCat/ProperIf/IfElse/WhileDo/DoWhile/Goto ✅；**2026-06-28 重大修复**：移植 `findSpanningTree`（block.cc:1009-1110）DFS 边分类，回边检测从支配者法改为 Ghidra 式 F_BACK_EDGE 标记（curl main 回边 0→19）；保护回边不被 goto cascade 切断（对齐 TraceDAG 跳过 loop edges）；goto_cascade 内层补 WhileDo/DoWhile（对齐 collapseInternal）。**循环检测层 L3**。**2026-06-28 collapseInternal 移植实验结论**：忠实移植 Ghidra collapseInternal 两层 repeat-until-stable（blockaction.cc:1768-1851）替换自定义多阶段后，循环结构化**退步**（8→1 循环）。原因是旧自定义阶段的 structure_loops_first/collapse_loops 虽不忠实架构但实际工作。**已回退保留旧实现**（铁律 8）。**剩余 while 缺口根因转移**：curl main 只检测到 3 回边（全 head=5，1 循环），而 Ghidra main 有 6 个 while（6 循环）。**根因在 CFG 构建层**（基本块划分/边建立），非纯结构化。cat-chain 类型限制（只允许 Basic/Copy，Ghidra 允许任意非 switch 块）也是因素之一，但单独放宽未提升 while | `blockaction.cc` |

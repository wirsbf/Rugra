...
| 35 | `prettyprint.cc` | `prettyprint.rs` (3020行) | ✅ **L3（2026-07-04 恢复）** | **Emit trait 层对齐** + **post_process_output 改为忠实空操作**（对齐 Ghidra EmitMarkup 零后处理）。原 27+ 趟文本后处理（违反铁律 5.5）已移除（保留为 `post_process_output_legacy` dead code 供参考）。移除后 curl 仍 24/24 反编译、23/24 gcc——无回归。 | `prettyprint.cc` |

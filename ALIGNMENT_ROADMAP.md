...
| 13 | `merge.cc` | `merge.rs` | ✅ **L3（2026-07-04 完整对齐；2026-08-15 补 `update_high_cover` 自锁修复）** | merge 子系统全部 public/private 方法已移植（mergeOpcode/mergeTestRequired/snip 子系统/dominant-copy/SUBPIECE 影子/hideShadows/mergeOp per-op forced-merge/redundant-copy 标记）。**2026-08-15 `COVER-REBUILD-SELFLOCK-0001`**：`update_high_cover` 逐 instance 改调 `Varnode::update_cover_locked`，消除持 root 写锁再入 `Cover::rebuild` 的生产自锁（parseconfig 真实输入 MULTIEQUAL slot2 同 Arc 拓扑），行为门禁归 cover_rebuild_1204 fixture。已知简化：build_dominant_copy/allocate_copy_trim 的 union 解析路径省略（需 inheritResolution/forceFacingType/getUnionField 基础设施）；mergeRangeMust 用 merge_test_must+merge_force 近似（Ghidra 失败 throw，Rugra 跳过）。 | `merge.cc` |
...
| 19 | `condexe.cc` | `condexe.rs` | 🔧 L2 | **2026-08-23 CONDEXE-GAPS 审计勘误**：PathMeld 归因错误（condexe.cc 零引用）；forceSpecific/removeBlockEdges/setOut 在锁定 oracle 不存在；"238 行只检测"已过时（现 1555 行，20/27 函数结构 MATCH 但全 UNTESTED）。真实缺口：1 MISSING（buildHeritageArray，condexe.cc:23-37）+6 MISMATCH（pullbackOp storage/begin、doReplacement 死循环+throw 吞、discoverPathIsTrue 判反）+2 Phase-0 共享缺陷（remove_from_flow_split 序列反+越界 panic、get_true/false_out 极性反+重复翻转）。op destroy/insert 双向不变量本身核实忠实。TODO=CONDEXE-{CFG,TRUEOUT,ACTION,HERITAGE,PULLBACK,ERROR,FIXTURE}-0001..0007 | `condexe.cc`, `expression.cc` |
...
| # | Ghidra 模块 | Rugra 模块 | 状态 | 差距说明 | Ghidra 源码参考 |
|---|---|---|---|---|---|
| 28 | `type.cc` (4674行) | `type_system/datatype.rs` + `typefactory.rs` | 🔧 L2 | 2026-08-21 R3：datatype 层 submeta、公开虚派发 compare/dependency、Pointer/PointerRel state+space、TypeCode model/flags、Spacebase identity/invalid、Unicode1 已有 72-record locked 12.0.4 MATCH 投影并获 scoped Cross-Review APPROVE。整体仍 MISMATCH：6 个 TypeFactory producer 分歧已实测；TypeCode/Array/Union/Partial/Struct/space/24-submeta 完整矩阵仍 UNTESTED，分别绑定 `TYPEFACTORY-POINTER-CANONICAL-0001`、`TYPEFACTORY-SUBMETA-RECLASS-0001`、`DATATYPE-TYPEORDER-RESIDUAL-0001`；不得升 L3 | `type.cc` |

...
| # | Ghidra 模块 | Rugra 模块 | 状态 | 差距说明 | Ghidra 源码参考 |
|---|---|---|---|---|---|
| 17 | `blockaction.cc` (2366行) | `blockaction.rs` (4560行) | 🔧 **L2（G4 核心完成，循环检测已修复，结构化仍有缺口）** | identifyInternal/selfIdentify ✅；ruleBlockCat/ProperIf/IfElse/WhileDo/DoWhile/Goto ✅；**2026-06-28 重大修复**：移植 `findSpanningTree`（block.cc:1009-1110）DFS 边分类，回边检测从支配者法改为 Ghidra 式 F_BACK_EDGE 标记（curl main 回边 0→19）；保护回边不被 goto cascade 切断（对齐 TraceDAG 跳过 loop edges）；LoopBody 完整移植 + orderLoopBodies pipeline + apply_loop_exit_marks + TraceDAG 集成 + run_goto_cascade。**循环检测层 L3**。**剩余结构化缺口（L2）**：循环体（多块）未被折叠成 try_rule_while_do 能识别的单 clause——curl 只产出 4 while（Ghidra 34）。根因：cat-chain 因循环体内部多入边/多出边而跳过，循环体内部结构化（proper_if/if_else）不够充分，无法把循环体折叠成单入单出回边的 BlockList。需移植 Ghidra 完整规则集顺序（Goto→Cat→ProperIf→IfElse→WhileDo）+ repeat-until-stable 充分收敛 + 可能缺 ruleBlockInfLoop | `blockaction.cc` |

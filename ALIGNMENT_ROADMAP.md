...
| 21 | **`jumptable.cc`** | — | 🔧 L2 | **骨架已移植**：LoadTable/PathMeld/GuardRecord/JumpValues(+Range/RangeDefault)/JumpModel trait/JumpModelTrivial/JumpBasic/JumpTable/EmulateFunction 全部数据结构 + 守卫分析 + 最小规范化变量查找。**Varnode::def 深度遍历已实现**（find_determining_varnodes DFS、quasi_copy COPY/INT_AND/INT_OR/SEXT/ZEXT/PIECE/SUBPIECE 链、get_max_value INT_AND/MULTIEQUAL 检查、isLoadInPath LOAD 检测）。L3 缺：emulate_path 地址计算、pullBack、CFG 重写（foldInGuards/switchOver） | `jumptable.cc` |

...
| 25 | `transform.cc` | `transform.rs` | 🔧 L2 | 五阶段名称相同但内部事务不同：piece storage/endian/property/IOP 丢失，MULTIEQUAL 插入点错误，INDIRECT 标记 no-op，preexisting inputs/xref 与 nullable slot/bank 清理破坏；非法 lane/getPiece 应抛错却继续 | `transform.cc` |
